...
[db_table("users")]
message User {
    [db_column("id"), db_primary_key(1), db_generated(identity)] uint64 id;
...
- TBE `default` 只允许布尔、整数、浮点、字符串和 enum 常量。编译器根据字段类型生成 SQL literal，不接受任意 SQL 表达式。
...
| `uint64` 及别名 | `NUMERIC` + 值域 `CHECK` | `numeric(20,0)` + 值域 `CHECK` |
| `float`/`f32` | `REAL` | `real` |
...
SQLite 的 `INTEGER PRIMARY KEY` 对应 rowid，只有这种单列有符号整数主键可以使用 `AUTOINCREMENT`。PostgreSQL 的 identity 使用 `GENERATED BY DEFAULT AS IDENTITY`。`uint64` identity 因 PostgreSQL identity 不支持 `numeric(20,0)`，v1 明确拒绝。
...
- SQLite 集成测试把生成结果交给真实 SQLite API 执行，并通过 catalog/插入约束验证表结构。
