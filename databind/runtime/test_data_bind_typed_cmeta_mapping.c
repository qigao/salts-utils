...
static DataBindStatus scalar_descriptor_status(const cmeta_data_desc *value) {
  ...
  const DataBindTypedField overlay_field = {
      .name = "wire_value", .kind = DATA_BIND_TYPED_OBJECT,
      .wire_kind = DATA_BIND_TYPED_U8};
  const DataBindTypedType overlay = {
      .name = "ScalarStorage", .size = 1u, .fields = &overlay_field,
      .field_count = 1u};
  const DataBindTypedDescriptor descriptor =
      DATA_BIND_TYPED_DESCRIPTOR_INIT(&overlay, &root_data);
  DataBindError error = DATA_BIND_ERROR_INIT;
  return data_bind_typed_descriptor_validate(&descriptor, &error);
}
