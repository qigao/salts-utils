...
spec("generated string canonical native lowering") {
  it("joins Semantic IR generated C storage to canonical CMeta lifecycle") {
    const cmeta_data_desc *native = NULL;
    DataBindNativeDescriptor descriptor =
        DATA_BIND_NATIVE_DESCRIPTOR_INIT("NativeStringRecord", NULL);
    ...
    descriptor.native_data = native;
    check_equal(data_bind_native_descriptor_validate(&descriptor, &error), DATA_BIND_OK);
    ...
    if (native != NULL && workspace != NULL) {
      ...
      check_equal(data_bind_native_init(&options, descriptor.native_data,
                                        &value, sizeof(value), &diagnostic),
                  DATA_BIND_OK);
      check_null(value.text);

      value.id = 7u;
      value.text = tstr_dup("semantic-native");
      check_not_null(value.text);
      if (value.text != NULL) {
        check_equal(data_bind_native_clear(&options, descriptor.native_data,
                                           &value, sizeof(value), &diagnostic),
                    DATA_BIND_OK);
