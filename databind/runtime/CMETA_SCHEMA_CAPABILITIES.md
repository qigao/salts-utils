...
| signed integers | `CMETA_DATA_SINT` | CMeta core `cmeta_data_intN` descriptors | supported runtime/generated native graphs | All `intN_t` / `intN` / `iN` spellings for N=8/16/32/64 preserve exact width; never use platform `long` for schema `int64`. |
| unsigned integers | `CMETA_DATA_UINT` | CMeta core `cmeta_data_uintN` descriptors | supported runtime/generated native graphs | All `uintN_t` / `uintN` / `uN` spellings for N=8/16/32/64, plus `byte`=uint8. |
...
| `map<string,V>` | `CMETA_DATA_MAP` | generated C emits an explicit `cmeta_data_map_ops` provider when `V` has a complete canonical descriptor | schema reflection remains kind-only; generated native graphs expose canonical `tstr` key and `V` data | Generated maps are unique-key, insertion-ordered vectors. The provider exposes bounded K/V traversal and never infers metadata from erased `vec_t` storage. |
