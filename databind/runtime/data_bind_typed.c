#include "data_bind_typed_internal.h"
...
#ifdef _MSC_VER
typedef union data_bind_typed_max_align {
  long double long_double;
  void *pointer;
  long long integer;
} data_bind_typed_max_align_t;
  #define DATA_BIND_TYPED_MAX_ALIGNOF _Alignof(data_bind_typed_max_align_t)
#else
  #define DATA_BIND_TYPED_MAX_ALIGNOF _Alignof(max_align_t)
#endif

typedef struct DataBindTypedCMetaKindMapping {
  const cmeta_data_desc *data;
  DataBindTypedKind kind;
} DataBindTypedCMetaKindMapping;
...
static int typed_is_integer(DataBindTypedKind kind) {
  return kind >= DATA_BIND_TYPED_I8 && kind <= DATA_BIND_TYPED_U64;
}

static DataBindTypedKind typed_enum_storage_kind(DataBindTypedKind wire_kind) {
  return typed_is_integer(wire_kind) ? wire_kind : DATA_BIND_TYPED_I32;
}

static size_t typed_kind_size(DataBindTypedKind kind) {
  switch (kind) {
  case DATA_BIND_TYPED_BOOL:
  case DATA_BIND_TYPED_I8:
  case DATA_BIND_TYPED_U8:
    return 1;
  case DATA_BIND_TYPED_I16:
  case DATA_BIND_TYPED_U16:
    return 2;
  case DATA_BIND_TYPED_I32:
  case DATA_BIND_TYPED_U32:
  case DATA_BIND_TYPED_F32:
  case DATA_BIND_TYPED_ENUM:
    return 4;
  case DATA_BIND_TYPED_I64:
  case DATA_BIND_TYPED_U64:
  case DATA_BIND_TYPED_F64:
    return 8;
  case DATA_BIND_TYPED_UUID:
...
static DataBindStatus typed_validate_descriptor_at(const DataBindTypedType *type, unsigned depth,
                                                   DataBindError *error);

static int typed_optional_present(const DataBindTypedType *type, const void *object,
                                  const DataBindTypedField *field) {
  const uint8_t *presence;
  if ((field->flags & DATA_BIND_TYPED_FIELD_OPTIONAL) == 0) return 1;
...
static void typed_optional_set(const DataBindTypedType *type, void *object, const DataBindTypedField *field) {
  uint8_t *presence;
  if ((field->flags & DATA_BIND_TYPED_FIELD_OPTIONAL) == 0 || type->presence_size == 0) return;
  presence = (uint8_t *)object + type->presence_offset;
  presence[field->optional_bit / 8u] |= (uint8_t)(1u << (field->optional_bit % 8u));
}

static DataBindStatus typed_init_value(DataBindTypedKind kind, const DataBindTypedType *object_type,
                                       void *ptr, size_t count, DataBindError *error) {
  size_t i;
  if (kind == DATA_BIND_TYPED_STRING) {
    for (i = 0; i < count; ++i)
      ((tstr *)ptr)[i] = NULL;
  } else if (kind == DATA_BIND_TYPED_BYTES) {
    for (i = 0; i < count; ++i) {
  ...
  } else if (kind == DATA_BIND_TYPED_OBJECT) {
    for (i = 0; i < count; ++i) {
      DataBindStatus status =
          data_bind_typed_init(object_type, (uint8_t *)ptr + i * object_type->size, error);
...
DataBindStatus data_bind_typed_init(const DataBindTypedType *type, void *object, DataBindError *error) {
  size_t i;
  ...
  for (i = 0; i < type->field_count; ++i) {
    const DataBindTypedField *field = &type->fields[i];
    void *ptr = (uint8_t *)object + field->offset;
    DataBindStatus status;
    if (field->kind == DATA_BIND_TYPED_FIXED_ARRAY) {
      status =
          typed_init_value(field->element_kind, field->object_type, ptr, field->fixed_count, error);
    } else if (field->kind == DATA_BIND_TYPED_BYTES || field->kind == DATA_BIND_TYPED_LIST ||
               field->kind == DATA_BIND_TYPED_SET || field->kind == DATA_BIND_TYPED_MAP) {
      status =
          vec_init_bytes((vec_t *)ptr,
                         field->kind == DATA_BIND_TYPED_BYTES ? sizeof(uint8_t) : field->element_size,
...
static void typed_clear_value(DataBindTypedKind kind, const DataBindTypedType *object_type, void *ptr,
                              size_t count) {
  size_t i;
  if (kind == DATA_BIND_TYPED_STRING) {
    for (i = 0; i < count; ++i)
  ...
  } else if (kind == DATA_BIND_TYPED_OBJECT && object_type != NULL) {
    for (i = 0; i < count; ++i)
      data_bind_typed_clear(object_type, (uint8_t *)ptr + i * object_type->size);
  }
}

void data_bind_typed_clear(const DataBindTypedType *type, void *object) {
  size_t i;
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    if (field->kind == DATA_BIND_TYPED_FIXED_ARRAY) {
    ...
    } else if (field->kind == DATA_BIND_TYPED_LIST || field->kind == DATA_BIND_TYPED_SET) {
      vec_t *vec = (vec_t *)ptr;
      typed_clear_value(field->element_kind, field->object_type, vec->data, vec->size);
      vec_destroy(vec);
    } else if (field->kind == DATA_BIND_TYPED_MAP) {
      vec_t *vec = (vec_t *)ptr;
      size_t j;
      ...
      vec_destroy(vec);
    } else if (field->kind == DATA_BIND_TYPED_BYTES) {
...
static DataBindStatus typed_read_scalar(DataBindTypedKind kind, DataBindTypedKind wire_kind,
  ...
  if (kind == DATA_BIND_TYPED_F32 || kind == DATA_BIND_TYPED_F64) {
  ...
  if (kind == DATA_BIND_TYPED_ENUM) kind = typed_enum_storage_kind(wire_kind);
  if (kind == DATA_BIND_TYPED_U64) {
...
static DataBindStatus typed_from_value_at(const DataBindTypedType *type, const DataBindValue *value,
                                          void *object, const char *prefix, DataBindError *error);

static DataBindStatus typed_from_one(DataBindTypedKind kind, DataBindTypedKind wire_kind,
                                     const DataBindTypedType *object_type, const DataBindValue *value,
  ...
  if (kind == DATA_BIND_TYPED_OBJECT) return typed_from_value_at(object_type, value, out, "", error);
...
static DataBindStatus typed_from_value_at(const DataBindTypedType *type, const DataBindValue *value,
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    if (child == NULL && field->kind == DATA_BIND_TYPED_OBJECT && field->object_type != NULL) {
      ...
      if ((field->flags & DATA_BIND_TYPED_FIELD_OPTIONAL) != 0) continue;
      return status;
    }
    if (child == NULL) {
      if ((field->flags & DATA_BIND_TYPED_FIELD_OPTIONAL) != 0) continue;
    ...
    typed_optional_set(type, object, field);
    if (field->kind == DATA_BIND_TYPED_BYTES || field->kind == DATA_BIND_TYPED_FIXED_BYTES) {
      const uint8_t *bytes;
      size_t len;
      ...
      if (field->kind == DATA_BIND_TYPED_FIXED_BYTES) {
    ...
    } else if (field->kind == DATA_BIND_TYPED_FIXED_ARRAY) {
      ...
      for (j = 0; j < count; ++j) {
        ...
        if (field->element_kind == DATA_BIND_TYPED_OBJECT || field->element_kind == DATA_BIND_TYPED_BYTES) {
      ...
      for (j = 0; j < count; ++j) {
        ...
        *(tstr *)(entry + field->map_key_offset) = tstr_dup(item.key);
        ...
        if (field->map_value_kind == DATA_BIND_TYPED_OBJECT || field->map_value_kind == DATA_BIND_TYPED_BYTES) {
          status = typed_init_value(field->map_value_kind, field->map_value_type,
                                    entry + field->map_value_offset, 1, error);
          if (status != DATA_BIND_OK) return status;
        }
        status =
            typed_from_one(field->map_value_kind, field->map_value_wire_kind, field->map_value_type,
                           item.value, entry + field->map_value_offset, field->name, error);
        if (status != DATA_BIND_OK) return status;
      }
    } else {
...
DataBindStatus data_bind_typed_from_value(const DataBindTypedType *type, const DataBindValue *value,
  ...
  status = data_bind_typed_init(type, temporary, error);
  ...
  data_bind_typed_clear(type, temporary);
  free(temporary);
  return status;
}

static json_value_t *typed_scalar_json(DataBindTypedKind kind, DataBindTypedKind wire_kind, const void *ptr,
                                       const char *path, DataBindError *error) {
  if (kind == DATA_BIND_TYPED_BOOL) return json_create_bool(*(const uint8_t *)ptr != 0);
  if (kind == DATA_BIND_TYPED_STRING) {
  ...
  if (kind == DATA_BIND_TYPED_F32) {
    ...
    if (!isfinite(value)) {
      typed_error(error, DATA_BIND_ERR_TYPE_MISMATCH, path, "Typed float must be finite for JSON");
      return NULL;
    }
    return json_create_number(value);
  }
  if (kind == DATA_BIND_TYPED_F64) {
    double value = *(const double *)ptr;
    if (!isfinite(value)) {
      typed_error(error, DATA_BIND_ERR_TYPE_MISMATCH, path, "Typed double must be finite for JSON");
      return NULL;
    }
    return json_create_number(value);
  }
...
static json_value_t *typed_json_created(json_value_t *value, const char *path,
                                        DataBindError *error) {
  if (value == NULL)
    typed_error(error, DATA_BIND_ERR_OOM, path, "Out of memory creating canonical JSON value");
  return value;
}
...
static json_value_t *typed_bytes_json(const uint8_t *data, size_t len, const char *path,
  ...
  return typed_json_created(json_create_string_n((const char *)data, len), path, error);
}

static json_value_t *typed_one_json(DataBindTypedKind kind, DataBindTypedKind wire_kind,
                                    const DataBindTypedType *object_type, const void *ptr,
                                    const char *path, DataBindError *error) {
  if (kind == DATA_BIND_TYPED_OBJECT) return data_bind_typed_to_json(object_type, ptr, error);
  if (kind == DATA_BIND_TYPED_BYTES) {
...
json_value_t *data_bind_typed_to_json(const DataBindTypedType *type, const void *object,
                                DataBindError *error) {
  json_value_t *root;
  size_t i;
  if (type == NULL || object == NULL) {
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    if (field->kind == DATA_BIND_TYPED_BYTES || field->kind == DATA_BIND_TYPED_FIXED_BYTES) {
      const uint8_t *data;
      size_t len;
      if (field->kind == DATA_BIND_TYPED_BYTES) {
        const vec_t *vec = (const vec_t *)ptr;
    ...
    } else if (field->kind == DATA_BIND_TYPED_FIXED_ARRAY || field->kind == DATA_BIND_TYPED_LIST ||
               field->kind == DATA_BIND_TYPED_SET) {
      const uint8_t *data;
      size_t count;
      ...
      for (j = 0; child != NULL && j < vec->size; ++j) {
        ...
        item =
            typed_one_json(field->map_value_kind, field->map_value_wire_kind, field->map_value_type,
...
void data_bind_typed_json_free(json_value_t **value) {
  if (!value) return;
  json_free(*value);
  *value = NULL;
}

static int typed_scalar_kind_from_name(const char *name, DataBindTypedKind *kind) {
  if (name == NULL || kind == NULL) return 0;
  if (strcmp(name, "bool") == 0) *kind = DATA_BIND_TYPED_BOOL;
  else if (strcmp(name, "int8_t") == 0 || strcmp(name, "int8") == 0 || strcmp(name, "i8") == 0)
    *kind = DATA_BIND_TYPED_I8;
  else if (strcmp(name, "uint8_t") == 0 || strcmp(name, "uint8") == 0 || strcmp(name, "u8") == 0 ||
           strcmp(name, "byte") == 0)
    *kind = DATA_BIND_TYPED_U8;
  else if (strcmp(name, "int16_t") == 0 || strcmp(name, "int16") == 0 || strcmp(name, "i16") == 0)
    *kind = DATA_BIND_TYPED_I16;
    ...
    *kind = DATA_BIND_TYPED_I64;
  else if (strcmp(name, "uint64_t") == 0 || strcmp(name, "uint64") == 0 || strcmp(name, "u64") == 0)
    *kind = DATA_BIND_TYPED_U64;
  else if (strcmp(name, "float") == 0) *kind = DATA_BIND_TYPED_F32;
  else if (strcmp(name, "double") == 0) *kind = DATA_BIND_TYPED_F64;
  else if (strcmp(name, "string") == 0) *kind = DATA_BIND_TYPED_STRING;
  else if (strcmp(name, "bytes") == 0) *kind = DATA_BIND_TYPED_BYTES;
  else if (strcmp(name, "uuid") == 0) *kind = DATA_BIND_TYPED_UUID;
  else return 0;
  return 1;
}

static int typed_named_kind_matches(DataBind *codec, const char *name, DataBindTypedKind kind,
                                    DataBindTypedKind wire_kind, const DataBindTypedType *object_type) {
  DataBindTypedKind schema_kind;
  DataBindSchemaType schema_type = DATA_BIND_SCHEMA_TYPE_INIT;
  if (kind == DATA_BIND_TYPED_OBJECT)
    return object_type != NULL && name != NULL && strcmp(object_type->name, name) == 0;
  if (kind != DATA_BIND_TYPED_ENUM)
...
static int typed_field_schema_matches(DataBind *codec, const DataBindTypedField *field,
                                      const DataBindSchemaField *schema) {
  int descriptor_optional = (field->flags & DATA_BIND_TYPED_FIELD_OPTIONAL) != 0;
  int descriptor_offset = (field->flags & DATA_BIND_TYPED_FIELD_WIRE_OFFSET) != 0;
  if (field->name == NULL || schema->name == NULL || strcmp(field->name, schema->name) != 0 ||
    ...
    return 0;
  if ((field->flags & DATA_BIND_TYPED_FIELD_GROUP) != 0)
    return schema->is_group && field->kind == DATA_BIND_TYPED_LIST && field->object_type != NULL &&
           schema->group_type != NULL && strcmp(field->object_type->name, schema->group_type) == 0;
  if (field->kind == DATA_BIND_TYPED_MAP)
  ...
  if (field->kind == DATA_BIND_TYPED_LIST || field->kind == DATA_BIND_TYPED_SET ||
      field->kind == DATA_BIND_TYPED_FIXED_ARRAY) {
    const char *expected_collection = field->kind == DATA_BIND_TYPED_FIXED_ARRAY
                                          ? "array"
                                          : (field->kind == DATA_BIND_TYPED_SET ? "set" : "list");
    return schema->is_collection && schema->inner_type != NULL &&
           ...
           (field->kind != DATA_BIND_TYPED_FIXED_ARRAY || schema->is_fixed_size) &&
...
static int typed_field_host_extent(const DataBindTypedField *field, size_t *extent) {
  if (field == NULL || extent == NULL) return 0;
  switch (field->kind) {
  case DATA_BIND_TYPED_STRING:
    *extent = sizeof(tstr);
    return 1;
  case DATA_BIND_TYPED_BYTES:
  case DATA_BIND_TYPED_LIST:
  case DATA_BIND_TYPED_SET:
  case DATA_BIND_TYPED_MAP:
    *extent = sizeof(vec_t);
    return 1;
  case DATA_BIND_TYPED_FIXED_BYTES:
    *extent = field->fixed_count;
    return 1;
  case DATA_BIND_TYPED_OBJECT:
    if (field->object_type == NULL) return 0;
    *extent = field->object_type->size;
    return 1;
  case DATA_BIND_TYPED_FIXED_ARRAY:
...
static int typed_value_host_extent(DataBindTypedKind kind, DataBindTypedKind wire_kind,
                                   const DataBindTypedType *object_type, size_t *extent) {
  if (extent == NULL) return 0;
  if (kind == DATA_BIND_TYPED_STRING) *extent = sizeof(tstr);
  else if (kind == DATA_BIND_TYPED_BYTES) *extent = sizeof(vec_t);
  else if (kind == DATA_BIND_TYPED_OBJECT && object_type != NULL) *extent = object_type->size;
  else if (kind == DATA_BIND_TYPED_ENUM) *extent = typed_kind_size(typed_enum_storage_kind(wire_kind));
  else *extent = typed_kind_size(kind);
  return *extent != 0;
}

static int typed_field_wire_extent(const DataBindTypedField *field, size_t *extent) {
  size_t element_wire_size;
  if (field == NULL || extent == NULL) return 0;
  if (field->kind == DATA_BIND_TYPED_OBJECT) {
    if (field->object_type == NULL) return 0;
  ...
  if (field->kind == DATA_BIND_TYPED_FIXED_ARRAY) {
    if (field->element_kind == DATA_BIND_TYPED_OBJECT) {
      if (field->object_type == NULL) return 0;
      element_wire_size = field->object_type->fixed_block_size;
    } else {
      element_wire_size = typed_kind_size(
          field->element_kind == DATA_BIND_TYPED_ENUM ? field->element_wire_kind : field->element_kind);
    }
    return element_wire_size != 0 &&
           typed_multiply_fits(field->fixed_count, element_wire_size, extent);
  }
  *extent = typed_kind_size(field->kind == DATA_BIND_TYPED_ENUM ? field->wire_kind : field->kind);
  return *extent != 0;
}

static int typed_kind_owns_storage(DataBindTypedKind kind) {
  return kind == DATA_BIND_TYPED_STRING || kind == DATA_BIND_TYPED_BYTES || kind == DATA_BIND_TYPED_OBJECT ||
         kind == DATA_BIND_TYPED_LIST || kind == DATA_BIND_TYPED_SET || kind == DATA_BIND_TYPED_MAP;
}

static int typed_field_owns_storage(const DataBindTypedField *field) {
  if (field == NULL) return 0;
  if (field->kind == DATA_BIND_TYPED_FIXED_ARRAY) return typed_kind_owns_storage(field->element_kind);
  return typed_kind_owns_storage(field->kind);
}

static int typed_type_has_tail(const DataBindTypedType *type) {
  size_t i;
  if (type == NULL) return 0;
  for (i = 0; i < type->field_count; ++i)
    if ((type->fields[i].flags & (DATA_BIND_TYPED_FIELD_GROUP | DATA_BIND_TYPED_FIELD_VAR_DATA)) != 0) return 1;
  return 0;
}
...
static DataBindStatus typed_validate_descriptor_at(const DataBindTypedType *type, unsigned depth,
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    size_t host_extent;
    size_t j;
    const DataBindTypedType *nested_type = NULL;
    ...
    if ((field->flags & DATA_BIND_TYPED_FIELD_OPTIONAL) != 0 &&
    ...
    for (j = 0; j < i; ++j) {
      const DataBindTypedField *previous = &type->fields[j];
    ...
    if (field->kind == DATA_BIND_TYPED_OBJECT) {
      ...
      if (field->element_kind == DATA_BIND_TYPED_OBJECT) nested_type = field->object_type;
    } else if (field->kind == DATA_BIND_TYPED_MAP) {
      size_t value_extent;
      ...
      if (field->map_value_kind == DATA_BIND_TYPED_OBJECT) nested_type = field->map_value_type;
    }
    if ((field->flags & DATA_BIND_TYPED_FIELD_GROUP) != 0 &&
        (field->kind != DATA_BIND_TYPED_LIST || field->element_kind != DATA_BIND_TYPED_OBJECT ||
         field->object_type == NULL || field->element_size != field->object_type->size))
      return typed_error(error, DATA_BIND_ERR_SCHEMA, field->name,
                         "Typed group descriptor is not an owning record vector");
    if ((field->flags & DATA_BIND_TYPED_FIELD_VAR_DATA) != 0 && field->kind != DATA_BIND_TYPED_STRING &&
        field->kind != DATA_BIND_TYPED_BYTES)
      return typed_error(error, DATA_BIND_ERR_SCHEMA, field->name,
...
DataBindStatus data_bind_typed_validate_descriptor(const DataBindTypedType *type, DataBindError *error) {
  return typed_validate_descriptor_at(type, 0u, error);
}

enum { DATA_BIND_TYPED_NATIVE_MAX_DEPTH = 32u };
...
static int typed_native_scalar_supported(const cmeta_data_desc *data) {
  const DataBindTypedCMetaKindMapping typed_cmeta_kind_mappings[] = {
      {&cmeta_data_bool, DATA_BIND_TYPED_BOOL},        {&cmeta_data_int, DATA_BIND_TYPED_I32},
      {&cmeta_data_long, DATA_BIND_TYPED_I64},         {&salts_int8_cmeta_data, DATA_BIND_TYPED_I8},
      {&salts_uint8_cmeta_data, DATA_BIND_TYPED_U8},   {&salts_int16_cmeta_data, DATA_BIND_TYPED_I16},
      {&salts_uint16_cmeta_data, DATA_BIND_TYPED_U16}, {&salts_int32_cmeta_data, DATA_BIND_TYPED_I32},
      {&salts_uint32_cmeta_data, DATA_BIND_TYPED_U32}, {&salts_int64_cmeta_data, DATA_BIND_TYPED_I64},
      {&salts_uint64_cmeta_data, DATA_BIND_TYPED_U64}, {&cmeta_data_float, DATA_BIND_TYPED_F32},
      {&cmeta_data_double, DATA_BIND_TYPED_F64},
  };
  size_t i;
...
static DataBindStatus typed_native_record_preflight(const cmeta_data_desc *data,
  ...
  if (depth > DATA_BIND_TYPED_NATIVE_MAX_DEPTH)
    return typed_error(error, DATA_BIND_ERR_SCHEMA, path,
  ...
  if (data->storage_type == NULL || !cmeta_type_desc_valid(data->storage_type) ||
      ...
      data->storage_type->align > DATA_BIND_TYPED_MAX_ALIGNOF)
    return typed_error(error, DATA_BIND_ERR_SCHEMA, path,
  ...
  for (i = 0u; i < shape->field_count; ++i) {
    ...
    const DataBindTypedField *wire_field = &overlay->fields[i];
    ...
    if (!typed_nonempty(native_field->stable_id) || !typed_nonempty(layout_field->name) ||
        ...
        (wire_field->flags & (DATA_BIND_TYPED_FIELD_OPTIONAL | DATA_BIND_TYPED_FIELD_GROUP)) != 0u)
...
static DataBindStatus typed_descriptor_native_record(const DataBindTypedDescriptor *descriptor,
                                                     TypedNativeRecord *out, DataBindError *error) {
  const size_t required_size =
      offsetof(DataBindTypedDescriptor, native_data) + sizeof(descriptor->native_data);
  const cmeta_data_desc *ancestors[DATA_BIND_TYPED_NATIVE_MAX_DEPTH + 1u];
  const char *path;
  if (descriptor == NULL || descriptor->struct_size < required_size ||
      descriptor->abi_version != DATA_BIND_TYPED_DESCRIPTOR_ABI_VERSION || descriptor->overlay == NULL ||
...
static int64_t typed_native_enum_signed_value(const cmeta_enum_domain *domain, uint64_t bits) {
  uint64_t mask = UINT64_MAX >> (64u - domain->bits);
  if ((bits & (UINT64_C(1) << (domain->bits - 1u))) != 0u) return -1 - (int64_t)(mask - bits);
  return (int64_t)bits;
}

static const char *typed_json_numeric_text(const json_value_t *value, size_t *out_length) {
...
static int typed_json_read_i64(const json_value_t *value, int64_t *out) {
  ...
  if (text == NULL || !data_bind_internal_parse_integer_magnitude(
...
static int typed_json_read_u64(const json_value_t *value, uint64_t *out) {
  ...
  text = typed_json_numeric_text(value, &length);
  if (text == NULL ||
      !data_bind_internal_parse_integer_magnitude(text, length, UINT64_MAX, 0, out, &negative))
    return 0;
  return !negative;
}
...
static int typed_json_read_f64(const json_value_t *value, double *out) {
  ...
  if (errno == ERANGE || end != text + length || !isfinite(parsed)) return 0;
  *out = parsed;
  return 1;
}

static int typed_enum_named_bits(const cmeta_enum_domain *domain, const char *text, size_t length,
                                 uint64_t *out) {
  size_t i;
  if (domain == NULL || text == NULL || out == NULL) return 0;
  for (i = 0u; i < domain->count; ++i) {
    const cmeta_enum_bits_item *item = &domain->items[i];
    if ((strlen(item->symbol) == length && memcmp(item->symbol, text, length) == 0) ||
        (strlen(item->text) == length && memcmp(item->text, text, length) == 0)) {
      *out = item->bits;
      return 1;
    }
  }
...
static int typed_enum_flags_text(const cmeta_enum_domain *domain, const char *text, size_t length,
  ...
  while (position < length) {
    ...
    while (position < length && text[position] != ' ' && text[position] != '\t' &&
           text[position] != '|' && text[position] != ',' && text[position] != '+')
      ++position;
    if (!typed_enum_named_bits(domain, text + start, position - start, &bits)) {
      uint64_t magnitude;
      uint64_t maximum = domain->signedness == CMETA_ENUM_SIGNED ? (mask >> 1u) + 1u : mask;
      int negative;
      if (!data_bind_internal_parse_integer_magnitude(text + start, position - start, maximum,
                                                      domain->signedness == CMETA_ENUM_SIGNED,
                                                      &magnitude, &negative))
        return 0;
      if (!negative && domain->signedness == CMETA_ENUM_SIGNED && magnitude > (mask >> 1u))
        return 0;
...
static int typed_json_enum_bits(const cmeta_enum_domain *domain, const json_value_t *value,
                                uint64_t *out) {
  uint64_t mask;
  size_t i;
  ...
  if (json_type(value) == JSON_STRING) {
    ...
    if (domain->kind == CMETA_ENUM_FLAGS && typed_enum_flags_text(domain, text, length, mask, out))
      return 1;
  }
  if (domain->kind == CMETA_ENUM_FLAGS && json_type(value) == JSON_ARRAY) {
    ...
    for (i = 0u; i < json_array_size(value); ++i) {
      uint64_t item;
      if (!typed_json_enum_bits(domain, json_array_get(value, i), &item)) return 0;
      bits |= item;
    }
    *out = bits;
    return 1;
  ...
  if (domain->signedness == CMETA_ENUM_SIGNED) {
    ...
    int64_t maximum = (int64_t)(mask >> 1u);
    if (!typed_json_read_i64(value, &signed_value) || signed_value < -maximum - 1 ||
...
static DataBindStatus typed_native_from_json_scalar(const cmeta_data_desc *data,
                                                    const json_value_t *value, void *storage,
                                                    const char *path, DataBindError *error) {
  int64_t signed_value;
  ...
  if (typed_cmeta_scalar_matches(data, &salts_bool8_cmeta_data)) {
    ...
    size_t text_length = json_type(value) == JSON_STRING ? json_string_len(value) : 0u;
    if (json_type(value) == JSON_BOOL) {
      candidate = json_bool(value) ? 1u : 0u;
    } else if (text != NULL && ((text_length == 4u && memcmp(text, "true", 4u) == 0) ||
                                (text_length == 3u && memcmp(text, "yes", 3u) == 0))) {
      candidate = 1u;
    } else if (text != NULL && ((text_length == 5u && memcmp(text, "false", 5u) == 0) ||
      ...
      candidate = 0u;
    } else if (json_type(value) == JSON_NUMBER && typed_json_read_f64(value, &floating_value)) {
  ...
  if (salts_uuid_cmeta_data_valid(data)) {
    ...
    if (json_type(value) != JSON_STRING || salts_uuid_parse(json_string(value), &candidate) != 0)
      return typed_error(error, DATA_BIND_ERR_TYPE_MISMATCH, path, "Expected UUID value");
    if (cmeta_data_fixed_copy(data, storage, &candidate, sizeof(candidate)) != CMETA_OK)
      return typed_error(error, DATA_BIND_ERR_SCHEMA, path,
  ...
  if (data->kind == CMETA_DATA_BYTES && cmeta_data_fixed_ops_of(data) != NULL) {
    ...
    if (cmeta_data_fixed_copy(data, storage, json_string(value), extent) != CMETA_OK)
      return typed_error(error, DATA_BIND_ERR_SCHEMA, path,
  ...
  if (typed_cmeta_scalar_matches(data, &salts_int8_cmeta_data)) {
    ...
    *(uint64_t *)storage = unsigned_value;
    ...
    if (!typed_json_read_f64(value, &floating_value) || floating_value < -(double)FLT_MAX ||
...
static json_value_t *typed_native_default_json(const cmeta_data_desc *data,
  ...
  if ((data->kind == CMETA_DATA_SINT || data->kind == CMETA_DATA_UINT) &&
      (strcmp(default_value, "true") == 0 || strcmp(default_value, "false") == 0))
...
static DataBindStatus typed_native_from_json(DataBind *codec, const cmeta_data_desc *data,
                                             const DataBindTypedType *overlay, const json_value_t *value,
                                             void *storage, const char *path,
                                             int invalid_scalar_uses_default,
                                             DataBindError *error) {
  const cmeta_data_struct_shape *shape;
  size_t i;
  ...
  if (overlay == NULL || json_type(value) != JSON_OBJECT)
    return typed_error(error, DATA_BIND_ERR_TYPE_MISMATCH, path, "Expected schema object");
  shape = (const cmeta_data_struct_shape *)data->shape;
  for (i = 0u; i < shape->field_count; ++i) {
    const cmeta_data_field_desc *native_field = &shape->fields[i];
    const DataBindTypedField *wire_field = &overlay->fields[i];
    ...
    if (status == DATA_BIND_ERR_TYPE_MISMATCH && child_from_input && invalid_scalar_uses_default &&
      ...
      status = typed_native_from_json_scalar(native_field->value, default_value,
...
static json_value_t *typed_native_to_json(DataBind *codec, const cmeta_data_desc *data,
                                          const DataBindTypedType *overlay, const void *storage,
                                          const char *path, DataBindError *error) {
  ...
  if (data->kind == CMETA_DATA_STRUCT) {
    ...
    size_t i;
    if (root == NULL) {
      typed_error(error, DATA_BIND_ERR_OOM, path, "Out of memory creating JSON object");
      return NULL;
    }
    for (i = 0u; i < shape->field_count; ++i) {
      const cmeta_data_field_desc *native_field = &shape->fields[i];
      ...
      if (typed_native_path(field_path, sizeof(field_path), path, native_field->name, error) !=
          DATA_BIND_OK) {
        json_free(root);
        return NULL;
      }
  ...
  if (data->kind == CMETA_DATA_ENUM) {
    ...
    return typed_json_created(domain->signedness == CMETA_ENUM_SIGNED
  ...
  if (data->kind == CMETA_DATA_BYTES && cmeta_data_fixed_ops_of(data) != NULL) {
    ...
    if (cmeta_data_fixed_extent(data, &extent) != CMETA_OK) {
      typed_error(error, DATA_BIND_ERR_SCHEMA, path, "Fixed bytes provider has no exact extent");
      return NULL;
    }
    candidate = calloc(1u, extent);
    if (candidate == NULL) {
      typed_error(error, DATA_BIND_ERR_OOM, path, "Out of memory validating fixed bytes storage");
      return NULL;
    }
  ...
  if (typed_cmeta_scalar_matches(data, &cmeta_data_float)) {
    ...
    if (isfinite(value)) return typed_json_created(json_create_number(value), path, error);
  } else if (typed_cmeta_scalar_matches(data, &cmeta_data_double)) {
...
enum { DATA_BIND_TYPED_TEXT_MAX_PATH = 255u };
...
static int typed_csv_append_field(tstr *out, const char *text, size_t length) {
  ...
  int quoted = 0;
  if (out == NULL || *out == NULL || (text == NULL && length != 0u)) return 0;
  if ((length != 0u && (text[0] == ' ' || text[0] == '\t' || text[length - 1u] == ' ' ||
                        text[length - 1u] == '\t')) ||
  ...
  for (index = 0u; index < length; ++index) {
    ...
    if (!typed_text_append(out, text + start, index - start) || !typed_text_append(out, "\"\"", 2u))
      return 0;
    start = index + 1u;
  }
  return typed_text_append(out, text + start, length - start) && typed_text_append(out, "\"", 1u);
}
...
static tstr typed_csv_child_path(const tstr prefix, const char *name, size_t name_length,
                                 DataBindStatus *status) {
  ...
  *status = DATA_BIND_ERR_TYPE_MISMATCH;
  if (!typed_csv_path_component_valid(name, name_length) ||
      prefix_length > DATA_BIND_TYPED_TEXT_MAX_PATH - separator_length ||
      name_length > DATA_BIND_TYPED_TEXT_MAX_PATH - prefix_length - separator_length)
    return NULL;
...
static DataBindStatus typed_csv_scalar_text(const json_value_t *value, const char **text,
                                            size_t *length) {
  if (value == NULL || text == NULL || length == NULL) return DATA_BIND_ERR_INVALID_ARG;
  if (json_type(value) == JSON_STRING) {
    *text = json_string(value);
    *length = json_string_len(value);
...
static DataBindStatus typed_csv_flatten_json(const json_value_t *value, const tstr path,
                                             tstr *headers, tstr *row, size_t *cell_count,
                                             unsigned depth) {
  size_t index;
  if (value == NULL || headers == NULL || row == NULL || cell_count == NULL)
    return DATA_BIND_ERR_INVALID_ARG;
  if (json_type(value) == JSON_OBJECT) {
    if (depth > DATA_BIND_TYPED_NATIVE_MAX_DEPTH) return DATA_BIND_ERR_RUNTIME;
    ...
    for (index = 0u; index < json_object_size(value); ++index) {
      ...
      status = typed_csv_flatten_json(json_object_value(value, index), child_path, headers, row,
                                      cell_count, depth + 1u);
  ...
  }
  {
    ...
    if ((*cell_count != 0u &&
         (!typed_text_append(headers, ",", 1u) || !typed_text_append(row, ",", 1u))) ||
...
static DataBindStatus typed_json_serialize_csv(const json_value_t *json, char **out,
                                               size_t *out_length, const char *path,
                                               DataBindError *error) {
  ...
  if (headers == NULL || row == NULL) {
    tstr_free(headers);
    tstr_free(row);
    return typed_error(error, DATA_BIND_ERR_OOM, path, "Out of memory creating native CSV rows");
  }
  status = typed_csv_flatten_json(json, NULL, &headers, &row, &cell_count, 0u);
  if (status == DATA_BIND_OK && cell_count == 0u) status = DATA_BIND_ERR_TYPE_MISMATCH;
  if (status == DATA_BIND_OK) {
    csv = headers;
    headers = NULL;
    if (!typed_text_append(&csv, "\r\n", 2u) || !typed_text_append(&csv, row, tstr_len(row)) ||
  ...
  if (status == DATA_BIND_OK) {
    *out = tstr_to_cstr(csv);
    ...
    else if (out_length != NULL) *out_length = tstr_len(csv);
  }
  tstr_free(headers);
  tstr_free(row);
  tstr_free(csv);
...
static int typed_csv_join_path(char *out, size_t capacity, const char *prefix, const char *name) {
  int written;
  if (out == NULL || capacity == 0u || name == NULL) return 0;
  written = prefix != NULL && prefix[0] != '\0' ? snprintf(out, capacity, "%s.%s", prefix, name)
...
static int typed_csv_has_path(const csv_doc_t *document, const char *path) {
  ...
  for (column = 0u; column < csv_column_count(document); ++column) {
    ...
    if (header != NULL && data_bind_internal_csv_header_matches_path(header, path)) return 1;
  }
  return 0;
}

static DataBindStatus typed_csv_native_json(DataBind *codec, const cmeta_data_desc *data,
                                            const DataBindTypedType *overlay, const csv_doc_t *document,
                                            size_t row, const char *prefix, json_value_t **out,
                                            const char *type_name, DataBindError *error) {
  const cmeta_data_struct_shape *shape;
  json_value_t *object;
  size_t field_index;
  if (codec == NULL || data == NULL || overlay == NULL || document == NULL || out == NULL ||
  ...
  for (field_index = 0u; field_index < shape->field_count; ++field_index) {
    ...
    const DataBindTypedField *wire_field = &overlay->fields[field_index];
    const char *output_name =
        data_bind_internal_json_field_output_name(codec, overlay->name, field_index);
    size_t input_count =
        data_bind_internal_field_input_name_count(codec, overlay->name, field_index);
    json_value_t *child = NULL;
    size_t input_index;
    ...
    for (input_index = 0u; input_index < input_count; ++input_index) {
      const char *input_name =
          data_bind_internal_field_input_name_at(codec, overlay->name, field_index, input_index);
      char field_path[DATA_BIND_TYPED_TEXT_MAX_PATH + 1u];
      DataBindStatus status;
      if (!typed_csv_join_path(field_path, sizeof(field_path), prefix, input_name)) {
        json_free(object);
        return typed_error(error, DATA_BIND_ERR_SCHEMA, type_name,
      ...
      if (native_field->value->kind == CMETA_DATA_STRUCT) {
        ...
        status = typed_csv_native_json(codec, native_field->value, wire_field->nested_overlay,
                                       document, row, field_path, &child, type_name, error);
        if (status != DATA_BIND_OK) {
          json_free(object);
          return status;
        }
        ...
        const char *cell;
        size_t cell_length;
        if (!data_bind_internal_csv_find_path_column(document, field_path, &column)) continue;
...
static DataBindStatus typed_csv_parse_json(DataBind *codec, const cmeta_data_desc *data,
                                           const DataBindTypedType *overlay, const char *text,
                                           size_t length, size_t row, json_value_t **out,
                                           const char *type_name, DataBindError *error) {
  ...
  if (document == NULL)
    return typed_error(error, DATA_BIND_ERR_PARSE, type_name, "CSV parse failed");
  if (row >= csv_row_count(document)) {
    csv_free(document);
    return typed_error(error, DATA_BIND_ERR_TYPE_MISMATCH, type_name, "CSV row is out of range");
  }
  status = typed_csv_native_json(codec, data, overlay, document, row, NULL, out, type_name, error);
  csv_free(document);
  return status == DATA_BIND_OK ? typed_error(error, DATA_BIND_OK, NULL, NULL) : status;
}

static salts_xml_node typed_xml_child_named(salts_xml_node parent, const char *name) {
  size_t index;
...
static int typed_xml_attribute_named(salts_xml_node parent, const char *name,
  ...
  for (index = 0u; index < salts_xml_node_attribute_count(parent); ++index) {
    ...
    salts_xml_string_view qualified = salts_xml_attribute_qualified_name(attribute);
...
static DataBindStatus typed_xml_native_json(DataBind *codec, const cmeta_data_desc *data,
                                            const DataBindTypedType *overlay, salts_xml_node node,
                                            json_value_t **out, unsigned depth,
                                            const char *type_name, DataBindError *error) {
  const cmeta_data_struct_shape *shape;
  json_value_t *object;
  size_t field_index;
  ...
  for (field_index = 0u; field_index < shape->field_count; ++field_index) {
    ...
    else if (attribute_text.size == 0u && native_field->value->kind != CMETA_DATA_BYTES)
      status = DATA_BIND_OK;
    else {
      child = json_create_string_n(attribute_text.data != NULL ? attribute_text.data : "",
                                   attribute_text.size);
      status = child != NULL ? DATA_BIND_OK
                             : typed_error(error, DATA_BIND_ERR_OOM, type_name,
...
static DataBindStatus typed_xml_parse_json(DataBind *codec, const cmeta_data_desc *data,
                                           const DataBindTypedType *overlay, const char *text,
                                           size_t length, json_value_t **out, const char *type_name,
                                           DataBindError *error) {
  salts_xml_document document = {0};
  salts_xml_node root;
  DataBindStatus status;
  if (salts_xml_parse(&document, text, length, NULL, NULL) != SALTS_XML_OK)
    return typed_error(error, DATA_BIND_ERR_PARSE, type_name, "XML parse failed");
  root = salts_xml_document_root(&document);
  status = typed_xml_native_json(codec, data, overlay, root, out, 0u, type_name, error);
...
static DataBindStatus typed_json_to_xml(const json_value_t *value, salts_xml_node node,
                                        unsigned depth) {
  size_t index;
  ...
  if (json_type(value) == JSON_OBJECT) {
    ...
    for (index = 0u; index < json_object_size(value); ++index) {
      ...
      DataBindStatus status;
      if (!typed_xml_name_valid(name, name_length)) return DATA_BIND_ERR_TYPE_MISMATCH;
      xml_status = salts_xml_node_add_element(node, name, &child);
      if (xml_status != SALTS_XML_OK)
        return xml_status == SALTS_XML_ALLOCATION_FAILED ? DATA_BIND_ERR_OOM
                                                         : DATA_BIND_ERR_TYPE_MISMATCH;
      status = typed_json_to_xml(json_object_value(value, index), child, depth + 1u);
      if (status != DATA_BIND_OK) return status;
    }
    return DATA_BIND_OK;
  }
  {
    ...
    return xml_status == SALTS_XML_OK
               ? DATA_BIND_OK
               : (xml_status == SALTS_XML_ALLOCATION_FAILED ? DATA_BIND_ERR_OOM
                                                            : DATA_BIND_ERR_TYPE_MISMATCH);
  }
}

static DataBindStatus typed_json_serialize_xml(const char *type_name, const json_value_t *json,
...
static DataBindStatus typed_text_parse_json(DataBind *codec, const cmeta_data_desc *data,
                                            const DataBindTypedType *overlay, DataBindFormat format,
                                            const char *text, size_t length, size_t row,
                                            json_value_t **out, const char *type_name,
                                            DataBindError *error) {
  if (format == DATA_BIND_FORMAT_JSON) {
    *out = json_parse(text, length);
    return *out != NULL ? typed_error(error, DATA_BIND_OK, NULL, NULL)
                        : typed_error(error, DATA_BIND_ERR_PARSE, type_name, "JSON parse failed");
  }
  if (format == DATA_BIND_FORMAT_YAML) {
    cyaml_doc_t *document = cyaml_parse(text, length, NULL, NULL);
    if (document == NULL)
      return typed_error(error, DATA_BIND_ERR_PARSE, type_name, "YAML parse failed");
    *out = json_value_from_cyaml(document);
    cyaml_free(document);
    return *out != NULL ? typed_error(error, DATA_BIND_OK, NULL, NULL)
                        : typed_error(error, DATA_BIND_ERR_TYPE_MISMATCH, type_name,
                                      "YAML value cannot be represented as JSON-compatible data");
  }
  if (format == DATA_BIND_FORMAT_CSV)
    return typed_csv_parse_json(codec, data, overlay, text, length, row, out, type_name, error);
  if (format == DATA_BIND_FORMAT_XML)
    return typed_xml_parse_json(codec, data, overlay, text, length, out, type_name, error);
  return typed_error(error, DATA_BIND_ERR_INVALID_ARG, type_name,
                     "Unknown canonical descriptor input format");
}

static DataBindStatus typed_json_serialize_text(DataBindFormat format, const char *type_name,
                                                const json_value_t *json, char **out,
...
DataBindStatus data_bind_typed_descriptor_validate(const DataBindTypedDescriptor *descriptor,
                                             DataBindError *error) {
...
DataBindStatus data_bind_typed_descriptor_init(const DataBindTypedDescriptor *descriptor, void *object,
                                         DataBindError *error) {
...
DataBindStatus data_bind_typed_descriptor_clear(const DataBindTypedDescriptor *descriptor, void *object,
                                          DataBindError *error) {
  TypedNativeRecord native;
  DataBindStatus status = typed_descriptor_native_record(descriptor, &native, error);
  if (status != DATA_BIND_OK) return status;
...
static DataBindStatus typed_validate_layout_at(const DataBindTypedType *type, unsigned depth,
                                               DataBindError *error) {
  size_t i;
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    size_t wire_extent;
    size_t j;
    if ((field->flags & DATA_BIND_TYPED_FIELD_WIRE_OFFSET) != 0) {
      ...
      for (j = 0; j < i; ++j) {
        const DataBindTypedField *previous = &type->fields[j];
        size_t previous_extent;
        if ((previous->flags & DATA_BIND_TYPED_FIELD_WIRE_OFFSET) == 0) continue;
    ...
    if (field->kind == DATA_BIND_TYPED_OBJECT) {
      ...
      if (status != DATA_BIND_OK) return status;
    } else if (field->kind == DATA_BIND_TYPED_FIXED_ARRAY && field->element_kind == DATA_BIND_TYPED_OBJECT) {
      if (typed_type_has_tail(field->object_type))
        return typed_error(error, DATA_BIND_ERR_SCHEMA, field->name,
                           "Nested binary arrays must have a fixed wire layout");
      status = typed_validate_layout_at(field->object_type, depth + 1u, error);
      if (status != DATA_BIND_OK) return status;
    } else if ((field->flags & DATA_BIND_TYPED_FIELD_GROUP) != 0) {
      if (typed_type_has_tail(field->object_type))
        return typed_error(error, DATA_BIND_ERR_SCHEMA, field->name,
                           "Typed group entries must have a fixed wire layout");
      status = typed_validate_layout_at(field->object_type, depth + 1u, error);
      if (status != DATA_BIND_OK) return status;
    } else if ((field->flags & DATA_BIND_TYPED_FIELD_WIRE_OFFSET) == 0 &&
               (field->flags & (DATA_BIND_TYPED_FIELD_GROUP | DATA_BIND_TYPED_FIELD_VAR_DATA)) == 0) {
      return typed_error(error, DATA_BIND_ERR_SCHEMA, field->name,
...
static int typed_supports_direct_binary(const DataBindTypedType *type) {
  size_t i;
  if (type == NULL) return 0;
  for (i = 0; i < type->field_count; ++i) {
    const DataBindTypedField *field = &type->fields[i];
    if (field->kind == DATA_BIND_TYPED_OBJECT ||
        (field->kind == DATA_BIND_TYPED_FIXED_ARRAY && field->element_kind == DATA_BIND_TYPED_OBJECT) ||
        (field->flags & DATA_BIND_TYPED_FIELD_GROUP) != 0) {
...
static DataBindStatus typed_validate_schema_at(DataBind *codec, const char *type_name,
                                               const DataBindTypedType *type, unsigned depth,
                                               DataBindError *error) {
  DataBindSchemaType schema_type = DATA_BIND_SCHEMA_TYPE_INIT;
  DataBindStatus status;
  size_t i;
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    } else if (field->kind == DATA_BIND_TYPED_OBJECT) {
      nested_type = field->object_type;
      nested_name = schema_field.type;
    } else if ((field->kind == DATA_BIND_TYPED_FIXED_ARRAY || field->kind == DATA_BIND_TYPED_LIST ||
                field->kind == DATA_BIND_TYPED_SET) &&
               field->element_kind == DATA_BIND_TYPED_OBJECT) {
      nested_type = field->object_type;
      nested_name = schema_field.inner_type;
    } else if (field->kind == DATA_BIND_TYPED_MAP && field->map_value_kind == DATA_BIND_TYPED_OBJECT) {
...
DataBindStatus data_bind_typed_validate_schema(DataBind *codec, const char *type_name,
                                         const DataBindTypedType *type, DataBindError *error) {
...
static int typed_native_schema_field_matches(DataBind *codec, const cmeta_data_desc *native,
                                             const DataBindTypedField *wire,
                                             const DataBindSchemaField *schema) {
  int has_wire_offset;
  if (native == NULL || wire == NULL || schema == NULL || !typed_nonempty(wire->name) ||
      !typed_nonempty(schema->name) || strcmp(wire->name, schema->name) != 0 ||
      schema->is_optional ||
      (wire->flags & (DATA_BIND_TYPED_FIELD_OPTIONAL | DATA_BIND_TYPED_FIELD_GROUP)) != 0u)
    return 0;
  has_wire_offset = (wire->flags & DATA_BIND_TYPED_FIELD_WIRE_OFFSET) != 0;
  ...
  if (native->kind == CMETA_DATA_BYTES && cmeta_data_fixed_ops_of(native) != NULL) {
    ...
    return typed_nonempty(schema->type) && strcmp(schema->type, "bytes") == 0 &&
           ...
           wire->wire_kind == DATA_BIND_TYPED_FIXED_BYTES;
  }
  {
    DataBindTypedKind schema_kind;
...
static DataBindStatus typed_native_validate_schema_at(DataBind *codec, const char *type_name,
                                                      const cmeta_data_desc *data,
                                                      const DataBindTypedType *overlay, unsigned depth,
                                                      DataBindError *error) {
  const cmeta_data_struct_shape *shape = (const cmeta_data_struct_shape *)data->shape;
  DataBindSchemaType schema_type = DATA_BIND_SCHEMA_TYPE_INIT;
  DataBindStatus status;
...
static void typed_read_wire_scalar(DataBindTypedKind kind, const uint8_t *source, int big_endian,
                                   void *output) {
  switch (kind) {
  case DATA_BIND_TYPED_BOOL:
...
static void typed_read_enum(DataBindTypedKind wire_kind, const uint8_t *source, int big_endian,
                            void *output) {
  typed_read_wire_scalar(typed_enum_storage_kind(wire_kind), source, big_endian, output);
}

static DataBindStatus typed_read_fixed(const DataBindTypedType *type, const uint8_t *data, size_t len,
                                       void *object, DataBindError *error) {
  size_t i;
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    size_t j;
    size_t element_wire_size;
    if ((field->flags & DATA_BIND_TYPED_FIELD_WIRE_OFFSET) == 0) continue;
    ...
    if (field->kind == DATA_BIND_TYPED_OBJECT) {
      ...
      if (status != DATA_BIND_OK) return status;
    } else if (field->kind == DATA_BIND_TYPED_FIXED_BYTES) {
      memcpy(output, source, field->fixed_count);
    } else if (field->kind == DATA_BIND_TYPED_FIXED_ARRAY) {
      element_wire_size =
          field->element_kind == DATA_BIND_TYPED_OBJECT
              ? field->object_type->fixed_block_size
              : typed_kind_size(field->element_kind == DATA_BIND_TYPED_ENUM ? field->element_wire_kind
                                                                      : field->element_kind);
      for (j = 0; j < field->fixed_count; ++j) {
        ...
        if (field->element_kind == DATA_BIND_TYPED_OBJECT) {
          ...
          if (status != DATA_BIND_OK) return status;
        } else if (field->element_kind == DATA_BIND_TYPED_ENUM) {
    ...
    } else if (field->kind == DATA_BIND_TYPED_ENUM) {
...
static DataBindStatus typed_read_tail(const DataBindTypedType *type, const uint8_t *data, size_t len,
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    if ((field->flags & DATA_BIND_TYPED_FIELD_GROUP) != 0) {
      ...
      if (present) {
        ...
        for (j = 0; j < count; ++j) {
          void *element = (uint8_t *)vec->data + j * field->element_size;
          DataBindStatus status = data_bind_typed_init(field->object_type, element, error);
          if (status == DATA_BIND_OK)
      ...
      cursor += payload_size;
    } else if ((field->flags & DATA_BIND_TYPED_FIELD_VAR_DATA) != 0) {
      uint32_t value_size;
      if (!typed_size_fits(cursor, 4u, len))
        return typed_error(error, DATA_BIND_ERR_PARSE, field->name,
      ...
      if (present && field->kind == DATA_BIND_TYPED_STRING) {
...
DataBindStatus data_bind_typed_parse_binary(const DataBindTypedType *type, const void *data, size_t len,
                                      void *object, DataBindError *error) {
  void *temporary;
  size_t consumed = 0;
  DataBindStatus status;
...
DataBindStatus data_bind_typed_parse_ex(DataBind *codec, const char *type_name, const DataBindTypedType *type,
  ...
  if (format == DATA_BIND_FORMAT_BINARY) {
    status = data_bind_typed_validate_schema(codec, type_name, type, error);
    if (status != DATA_BIND_OK) return status;
    if (typed_supports_direct_binary(type))
      return data_bind_typed_parse_binary(type, data, len, object, error);
  ...
  else return typed_error(error, DATA_BIND_ERR_INVALID_ARG, NULL, "Unknown typed input format");
  if (status != DATA_BIND_OK) return status;
  temporary = calloc(1, type->size);
  if (temporary == NULL) {
...
DataBindStatus data_bind_typed_parse(DataBind *codec, const char *type_name, const DataBindTypedType *type,
  ...
  return data_bind_typed_parse_ex(codec, type_name, type, parsed_format, data, len, row, object, error);
}

static int typed_native_wire_extent(const cmeta_data_desc *data, const DataBindTypedField *wire,
                                    size_t *extent) {
...
static DataBindStatus typed_native_validate_wire(const cmeta_data_desc *data,
                                                 const DataBindTypedType *overlay, const char *path,
                                                 DataBindError *error) {
  const cmeta_data_struct_shape *shape = (const cmeta_data_struct_shape *)data->shape;
  size_t i;
  ...
  for (i = 0u; i < shape->field_count; ++i) {
    ...
    DataBindStatus status =
        typed_native_path(field_path, sizeof(field_path), path, native_field->name, error);
    if (status != DATA_BIND_OK) return status;
    if ((wire_field->flags & DATA_BIND_TYPED_FIELD_WIRE_OFFSET) == 0u ||
    ...
    for (j = 0u; j < i; ++j) {
      const DataBindTypedField *previous = &overlay->fields[j];
      size_t previous_extent;
...
static DataBindStatus typed_native_read_wire_scalar(const cmeta_data_desc *data,
                                                    DataBindTypedKind wire_kind, const uint8_t *source,
                                                    int big_endian, void *storage, const char *path,
                                                    DataBindError *error) {
  if (data->kind == CMETA_DATA_BYTES && cmeta_data_fixed_ops_of(data) != NULL) {
    size_t extent;
    if (wire_kind != DATA_BIND_TYPED_FIXED_BYTES || cmeta_data_fixed_extent(data, &extent) != CMETA_OK)
      return typed_error(error, DATA_BIND_ERR_SCHEMA, path,
  ...
  if (typed_cmeta_scalar_matches(data, &salts_bool8_cmeta_data) && wire_kind == DATA_BIND_TYPED_BOOL) {
  ...
  if (salts_uuid_cmeta_data_valid(data) && wire_kind == DATA_BIND_TYPED_UUID) {
    salts_uuid_t candidate = {{0}};
  ...
  if (typed_cmeta_scalar_matches(data, &salts_int8_cmeta_data) && wire_kind == DATA_BIND_TYPED_I8)
    *(int8_t *)storage = tbe_wire_read_i8(source, big_endian);
  else if (typed_cmeta_scalar_matches(data, &salts_uint8_cmeta_data) && wire_kind == DATA_BIND_TYPED_U8)
    *(uint8_t *)storage = tbe_wire_read_u8(source, big_endian);
  else if (typed_cmeta_scalar_matches(data, &salts_int16_cmeta_data) && wire_kind == DATA_BIND_TYPED_I16)
    *(int16_t *)storage = tbe_wire_read_i16(source, big_endian);
  else if (typed_cmeta_scalar_matches(data, &salts_uint16_cmeta_data) && wire_kind == DATA_BIND_TYPED_U16)
    *(uint16_t *)storage = tbe_wire_read_u16(source, big_endian);
  else if (typed_cmeta_scalar_matches(data, &salts_int32_cmeta_data) && wire_kind == DATA_BIND_TYPED_I32)
    *(int32_t *)storage = tbe_wire_read_i32(source, big_endian);
  else if (typed_cmeta_scalar_matches(data, &salts_uint32_cmeta_data) && wire_kind == DATA_BIND_TYPED_U32)
    *(uint32_t *)storage = tbe_wire_read_u32(source, big_endian);
  else if (typed_cmeta_scalar_matches(data, &salts_int64_cmeta_data) && wire_kind == DATA_BIND_TYPED_I64)
    *(int64_t *)storage = tbe_wire_read_i64(source, big_endian);
  else if (typed_cmeta_scalar_matches(data, &salts_uint64_cmeta_data) && wire_kind == DATA_BIND_TYPED_U64)
    *(uint64_t *)storage = tbe_wire_read_u64(source, big_endian);
  else if (typed_cmeta_scalar_matches(data, &cmeta_data_float) && wire_kind == DATA_BIND_TYPED_F32)
    *(float *)storage = tbe_wire_read_f32(source, big_endian);
  else if (typed_cmeta_scalar_matches(data, &cmeta_data_double) && wire_kind == DATA_BIND_TYPED_F64)
...
static DataBindStatus typed_native_read_fixed(const cmeta_data_desc *data,
                                              const DataBindTypedType *overlay, const uint8_t *source,
                                              void *storage, const char *path,
                                              DataBindError *error) {
  const cmeta_data_struct_shape *shape = (const cmeta_data_struct_shape *)data->shape;
  size_t i;
...
static DataBindStatus typed_native_write_wire_scalar(const cmeta_data_desc *data,
                                                     DataBindTypedKind wire_kind, uint8_t *destination,
...
DataBindStatus data_bind_typed_descriptor_parse(DataBind *codec, const char *type_name,
...
DataBindStatus data_bind_typed_serialize_ex(DataBind *codec, const char *type_name,
  ...
  json = data_bind_typed_to_json(type, object, error);
  if (json == NULL) {
    if (error != NULL && error->size >= offsetof(DataBindError, code) + sizeof(error->code) &&
        error->code != DATA_BIND_OK)
      return error->code;
...
DataBindStatus data_bind_typed_serialize(DataBind *codec, const char *type_name, const DataBindTypedType *type,
                                   const void *object, const char *format, char **out,
                                   size_t *out_len, DataBindError *error) {
  DataBindFormat parsed_format;
  if (!typed_format_from_string(format, &parsed_format) || parsed_format == DATA_BIND_FORMAT_BINARY)
    return typed_error(error, DATA_BIND_ERR_INVALID_ARG, format, "Unknown typed output format");
  return data_bind_typed_serialize_ex(codec, type_name, type, object, parsed_format, out, out_len, error);
}

DataBindStatus data_bind_typed_descriptor_serialize(DataBind *codec, const char *type_name,
                                              const DataBindTypedDescriptor *descriptor,
  ...
  status =
      typed_native_validate_schema_at(codec, type_name, native.data, native.overlay, 0u, error);
  if (status != DATA_BIND_OK) return status;
  json = typed_native_to_json(codec, native.data, native.overlay, object, type_name, error);
  if (json == NULL) {
    if (error != NULL && error->size >= offsetof(DataBindError, code) + sizeof(error->code) &&
        error->code != DATA_BIND_OK)
      return error->code;
  ...
  status = typed_json_serialize_text(format, type_name, json, out, out_len, error);
  json_free(json);
  return status;
}

DataBindStatus data_bind_typed_descriptor_serialize_binary(const DataBindTypedDescriptor *descriptor,
  ...
  status = data_bind_typed_descriptor_serialize_binary_into(
      descriptor, object, data, native.overlay->fixed_block_size, out_len, error);
  if (status != DATA_BIND_OK) {
    free(data);
    return status;
  }
...
DataBindStatus data_bind_typed_descriptor_serialize_binary_into(const DataBindTypedDescriptor *descriptor,
...
static size_t typed_binary_size(const DataBindTypedType *type, const void *object, int *supported) {
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    if ((field->flags & DATA_BIND_TYPED_FIELD_GROUP) != 0) {
      ...
      if (typed_optional_present(type, object, field)) {
        len = field->kind == DATA_BIND_TYPED_STRING
                  ? (*(const tstr *)ptr ? tstr_len(*(const tstr *)ptr) : 0)
                  : ((const vec_t *)ptr)->size;
      }
      if ((field->kind == DATA_BIND_TYPED_BYTES && len != 0 && ((const vec_t *)ptr)->data == NULL) ||
        ...
        *supported = 0;
        return 0;
               ...
               field->kind != DATA_BIND_TYPED_OBJECT) {
      *supported = 0;
      return 0;
    }
  }
...
static void typed_write_scalar(DataBindTypedKind kind, uint8_t *dst, int big_endian, const void *src) {
  switch (kind) {
  case DATA_BIND_TYPED_BOOL:
  case DATA_BIND_TYPED_U8:
...
static void typed_write_enum(DataBindTypedKind wire_kind, uint8_t *dst, int big_endian,
                             const void *src) {
  typed_write_scalar(typed_enum_storage_kind(wire_kind), dst, big_endian, src);
}

static int typed_write_fixed(const DataBindTypedType *type, const void *object, uint8_t *dst,
                             size_t size) {
  size_t i;
    ...
    if (field->kind == DATA_BIND_TYPED_OBJECT) {
      ...
      for (j = 0; j < field->fixed_count; ++j) {
        if (field->element_kind == DATA_BIND_TYPED_OBJECT) {
          ...
          DataBindTypedKind wire_kind = field->element_kind == DATA_BIND_TYPED_ENUM ? field->element_wire_kind
                                                                         : field->element_kind;
          uint8_t *element_dst = dst + field->wire_offset + j * typed_kind_size(wire_kind);
          const void *element_src = src + j * field->element_size;
          if (field->element_kind == DATA_BIND_TYPED_ENUM)
      ...
      if (field->kind == DATA_BIND_TYPED_ENUM)
...
DataBindStatus data_bind_typed_serialize_binary_into(const DataBindTypedType *type, const void *object,
  ...
  for (i = 0; i < type->field_count; ++i) {
    ...
    if ((field->flags & DATA_BIND_TYPED_FIELD_GROUP) != 0) {
      ...
      if (!typed_optional_present(type, object, field)) {
        bytes = NULL;
        len = 0u;
      } else if (field->kind == DATA_BIND_TYPED_STRING) {
        tstr text = *(const tstr *)ptr;
        bytes = text ? text : "";
...
DataBindStatus data_bind_typed_serialize_binary(const DataBindTypedType *type, const void *object,
  ...
  status = data_bind_typed_serialize_binary_into(type, object, data, total, out_len, error);
  if (status != DATA_BIND_OK) {
    free(data);
    return status;
  }
...
void data_bind_typed_serialized_free(void *data) { free(data); }
