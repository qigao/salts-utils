...
DataBindStatus data_bind_http_method_plan_compile_service(
  ...
  if (data_bind_binding_plan_execution(plan->binding) == NULL) {
    status = method_plan_fail(
        diagnostic, DATA_BIND_ERR_SCHEMA,
        "HTTP MethodPlan requires an exact Service execution adapter");
    goto fail;
  }
...
const DataBindServiceExecutionAdapter *data_bind_http_method_plan_execution(
    const DataBindHttpMethodPlan *plan) {
  return plan != NULL
             ? data_bind_binding_plan_execution(plan->binding)
             : NULL;
}
...
DataBindStatus data_bind_rpc_method_plan_compile_service(
  ...
  if (data_bind_binding_plan_execution(plan->binding) == NULL) {
    status = method_plan_fail(
        diagnostic, DATA_BIND_ERR_SCHEMA,
        "RPC MethodPlan requires an exact Service execution adapter");
    goto fail;
  }
...
const DataBindServiceExecutionAdapter *data_bind_rpc_method_plan_execution(
    const DataBindRpcMethodPlan *plan) {
  return plan != NULL
             ? data_bind_binding_plan_execution(plan->binding)
             : NULL;
