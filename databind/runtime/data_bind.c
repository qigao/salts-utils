...
struct data_bind_stream_t {
  ...
  size_t result_count;
  data_bind_stream_provider_lease provider;
  char *buffer;
  size_t size;
  size_t capacity;
...
static int data_bind_stream_provider_ops_valid(
    const data_bind_stream_provider_ops *ops) {
  return ops != NULL &&
         ops->size >= offsetof(data_bind_stream_provider_ops, destroy) +
                         sizeof(ops->destroy) &&
         ops->abi_version == DATA_BIND_STREAM_PROVIDER_OPS_ABI_VERSION &&
         ops->feed != NULL && ops->finish != NULL && ops->bind != NULL &&
         ops->cancel != NULL && ops->destroy != NULL;
}

static DataBindStatus data_bind_stream_provider_feed(
    data_bind_stream_t *stream, const char *data, size_t len,
    DataBindError *error) {
  data_bind_stream_format_state *state =
      data_bind_stream_provider_state(stream);
  if (state == NULL || state->feed_impl == NULL)
    return DATA_BIND_ERR_INVALID_ARG;
  return state->feed_impl(stream, data, len, error);
}

static DataBindStatus data_bind_stream_provider_finish(
    data_bind_stream_t *stream, DataBindValue **out_value,
    DataBindError *error) {
  data_bind_stream_format_state *state =
      data_bind_stream_provider_state(stream);
  if (state == NULL || state->finish_impl == NULL)
    return DATA_BIND_ERR_INVALID_ARG;
  return state->finish_impl(stream, out_value, error);
}

static DataBindStatus data_bind_stream_provider_bind(
    data_bind_stream_t *stream, const char *text, size_t len,
    DataBindValue **out_value, DataBindError *error) {
  data_bind_stream_format_state *state =
      data_bind_stream_provider_state(stream);
  if (stream == NULL || state == NULL || state->bind_impl == NULL)
    return DATA_BIND_ERR_INVALID_ARG;
  return state->bind_impl(
      stream->codec, stream->type_name, text, len, stream->path_or_expr,
      stream->query_limits_configured ? &stream->query_limits : NULL,
      &stream->query_diagnostic, out_value, error);
}

static DataBindStatus data_bind_stream_provider_cancel(
    data_bind_stream_t *stream, DataBindError *error) {
  (void)error;
  return stream != NULL ? DATA_BIND_OK : DATA_BIND_ERR_INVALID_ARG;
}

static void data_bind_stream_provider_destroy(data_bind_stream_t *stream) {
  data_bind_stream_format_state *state =
      data_bind_stream_provider_state(stream);
  data_bind_stream_format_state_cleanup(state);
  if (stream != NULL) stream->provider.state = NULL;
}

static const data_bind_stream_provider_ops DATA_BIND_STREAM_PROVIDER_OPS = {
    sizeof(data_bind_stream_provider_ops),
    DATA_BIND_STREAM_PROVIDER_OPS_ABI_VERSION,
    data_bind_stream_provider_feed,
    data_bind_stream_provider_finish,
    data_bind_stream_provider_bind,
    data_bind_stream_provider_cancel,
    data_bind_stream_provider_destroy};
...
static DataBindValue *bind_json_object(Node *schema_root, Node *record, json_value_t *object) {
  ...
  for (i = 0; i < fields->data.list.count; i++) {
    ...
    if (json_type(value) == JSON_NULL) {
      if (!field_flag(field, "is_nullable")) {
        data_bind_value_free(result);
        return NULL;
      }
      bound = dbv_new(DATA_BIND_VALUE_NULL);
      if (bound == NULL || dbv_object_set(result, name, bound) != DATA_BIND_OK) {
        data_bind_value_free(bound);
        data_bind_value_free(result);
        return NULL;
      }
...
static void data_bind_stream_error_msg(data_bind_stream_t *parser, const char *message) {
  ...
  snprintf(data_bind_stream_provider_state(parser)->stream_error, sizeof(data_bind_stream_provider_state(parser)->stream_error), "%s", message);
}
...
static int data_bind_stream_json_bind_value(data_bind_stream_t *parser, json_value_t *value) {
  ...
  if (data_bind_stream_provider_state(parser)->json_path_stream_mode == DATA_BIND_JSON_PATH_STREAM_FIRST) {
    ...
    parser->result_count++;
    data_bind_stream_provider_state(parser)->json_stream_done = 1;
    return 0;
  }
...
static int data_bind_stream_json_frame_reserve(data_bind_stream_t *parser) {
  ...
  if (data_bind_stream_provider_state(parser)->json_frame_count < data_bind_stream_provider_state(parser)->json_frame_capacity) return 0;
  if (data_bind_stream_provider_state(parser)->json_frame_count >= DATA_BIND_JSON_STREAM_MAX_DEPTH) {
    data_bind_stream_error_msg(parser, "JSON stream nesting too deep");
    return -1;
  }
  next_capacity = data_bind_stream_provider_state(parser)->json_frame_capacity == 0 ? 8 : data_bind_stream_provider_state(parser)->json_frame_capacity * 2;
  if (next_capacity > DATA_BIND_JSON_STREAM_MAX_DEPTH)
    next_capacity = DATA_BIND_JSON_STREAM_MAX_DEPTH;
  if (next_capacity <= data_bind_stream_provider_state(parser)->json_frame_capacity) {
    data_bind_stream_error_msg(parser, "JSON stream nesting too deep");
    return -1;
  }
  grown =
      (data_bind_json_stream_frame_t *)realloc(data_bind_stream_provider_state(parser)->json_frames, next_capacity * sizeof(*grown));
  if (grown == NULL) {
    data_bind_stream_error_msg(parser, "Out of memory growing JSON stream stack");
    return -1;
  }
  data_bind_stream_provider_state(parser)->json_frames = grown;
  data_bind_stream_provider_state(parser)->json_frame_capacity = next_capacity;
  return 0;
}
...
static int data_bind_stream_json_attach_value(data_bind_stream_t *parser, json_value_t *value) {
  ...
  if (data_bind_stream_provider_state(parser)->json_frame_count == 0) return 0;
  parent = &data_bind_stream_provider_state(parser)->json_frames[data_bind_stream_provider_state(parser)->json_frame_count - 1];
  if (parent->is_object) {
...
static int data_bind_stream_json_scalar(data_bind_stream_t *parser, json_value_t *value) {
  ...
  if (!data_bind_stream_provider_state(parser)->json_stream_active || data_bind_stream_provider_state(parser)->json_sax_depth == 0) {
    (json_free(value), value = NULL);
    return 0;
  }
  if (data_bind_stream_provider_state(parser)->json_sax_depth == 1 || data_bind_stream_provider_state(parser)->json_frame_count > 0) {
    if (data_bind_stream_json_attach_value(parser, value) != 0) {
      (json_free(value), value = NULL);
      return -1;
    }
    if (data_bind_stream_provider_state(parser)->json_frame_count == 0) {
...
static int data_bind_stream_json_container_start(data_bind_stream_t *parser, json_value_t *value,
  ...
  frame = &data_bind_stream_provider_state(parser)->json_frames[data_bind_stream_provider_state(parser)->json_frame_count++];
  frame->value = value;
...
static int data_bind_stream_json_container_end(data_bind_stream_t *parser, int is_object) {
  data_bind_json_stream_frame_t frame;
  if (parser == NULL || !data_bind_stream_provider_state(parser)->json_stream_active || data_bind_stream_provider_state(parser)->json_frame_count == 0) return 0;
  frame = data_bind_stream_provider_state(parser)->json_frames[data_bind_stream_provider_state(parser)->json_frame_count - 1];
  if (frame.is_object != is_object) {
    data_bind_stream_error_msg(parser, "JSON stream container mismatch");
    return -1;
  }
  free(frame.pending_key);
  data_bind_stream_provider_state(parser)->json_frames[--data_bind_stream_provider_state(parser)->json_frame_count].pending_key = NULL;
  if (data_bind_stream_provider_state(parser)->json_frame_count == 0) {
...
static int data_bind_stream_json_on_object_start(void *ctx) {
  ...
  if (data_bind_stream_provider_state(parser)->json_sax_depth == 0) {
    data_bind_stream_provider_state(parser)->json_root_seen = 1;
  } else {
    rc = data_bind_stream_json_container_start(parser, json_create_object(), 1);
  }
  data_bind_stream_provider_state(parser)->json_sax_depth++;
  return rc;
}
...
static int data_bind_stream_json_on_object_end(void *ctx) {
  data_bind_stream_t *parser = (data_bind_stream_t *)ctx;
  int rc;
  if (parser == NULL || data_bind_stream_provider_state(parser)->json_sax_depth == 0) return -1;
...
static int data_bind_stream_json_on_array_start(void *ctx) {
  ...
  if (data_bind_stream_provider_state(parser)->json_sax_depth == 0) {
    data_bind_stream_provider_state(parser)->json_root_seen = 1;
    if (data_bind_stream_provider_state(parser)->json_stream_candidate) {
      data_bind_stream_provider_state(parser)->json_stream_active = 1;
      free(parser->buffer);
      parser->buffer = NULL;
      parser->size = 0;
      parser->capacity = 0;
...
static int data_bind_stream_json_on_array_end(void *ctx) {
  ...
  int rc = 0;
  if (parser == NULL || data_bind_stream_provider_state(parser)->json_sax_depth == 0) return -1;
  if (data_bind_stream_provider_state(parser)->json_stream_active && data_bind_stream_provider_state(parser)->json_sax_depth == 1) {
    data_bind_stream_provider_state(parser)->json_stream_done = 1;
  } else {
...
static int data_bind_stream_json_path_match_start(void *ctx, json_type_t type) {
  ...
  if (parser == NULL) return -1;
  if (data_bind_stream_provider_state(parser)->json_path_stream_mode == DATA_BIND_JSON_PATH_STREAM_FIRST &&
      data_bind_stream_provider_state(parser)->json_stream_done) {
    data_bind_stream_provider_state(parser)->json_stream_active = 0;
    return 0;
  }
  if (data_bind_stream_provider_state(parser)->json_stream_active || data_bind_stream_provider_state(parser)->json_frame_count != 0 ||
      data_bind_stream_provider_state(parser)->json_match_value != NULL) {
...
static int data_bind_stream_json_path_scalar(data_bind_stream_t *parser, json_value_t *value) {
  ...
  if (parser == NULL || !data_bind_stream_provider_state(parser)->json_stream_active) {
    (json_free(value), value = NULL);
    return 0;
  }
  if (data_bind_stream_provider_state(parser)->json_frame_count != 0) {
  ...
  } else if (data_bind_stream_provider_state(parser)->json_match_value == NULL) {
    data_bind_stream_provider_state(parser)->json_match_value = value;
  } else {
    (json_free(value), value = NULL);
...
static int data_bind_stream_json_path_container_end(data_bind_stream_t *parser, int is_object) {
  data_bind_json_stream_frame_t *frame;
  if (parser == NULL || !data_bind_stream_provider_state(parser)->json_stream_active) return 0;
  if (data_bind_stream_provider_state(parser)->json_frame_count == 0) {
    data_bind_stream_error_msg(parser, "JSONPath stream container stack underflow");
    return -1;
  }
  frame = &data_bind_stream_provider_state(parser)->json_frames[data_bind_stream_provider_state(parser)->json_frame_count - 1U];
  if (frame->is_object != is_object) {
  ...
  free(frame->pending_key);
  frame->pending_key = NULL;
  data_bind_stream_provider_state(parser)->json_frame_count--;
  return 0;
}
...
static int data_bind_stream_json_path_match_end(void *ctx, json_type_t type) {
  ...
  (void)type;
  if (parser == NULL) return -1;
  if (!data_bind_stream_provider_state(parser)->json_stream_active) return 0;
  if (data_bind_stream_provider_state(parser)->json_frame_count != 0 || data_bind_stream_provider_state(parser)->json_match_value == NULL) {
    data_bind_stream_error_msg(parser, "Incomplete JSONPath stream match");
    return -1;
  }
  matched = data_bind_stream_provider_state(parser)->json_match_value;
  data_bind_stream_provider_state(parser)->json_match_value = NULL;
  data_bind_stream_provider_state(parser)->json_stream_active = 0;
...
static int data_bind_stream_xml_append(data_bind_stream_t *parser, const char *text, size_t len) {
  tstr next;
  if (parser == NULL || !data_bind_stream_provider_state(parser)->xml_capture_active || len == 0) return 0;
  next = tstr_cat_len(data_bind_stream_provider_state(parser)->xml_capture, text, len);
  if (next == NULL) {
    data_bind_stream_error_msg(parser, "Out of memory extending XML stream item");
    return -1;
  }
  data_bind_stream_provider_state(parser)->xml_capture = next;
  return 0;
}
...
static int data_bind_stream_xml_close_start(data_bind_stream_t *parser) {
  if (parser != NULL && data_bind_stream_provider_state(parser)->xml_capture_active && data_bind_stream_provider_state(parser)->xml_open_start) {
    if (data_bind_stream_xml_append_char(parser, '>') != 0) return -1;
    data_bind_stream_provider_state(parser)->xml_open_start = 0;
  }
  return 0;
}
...
static int data_bind_stream_xml_bind_capture(data_bind_stream_t *parser) {
  ...
  DataBindValue *item;
  if (parser == NULL || data_bind_stream_provider_state(parser)->xml_capture == NULL) return -1;
  if (salts_xml_parse(&doc, data_bind_stream_provider_state(parser)->xml_capture, tstr_len(data_bind_stream_provider_state(parser)->xml_capture),
...
static int data_bind_stream_xml_on_element_start(void *ctx, const char *name, size_t name_len) {
  data_bind_stream_t *parser = (data_bind_stream_t *)ctx;
  if (parser == NULL || !data_bind_stream_provider_state(parser)->xml_stream_candidate) return 0;
  if (!data_bind_stream_provider_state(parser)->xml_capture_active &&
      !data_bind_stream_xml_name_eq(name, name_len, data_bind_stream_provider_state(parser)->xml_stream_target)) {
    return 0;
  }
  if (!data_bind_stream_provider_state(parser)->xml_capture_active) {
    tstr_clear(data_bind_stream_provider_state(parser)->xml_capture);
    data_bind_stream_provider_state(parser)->xml_capture_active = 1;
    data_bind_stream_provider_state(parser)->xml_capture_depth = 0;
  ...
  data_bind_stream_provider_state(parser)->xml_open_start = 1;
  data_bind_stream_provider_state(parser)->xml_capture_depth++;
  return 0;
}
...
static int data_bind_stream_xml_on_attribute(void *ctx, const char *name, size_t name_len,
  ...
  data_bind_stream_t *parser = (data_bind_stream_t *)ctx;
  if (parser == NULL || !data_bind_stream_provider_state(parser)->xml_capture_active || !data_bind_stream_provider_state(parser)->xml_open_start) return 0;
  if (data_bind_stream_xml_append_char(parser, ' ') != 0 ||
      data_bind_stream_xml_append(parser, name, name_len) != 0 ||
...
static int data_bind_stream_xml_on_element_end(void *ctx, const char *name, size_t name_len) {
  data_bind_stream_t *parser = (data_bind_stream_t *)ctx;
  int rc;
  if (parser == NULL || !data_bind_stream_provider_state(parser)->xml_capture_active) return 0;
  ...
  if (data_bind_stream_provider_state(parser)->xml_capture_depth > 0) data_bind_stream_provider_state(parser)->xml_capture_depth--;
  if (data_bind_stream_provider_state(parser)->xml_capture_depth == 0) {
    rc = data_bind_stream_xml_bind_capture(parser);
    data_bind_stream_provider_state(parser)->xml_capture_active = 0;
    data_bind_stream_provider_state(parser)->xml_open_start = 0;
    tstr_clear(data_bind_stream_provider_state(parser)->xml_capture);
    return rc;
  }
...
static DataBindStatus data_bind_stream_sax_error(data_bind_stream_t *parser, DataBindError *error,
  ...
  if (parser != NULL) {
    if (data_bind_stream_provider_state(parser)->stream_error[0] != '\0') {
      message = data_bind_stream_provider_state(parser)->stream_error;
    } else if (data_bind_stream_provider_state(parser)->json_path_stream != NULL) {
      message = json_path_stream_error(data_bind_stream_provider_state(parser)->json_path_stream);
      path = "json";
    } else if (data_bind_stream_provider_state(parser)->json_sax != NULL) {
      message = json_sax_parser_error(data_bind_stream_provider_state(parser)->json_sax);
      path = "json";
    } else if (data_bind_stream_provider_state(parser)->yaml_sax != NULL) {
      const cyaml_error_t *native_error = cyaml_sax_parser_error(data_bind_stream_provider_state(parser)->yaml_sax);
      message = native_error ? native_error->msg : NULL;
      path = "yaml";
    } else if (data_bind_stream_provider_state(parser)->xml_sax != NULL) {
      message = salts_xml_sax_parser_error(data_bind_stream_provider_state(parser)->xml_sax);
      path = "xml";
    }
    data_bind_stream_provider_state(parser)->sax_failed = 1;
  }
...
static DataBindStatus data_bind_stream_sax_feed(data_bind_stream_t *parser, const char *data,
  ...
  if (parser == NULL || data_bind_stream_provider_state(parser)->sax_failed) {
    return data_bind_stream_sax_error(parser, error, "stream feed");
  }
  if (data_bind_stream_provider_state(parser)->json_path_stream != NULL) {
    if (json_path_stream_feed(data_bind_stream_provider_state(parser)->json_path_stream, data, len) != 0) {
      return data_bind_stream_sax_error(parser, error, "JSONPath stream feed");
    }
  } else if (data_bind_stream_provider_state(parser)->json_sax != NULL) {
    if (json_sax_parser_feed(data_bind_stream_provider_state(parser)->json_sax, data, len) != 0) {
      return data_bind_stream_sax_error(parser, error, "JSON stream feed");
    }
  } else if (data_bind_stream_provider_state(parser)->yaml_sax != NULL) {
    if (cyaml_sax_parser_feed(data_bind_stream_provider_state(parser)->yaml_sax, data, len) != 0) {
      return data_bind_stream_sax_error(parser, error, "YAML stream feed");
    }
  } else if (data_bind_stream_provider_state(parser)->xml_sax != NULL) {
    if (salts_xml_sax_parser_feed(data_bind_stream_provider_state(parser)->xml_sax, data, len) != 0) {
...
static DataBindStatus data_bind_stream_sax_finish(data_bind_stream_t *parser,
  ...
  if (data_bind_stream_provider_state(parser)->json_path_stream != NULL) {
    if (json_path_stream_finish(data_bind_stream_provider_state(parser)->json_path_stream) != 0) {
      return data_bind_stream_sax_error(parser, error, "JSONPath stream finish");
    }
  } else if (data_bind_stream_provider_state(parser)->json_sax != NULL) {
    if (json_sax_parser_finish(data_bind_stream_provider_state(parser)->json_sax) != 0) {
      return data_bind_stream_sax_error(parser, error, "JSON stream finish");
    }
  } else if (data_bind_stream_provider_state(parser)->yaml_sax != NULL) {
    if (cyaml_sax_parser_finish(data_bind_stream_provider_state(parser)->yaml_sax) != 0) {
      return data_bind_stream_sax_error(parser, error, "YAML stream finish");
    }
  } else if (data_bind_stream_provider_state(parser)->xml_sax != NULL) {
    if (salts_xml_sax_parser_finish(data_bind_stream_provider_state(parser)->xml_sax) != 0) {
...
static int data_bind_stream_csv_record_append(data_bind_stream_t *parser, char ch) {
  ...
  if (data_bind_stream_provider_state(parser)->csv_record_len >= parser->limits.max_record_bytes) {
    parser->limit_failed = 1;
    data_bind_stream_error_msg(parser, "CSV record byte limit exceeded");
    return 0;
  }
  if (data_bind_stream_provider_state(parser)->csv_record_len + 1 >= data_bind_stream_provider_state(parser)->csv_record_capacity) {
    next_capacity = data_bind_stream_provider_state(parser)->csv_record_capacity == 0 ? 256 : data_bind_stream_provider_state(parser)->csv_record_capacity * 2;
    if (next_capacity <= data_bind_stream_provider_state(parser)->csv_record_capacity) return 0;
    if (next_capacity > parser->limits.max_record_bytes + 1u)
      next_capacity = parser->limits.max_record_bytes + 1u;
    grown = (char *)realloc(data_bind_stream_provider_state(parser)->csv_record, next_capacity);
    if (grown == NULL) return 0;
    data_bind_stream_provider_state(parser)->csv_record = grown;
    data_bind_stream_provider_state(parser)->csv_record_capacity = next_capacity;
  }
  data_bind_stream_provider_state(parser)->csv_record[data_bind_stream_provider_state(parser)->csv_record_len++] = ch;
  data_bind_stream_provider_state(parser)->csv_record[data_bind_stream_provider_state(parser)->csv_record_len] = '\0';
  return 1;
}
...
static int data_bind_stream_csv_field_append(data_bind_stream_t *parser, char ch) {
  ...
  size_t next_capacity;
  if (parser == NULL) return 0;
  if (data_bind_stream_provider_state(parser)->csv_field_len >= parser->limits.max_field_bytes) {
    parser->limit_failed = 1;
    data_bind_stream_error_msg(parser, "CSV field byte limit exceeded");
    return 0;
  }
  if (data_bind_stream_provider_state(parser)->csv_field_len + 1 >= data_bind_stream_provider_state(parser)->csv_field_capacity) {
    next_capacity = data_bind_stream_provider_state(parser)->csv_field_capacity == 0 ? 128 : data_bind_stream_provider_state(parser)->csv_field_capacity * 2;
    if (next_capacity <= data_bind_stream_provider_state(parser)->csv_field_capacity) return 0;
    if (next_capacity > parser->limits.max_field_bytes + 1u)
      next_capacity = parser->limits.max_field_bytes + 1u;
    grown = (char *)realloc(data_bind_stream_provider_state(parser)->csv_field, next_capacity);
    if (grown == NULL) return 0;
    data_bind_stream_provider_state(parser)->csv_field = grown;
    data_bind_stream_provider_state(parser)->csv_field_capacity = next_capacity;
  }
  data_bind_stream_provider_state(parser)->csv_field[data_bind_stream_provider_state(parser)->csv_field_len++] = ch;
  data_bind_stream_provider_state(parser)->csv_field[data_bind_stream_provider_state(parser)->csv_field_len] = '\0';
  return 1;
}
...
static DataBindStatus data_bind_stream_csv_growth_error(data_bind_stream_t *parser,
  ...
  if (parser != NULL) data_bind_stream_provider_state(parser)->csv_failed = 1;
  if (parser != NULL && parser->limit_failed)
    return db_error_set(error, DATA_BIND_ERR_LIMIT, "stream.limit", -1, -1, "%s",
                        data_bind_stream_provider_state(parser)->stream_error);
...
static void data_bind_stream_csv_clear_fields(data_bind_stream_t *parser) {
  ...
  for (i = 0; i < data_bind_stream_provider_state(parser)->csv_field_count; i++) {
    free(data_bind_stream_provider_state(parser)->csv_field_storage[i]);
    data_bind_stream_provider_state(parser)->csv_field_storage[i] = NULL;
  }
  data_bind_stream_provider_state(parser)->csv_field_count = 0;
  data_bind_stream_provider_state(parser)->csv_field_len = 0;
  if (data_bind_stream_provider_state(parser)->csv_field != NULL) data_bind_stream_provider_state(parser)->csv_field[0] = '\0';
}
...
static int data_bind_stream_csv_finish_field(data_bind_stream_t *parser) {
  ...
  if (parser == NULL) return 0;
  if (data_bind_stream_provider_state(parser)->csv_field_count >= data_bind_stream_provider_state(parser)->csv_fields_capacity) {
    next_capacity = data_bind_stream_provider_state(parser)->csv_fields_capacity == 0 ? 8 : data_bind_stream_provider_state(parser)->csv_fields_capacity * 2;
    if (next_capacity <= data_bind_stream_provider_state(parser)->csv_fields_capacity) return 0;
    grown_fields = (vstr *)realloc(data_bind_stream_provider_state(parser)->csv_fields, next_capacity * sizeof(*grown_fields));
    if (grown_fields == NULL) return 0;
    data_bind_stream_provider_state(parser)->csv_fields = grown_fields;
    grown_storage =
        (char **)realloc(data_bind_stream_provider_state(parser)->csv_field_storage, next_capacity * sizeof(*grown_storage));
    if (grown_storage == NULL) return 0;
    data_bind_stream_provider_state(parser)->csv_field_storage = grown_storage;
    data_bind_stream_provider_state(parser)->csv_fields_capacity = next_capacity;
  }

  field_copy = (char *)malloc(data_bind_stream_provider_state(parser)->csv_field_len + 1);
  if (field_copy == NULL) return 0;
  if (data_bind_stream_provider_state(parser)->csv_field_len > 0) memcpy(field_copy, data_bind_stream_provider_state(parser)->csv_field, data_bind_stream_provider_state(parser)->csv_field_len);
  field_copy[data_bind_stream_provider_state(parser)->csv_field_len] = '\0';
  data_bind_stream_provider_state(parser)->csv_field_storage[data_bind_stream_provider_state(parser)->csv_field_count] = field_copy;
  data_bind_stream_provider_state(parser)->csv_fields[data_bind_stream_provider_state(parser)->csv_field_count] = vstr_from_buf(field_copy, data_bind_stream_provider_state(parser)->csv_field_len);
  data_bind_stream_provider_state(parser)->csv_field_count++;
  data_bind_stream_provider_state(parser)->csv_field_len = 0;
  if (data_bind_stream_provider_state(parser)->csv_field != NULL) data_bind_stream_provider_state(parser)->csv_field[0] = '\0';
  return 1;
}
...
static DataBindStatus data_bind_stream_csv_compile_filter(data_bind_stream_t *parser,
  ...
  if (parser == NULL || parser->path_or_expr == NULL || data_bind_stream_provider_state(parser)->csv_filter != NULL)
    return DATA_BIND_OK;

  native_limits = db_query_native_limits(
      parser->query_limits_configured ? &parser->query_limits : NULL);
  header_doc_len = data_bind_stream_provider_state(parser)->csv_header_len + 1;
  header_doc = (char *)malloc(header_doc_len + 1);
  if (header_doc == NULL) {
    data_bind_stream_provider_state(parser)->csv_failed = 1;
    return db_error_set(error, DATA_BIND_ERR_OOM, "data_bind_stream_feed", -1, -1,
                        "Out of memory building CSVPath stream header");
  }
  memcpy(header_doc, data_bind_stream_provider_state(parser)->csv_header, data_bind_stream_provider_state(parser)->csv_header_len);
  header_doc[data_bind_stream_provider_state(parser)->csv_header_len] = '\n';
  header_doc[header_doc_len] = '\0';

  if ((data_bind_stream_provider_state(parser)->csv_filter_doc = csv_parse_opts(header_doc, header_doc_len, &opts)) == NULL) {
    free(header_doc);
    data_bind_stream_provider_state(parser)->csv_failed = 1;
  ...
  free(header_doc);
  if (data_bind_stream_provider_state(parser)->csv_filter_doc == NULL) {
    data_bind_stream_provider_state(parser)->csv_failed = 1;
    return db_error_set(error, DATA_BIND_ERR_PARSE, "csv", -1, -1,
                        "Failed to parse CSVPath stream header");
  }

  data_bind_stream_provider_state(parser)->csv_filter = dsv_filter_create(data_bind_stream_provider_state(parser)->csv_filter_doc, 0);
  compiled = data_bind_stream_provider_state(parser)->csv_filter != NULL &&
             dsv_filter_compile_ex(data_bind_stream_provider_state(parser)->csv_filter, parser->path_or_expr,
                                   &native_limits, &native_diagnostic);
  db_query_diagnostic_copy(&parser->query_diagnostic, &native_diagnostic);
  if (!compiled) {
    const char *filter_error = data_bind_stream_provider_state(parser)->csv_filter != NULL
                                   ? dsv_filter_error(data_bind_stream_provider_state(parser)->csv_filter)
...
static DataBindStatus data_bind_stream_csv_process_record(data_bind_stream_t *parser,
  ...
  if (data_bind_stream_provider_state(parser)->csv_record_len == 0 && data_bind_stream_provider_state(parser)->csv_header_seen) {
    data_bind_stream_csv_clear_fields(parser);
    return DATA_BIND_OK;
  }

  if (!data_bind_stream_provider_state(parser)->csv_header_seen) {
    data_bind_stream_provider_state(parser)->csv_header = (char *)malloc(data_bind_stream_provider_state(parser)->csv_record_len + 1);
    if (data_bind_stream_provider_state(parser)->csv_header == NULL) {
      data_bind_stream_provider_state(parser)->csv_failed = 1;
      return db_error_set(error, DATA_BIND_ERR_OOM, "data_bind_stream_feed", -1, -1,
                          "Out of memory storing CSV stream header");
    }
    memcpy(data_bind_stream_provider_state(parser)->csv_header, data_bind_stream_provider_state(parser)->csv_record, data_bind_stream_provider_state(parser)->csv_record_len);
    data_bind_stream_provider_state(parser)->csv_header[data_bind_stream_provider_state(parser)->csv_record_len] = '\0';
    data_bind_stream_provider_state(parser)->csv_header_len = data_bind_stream_provider_state(parser)->csv_record_len;
    data_bind_stream_provider_state(parser)->csv_header_seen = 1;
    data_bind_stream_provider_state(parser)->csv_record_len = 0;
    if (data_bind_stream_provider_state(parser)->csv_record != NULL) data_bind_stream_provider_state(parser)->csv_record[0] = '\0';
  ...
  if (parser->path_or_expr != NULL) {
    if (data_bind_stream_provider_state(parser)->csv_filter == NULL) {
      status = data_bind_stream_csv_compile_filter(parser, error);
      if (status != DATA_BIND_OK) return status;
    }
    match = dsv_filter_check_values(data_bind_stream_provider_state(parser)->csv_filter, data_bind_stream_provider_state(parser)->csv_fields,
                                          data_bind_stream_provider_state(parser)->csv_field_count);
    if (match < 0) {
      const qvm_diagnostic_t *native_diagnostic = dsv_filter_qvm_diagnostic(data_bind_stream_provider_state(parser)->csv_filter);
    ...
    if (match == 0) {
      ...
      data_bind_stream_provider_state(parser)->csv_record_len = 0;
      if (data_bind_stream_provider_state(parser)->csv_record != NULL) data_bind_stream_provider_state(parser)->csv_record[0] = '\0';
      data_bind_stream_provider_state(parser)->csv_data_row++;
      return DATA_BIND_OK;
    }
  }

  doc_len = data_bind_stream_provider_state(parser)->csv_header_len + 1 + data_bind_stream_provider_state(parser)->csv_record_len + 1;
  ...
  memcpy(doc_text, data_bind_stream_provider_state(parser)->csv_header, data_bind_stream_provider_state(parser)->csv_header_len);
  doc_text[data_bind_stream_provider_state(parser)->csv_header_len] = '\n';
  memcpy(doc_text + data_bind_stream_provider_state(parser)->csv_header_len + 1, data_bind_stream_provider_state(parser)->csv_record, data_bind_stream_provider_state(parser)->csv_record_len);
  doc_text[doc_len - 1] = '\n';
  doc_text[doc_len] = '\0';
    ...
    if (data_bind_stream_emit_record(parser, value) != 0) {
      ...
      if (parser->canceled)
        return db_error_set(error, DATA_BIND_ERR_CANCELED, "record_callback", -1, -1,
                            ...
                            (unsigned long long)data_bind_stream_provider_state(parser)->csv_data_row);
      return db_error_set(error, DATA_BIND_ERR_RUNTIME, "record_callback", -1, -1,
  ...
  if (data_bind_stream_provider_state(parser)->csv_record != NULL) data_bind_stream_provider_state(parser)->csv_record[0] = '\0';
  data_bind_stream_provider_state(parser)->csv_data_row++;
  if (status != DATA_BIND_OK) data_bind_stream_provider_state(parser)->csv_failed = 1;
  return status;
}
...
static DataBindStatus data_bind_stream_csv_feed(data_bind_stream_t *parser, const char *data,
  ...
  for (i = 0; i < len; i++) {
    char ch = data[i];
  reprocess:
    if (data_bind_stream_provider_state(parser)->csv_skip_next_lf) {
      data_bind_stream_provider_state(parser)->csv_skip_next_lf = 0;
      if (ch == '\n') continue;
    }
    if (data_bind_stream_provider_state(parser)->csv_quote_pending) {
      data_bind_stream_provider_state(parser)->csv_quote_pending = 0;
      if (ch == '"') {
      ...
      data_bind_stream_provider_state(parser)->csv_in_quotes = 0;
      goto reprocess;
    }

    if (data_bind_stream_provider_state(parser)->csv_in_quotes) {
      if (!data_bind_stream_csv_record_append(parser, ch)) {
        return data_bind_stream_csv_growth_error(
            parser, error, "Out of memory extending CSV stream record");
      }
      if (ch == '"') {
        data_bind_stream_provider_state(parser)->csv_quote_pending = 1;
    ...
    if (ch == '"') {
      data_bind_stream_provider_state(parser)->csv_in_quotes = 1;
      if (!data_bind_stream_csv_record_append(parser, ch)) {
        return data_bind_stream_csv_growth_error(
            parser, error, "Out of memory extending CSV stream record");
      }
    ...
    if (ch == '\r' || ch == '\n') {
      ...
      if (status != DATA_BIND_OK) return status;
      if (ch == '\r') data_bind_stream_provider_state(parser)->csv_skip_next_lf = 1;
      continue;
    }
    if (!data_bind_stream_csv_record_append(parser, ch)) {
      return data_bind_stream_csv_growth_error(
...
static DataBindStatus data_bind_stream_csv_finish(data_bind_stream_t *parser,
  ...
  if (data_bind_stream_provider_state(parser)->csv_record_len > 0 || data_bind_stream_provider_state(parser)->csv_field_len > 0 || data_bind_stream_provider_state(parser)->csv_field_count > 0 ||
      !data_bind_stream_provider_state(parser)->csv_header_seen) {
    if (!data_bind_stream_csv_finish_field(parser)) {
      data_bind_stream_provider_state(parser)->csv_failed = 1;
  ...
  if (!data_bind_stream_provider_state(parser)->csv_header_seen || data_bind_stream_provider_state(parser)->csv_failed || parser->csv_values == NULL) {
...
static data_bind_stream_t *data_bind_stream_create_common(
  ...
  if (is_csv) {
    ...
    if (data_bind_stream_provider_state(parser)->json_stream_candidate ||
        data_bind_stream_provider_state(parser)->json_path_stream_mode == DATA_BIND_JSON_PATH_STREAM_ALL) {
    ...
    if (data_bind_stream_provider_state(parser)->json_path_stream_mode != DATA_BIND_JSON_PATH_STREAM_NONE) {
      const char *path_error;
      data_bind_stream_provider_state(parser)->json_path_program = json_path_compile(parser->path_or_expr);
      if (data_bind_stream_provider_state(parser)->json_path_program == NULL) {
        data_bind_stream_provider_state(parser)->json_path_stream_mode = DATA_BIND_JSON_PATH_STREAM_NONE;
        data_bind_value_free(parser->stream_values);
        parser->stream_values = NULL;
      } else {
        data_bind_stream_provider_state(parser)->json_path_stream = json_path_stream_create(
            data_bind_stream_provider_state(parser)->json_path_program, &DATA_BIND_JSON_PATH_STREAM_HANDLER, parser);
      }
      if (data_bind_stream_provider_state(parser)->json_path_program != NULL && data_bind_stream_provider_state(parser)->json_path_stream == NULL) {
        path_error = json_path_stream_error(NULL);
        if (path_error != NULL && strstr(path_error, "not streamable") != NULL) {
          json_path_program_free(data_bind_stream_provider_state(parser)->json_path_program);
          data_bind_stream_provider_state(parser)->json_path_program = NULL;
          data_bind_stream_provider_state(parser)->json_path_stream_mode = DATA_BIND_JSON_PATH_STREAM_NONE;
          data_bind_value_free(parser->stream_values);
          parser->stream_values = NULL;
        } else {
    ...
    if (data_bind_stream_provider_state(parser)->json_path_stream == NULL) {
      data_bind_stream_provider_state(parser)->json_sax = data_bind_stream_provider_state(parser)->json_stream_candidate
                             ? json_sax_parser_create_raw(&DATA_BIND_JSON_STREAM_HANDLER,
                                                               parser)
                             : json_sax_parser_create(&DATA_BIND_JSON_SAX_VALIDATE_HANDLER,
                                                           parser);
    }
    if (data_bind_stream_provider_state(parser)->json_path_stream == NULL && data_bind_stream_provider_state(parser)->json_sax == NULL) {
      data_bind_value_free(parser->stream_values);
      json_path_program_free(data_bind_stream_provider_state(parser)->json_path_program);
      free(parser->path_or_expr);
      free(parser->type_name);
    ...
    data_bind_stream_provider_state(parser)->yaml_sax =
        cyaml_sax_parser_create(&DATA_BIND_YAML_SAX_VALIDATE_HANDLER, parser, NULL);
    if (data_bind_stream_provider_state(parser)->yaml_sax == NULL) {
      free(parser->path_or_expr);
      free(parser->type_name);
      free(parser);
      db_error_set(error, DATA_BIND_ERR_OOM, "data_bind_stream_create", -1, -1,
    ...
    if (data_bind_stream_provider_state(parser)->xml_stream_candidate) {
      data_bind_stream_provider_state(parser)->xml_stream_target = data_bind_stream_xml_target_from_path(parser->path_or_expr);
      data_bind_stream_provider_state(parser)->xml_capture = tstr_new();
      parser->stream_values = dbv_new(DATA_BIND_VALUE_LIST);
      if (data_bind_stream_provider_state(parser)->xml_stream_target == NULL || data_bind_stream_provider_state(parser)->xml_capture == NULL ||
          parser->stream_values == NULL ||
          !dbv_sequence_set_limit(parser->stream_values,
                                  parser->limits.max_result_count)) {
        free(data_bind_stream_provider_state(parser)->xml_stream_target);
        tstr_free(data_bind_stream_provider_state(parser)->xml_capture);
        data_bind_value_free(parser->stream_values);
        free(parser->path_or_expr);
        free(parser->type_name);
        free(parser);
    ...
    data_bind_stream_provider_state(parser)->xml_sax = salts_xml_sax_parser_create(data_bind_stream_provider_state(parser)->xml_stream_candidate
                                                      ? &DATA_BIND_XML_STREAM_HANDLER
                                                      : &DATA_BIND_XML_SAX_VALIDATE_HANDLER,
                                                  parser, parser->limits.max_input_bytes);
    if (data_bind_stream_provider_state(parser)->xml_sax == NULL) {
      free(data_bind_stream_provider_state(parser)->xml_stream_target);
      tstr_free(data_bind_stream_provider_state(parser)->xml_capture);
      data_bind_value_free(parser->stream_values);
      free(parser->path_or_expr);
...
DataBindStatus data_bind_stream_set_limits(data_bind_stream_t *stream,
  ...
  if (data_bind_stream_provider_state(parser)->xml_sax != NULL &&
      salts_xml_sax_parser_set_buffer_limit(data_bind_stream_provider_state(parser)->xml_sax, limits->max_input_bytes) != 0) {
  ...
  parser->limits.size = sizeof(parser->limits);
  parser->limit_failed = 0;
  data_bind_stream_provider_state(parser)->stream_error[0] = '\0';
  db_error_clear(parser->error);
  return DATA_BIND_OK;
}
...
DataBindStatus data_bind_stream_set_query_limits(
  ...
  if (data_bind_stream_provider_state(parser)->json_path_program != NULL && parser->path_or_expr != NULL) {
    ...
    if (data_bind_stream_provider_state(parser)->json_path_stream != NULL &&
      ...
      json_path_stream_destroy(data_bind_stream_provider_state(parser)->json_path_stream);
      data_bind_stream_provider_state(parser)->json_path_stream = NULL;
      data_bind_stream_provider_state(parser)->json_path_stream_mode = DATA_BIND_JSON_PATH_STREAM_NONE;
      data_bind_value_free(parser->stream_values);
      parser->stream_values = NULL;
      data_bind_stream_provider_state(parser)->json_sax = replacement;
    }
  }
  db_error_clear(parser->error);
  return DATA_BIND_OK;
...
static DataBindStatus data_bind_stream_text_feed(data_bind_stream_t *parser, const char *data,
  ...
  if (data_bind_stream_provider_state(parser)->json_path_stream != NULL ||
      (data_bind_stream_provider_state(parser)->json_stream_candidate && data_bind_stream_provider_state(parser)->json_stream_active) ||
      data_bind_stream_provider_state(parser)->xml_stream_candidate) {
    parser->started = 1;
    db_error_clear(error);
    return DATA_BIND_OK;
  }
...
static DataBindStatus data_bind_stream_json_finish(data_bind_stream_t *parser,
  ...
  if (data_bind_stream_provider_state(parser)->json_path_stream != NULL ||
      ...
      data_bind_stream_provider_state(parser)->xml_stream_candidate) {
    if (data_bind_stream_provider_state(parser)->json_path_stream != NULL &&
        data_bind_stream_provider_state(parser)->json_path_stream_mode == DATA_BIND_JSON_PATH_STREAM_FIRST &&
        parser->result_count == 0) {
      parser->finished = 1;
