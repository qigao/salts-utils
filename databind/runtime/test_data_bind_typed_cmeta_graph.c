...
static void check_native_text_format_isolated(
  ...
  parse_status = data_bind_typed_descriptor_parse(
  ...
  if (parse_status == DATA_BIND_OK)
    serialize_status = data_bind_typed_descriptor_serialize(
  ...
  (void)data_bind_typed_descriptor_clear(descriptor, &value, &error);
...
static DataBindStatus parse_depth32(DataBindFormat format, const char *input,
                                    size_t input_length) {
  const DataBindTypedDescriptor *descriptor = Depth32_typed_descriptor();
  DataBindError error = DATA_BIND_ERROR_INIT;
  DataBind *codec = NULL;
  Depth32_t value;
  ...
  if (status == DATA_BIND_OK && descriptor != NULL) {
    ...
    status = data_bind_typed_descriptor_parse(
...
static DataBindStatus serialize_depth32(DataBindFormat format) {
  ...
  if (status == DATA_BIND_OK && descriptor != NULL) {
    status = data_bind_typed_descriptor_serialize(
...
static DataBindStatus parse_sample_count(DataBindFormat format,
  ...
  DataBindStatus status = Graph_codec_create(&codec, &error);
  if (status == DATA_BIND_OK && descriptor != NULL) {
    status = data_bind_typed_descriptor_parse(codec, "Sample", descriptor, format,
...
spec("generated native CMeta graph") {
  it("publishes structural metadata and validates the public descriptor") {
    ...
    if (data && descriptor) {
      ...
      DataBindTypedDescriptor descriptor_copy = *descriptor;
      ...
      check_equal(data_bind_typed_descriptor_validate(&descriptor_copy, &error),
                  DATA_BIND_OK);
      ...
      check(descriptor->overlay->fields[2].flags & DATA_BIND_TYPED_FIELD_WIRE_OFFSET);
  ...
  it("keeps supported native descriptor JSON paths isolated from dynamic values") {
    ...
    if (codec != NULL && descriptor != NULL)
      check_equal(data_bind_typed_descriptor_parse(
    ...
    if (codec != NULL && descriptor != NULL)
      check_equal(data_bind_typed_descriptor_serialize(
    ...
    data_bind_typed_serialized_free(serialized);
    if (descriptor != NULL)
      check_equal(data_bind_typed_descriptor_clear(descriptor, &value, &error),
                  DATA_BIND_OK);
    data_bind_free(codec);
    data_bind_set_value_pool_enabled(1);
  }
  ...
  it("preserves textual Boolean compatibility") {
    ...
    const DataBindTypedDescriptor *descriptor = BoolStorage_typed_descriptor();
    DataBindError error = DATA_BIND_ERROR_INIT;
    DataBind *codec = NULL;
  ...
  it("requires exact canonical providers for generated fixed values") {
    ...
    for (index = 0u; index < 3u; ++index) {
      DataBindTypedDescriptor altered_descriptor = *descriptor;
...

    {
      ...
      failed_len = 0u;
      check_equal(data_bind_typed_descriptor_serialize_binary_into(
  ...
  it("round-trips UINT64_MAX through canonical bits and descriptor binary APIs") {
    ...
    check_equal(data_bind_typed_descriptor_init(descriptor, &object, &error),
                DATA_BIND_OK);
    check_equal(data_bind_typed_descriptor_init(descriptor, &decoded, &error),
                DATA_BIND_OK);
    ...
    check_equal(data_bind_typed_descriptor_serialize_binary(
    ...
    data_bind_free(codec);
    data_bind_typed_serialized_free(wire);
    check_equal(data_bind_typed_descriptor_clear(descriptor, &decoded, &error),
                DATA_BIND_OK);
  ...
  it("rejects an unknown enum through canonical bits and preserves the object") {
    ...
    const DataBindTypedDescriptor *descriptor = Signed8Storage_typed_descriptor();
    const cmeta_data_desc *data;
    const cmeta_data_desc *enum_data;
    const cmeta_data_struct_shape *shape;
    DataBind *codec = NULL;
