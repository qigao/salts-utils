...
spec("DataBind Plugin projection semantic rejection") {
  it("rejects typed Service errors before creating outputs") {
    ...
    static const char client_output[] =
        "databind_plugin_typed_error_should_not_exist.client.c";
    Node *root = NULL;
    ...
    databind_compiler_plugin_config config = {
        ...
        .client_source_output = client_output,
    };
    ...
    (void)salts_fs_unlink(client_output);
    ...
    check(salts_fs_access(client_output, SALTS_FS_ACCESS_EXISTS) != 0);
  ...
  it("rejects invalid schema contract versions without outputs") {
    ...
    static const char client_output[] =
        "databind_plugin_bad_version_should_not_exist.client.c";
    ...
    for (i = 0u;
      ...
      check(salts_fs_access(
                client_output, SALTS_FS_ACCESS_EXISTS) != 0);
    }
  ...
  it("requires an explicit existing Component and never falls back to schema-wide publication") {
    ...
    static const char client_output[] =
        "databind_plugin_component_required_should_not_exist.client.c";
    Node *root = NULL;
    char *schema_data = NULL;
    databind_compiler_plugin_config config = {
        .plugin_version_major = 1u,
  ...
  it("publishes multiple Services as independent contracts") {
    ...
    static const char client_output[] =
        "databind_plugin_multi_service.client.c";
    Node *root = NULL;
    char *schema_data = NULL;
    ...
    check_equal(salts_fs_access(
                ...
                0);
    check_equal(salts_fs_access(
                    client_output, SALTS_FS_ACCESS_EXISTS),
                0);

    {
      salts_fs_buf_t client_generated = {0};
      check_equal(salts_fs_read_file(client_output, &client_generated), 0);
      check_not_null(client_generated.base);
      check_not_null(strstr(
          client_generated.base, "plugin_client_open"));
      check_not_null(strstr(
          client_generated.base, "cmeta_function_desc_equal"));
      salts_fs_buf_free(&client_generated);
    }
