...
#include <io.h>
#include <sys/stat.h>
#define databind_compiler_fdopen _fdopen
#define databind_compiler_open _open
...
#include <sys/stat.h>
#include <unistd.h>
#define databind_compiler_fdopen fdopen
#define databind_compiler_open open
...
static Node *databind_compiler_find_child(Node *parent, const char *name) {
...
static const char *databind_compiler_string_value(Node *parent, const char *name) {
  Node *child = databind_compiler_find_child(parent, name);
  if (!child || child->type != NODE_STRING) return NULL;
  return child->data.string_val;
}

static int databind_compiler_create_temporary_output(const char *output_path,
  ...
  *out_file = NULL;
  ...
  for (attempt = 0; attempt < TBE_COMPILER_TEMP_OUTPUT_ATTEMPTS; ++attempt) {
    ...
    descriptor = databind_compiler_open(temporary_path, TBE_COMPILER_TEMP_OPEN_FLAGS,
    ...
    file = databind_compiler_fdopen(descriptor, "wb");
    if (!file) {
#ifdef _WIN32
      _close(descriptor);
#else
...
static int databind_compiler_parse_size(const char *text, size_t *out) {
  char *end = NULL;
...
static const char *databind_compiler_attribute_value(Node *owner, const char *name) {
  Node *attributes;
  size_t i;

  if (!owner || !name) return NULL;
  attributes = databind_compiler_find_child(owner, "attributes");
  ...
  for (i = 0; i < attributes->data.list.count; ++i) {
    ...
    const char *attribute_name = databind_compiler_string_value(attribute, "name");
    if (attribute_name && strcmp(attribute_name, name) == 0)
      return databind_compiler_string_value(attribute, "value");
  }
  return NULL;
}

static size_t databind_compiler_attribute_count(Node *owner, const char *name) {
  Node *attributes;
  size_t i, count = 0;
...
static int databind_compiler_has_child(Node *parent, const char *name) {
  return databind_compiler_find_child(parent, name) != NULL;
}

static void databind_compiler_remove_children(Node *map, const char *name) {
  size_t out = 0;
...
static int databind_compiler_set_string(Node *map, const char *name, const char *value) {
  Node *node;
  ...
  databind_compiler_remove_children(map, name);
...
static void databind_compiler_pascal_identifier(const char *input, char *out, size_t out_size) {
  int capitalize = 1;
  size_t pos = 0;
...
static int databind_compiler_c_identifier_valid(const char *identifier) {
...
typedef struct databind_compiler_scalar_projection {
...
} databind_compiler_scalar_projection_t;

typedef enum databind_compiler_native_requirement {
...
} databind_compiler_native_requirement_t;

static const char *databind_compiler_native_requirement_name(
    databind_compiler_native_requirement_t requirement) {
  switch (requirement) {
...
static const databind_compiler_scalar_projection_t TBE_COMPILER_SCALAR_PROJECTIONS[] = {
...
static const databind_compiler_scalar_projection_t *databind_compiler_scalar_projection(const char *type) {
  ...
  for (i = 0; i < sizeof(TBE_COMPILER_SCALAR_PROJECTIONS) /
       ...
       ++i) {
    const databind_compiler_scalar_projection_t *projection = &TBE_COMPILER_SCALAR_PROJECTIONS[i];
...
static const databind_compiler_scalar_projection_t *databind_compiler_integer_type(const char *type) {
  const databind_compiler_scalar_projection_t *projection = databind_compiler_scalar_projection(type);
...
static const char *databind_compiler_cpp_scalar_type(const char *type) {
  const databind_compiler_scalar_projection_t *projection;
  if (!type) return "std::any";
  projection = databind_compiler_scalar_projection(type);
...
static const char *databind_compiler_go_scalar_type(const char *type) {
  const databind_compiler_scalar_projection_t *projection;
  if (!type) return "any";
  projection = databind_compiler_scalar_projection(type);
...
static const char *databind_compiler_ts_scalar_type(const char *type) {
  const databind_compiler_scalar_projection_t *projection;
...
static const char *databind_compiler_python_scalar_type(const char *type) {
  const databind_compiler_scalar_projection_t *projection;
  if (!type) return "Any";
  projection = databind_compiler_scalar_projection(type);
...
static const char *databind_compiler_rust_scalar_type(const char *type) {
  const databind_compiler_scalar_projection_t *projection;
  if (!type) return "()";
  projection = databind_compiler_scalar_projection(type);
...
static const char *databind_compiler_rfl_scalar_type(const char *type) {
  const databind_compiler_scalar_projection_t *projection;
...
static const char *databind_compiler_typed_kind(const char *type) {
  const databind_compiler_scalar_projection_t *projection;
  if (!type) return NULL;
  projection = databind_compiler_scalar_projection(type);
...
static const char *databind_compiler_typed_c_scalar(const char *type) {
  const databind_compiler_scalar_projection_t *projection;
  if (!type) return NULL;
  projection = databind_compiler_scalar_projection(type);
...
static Node *databind_compiler_find_record(Node *root, const char *list_name, const char *name) {
  Node *list = databind_compiler_find_child(root, list_name);
  size_t i;
  if (!list || list->type != NODE_LIST || !name) return NULL;
  for (i = 0; i < list->data.list.count; ++i) {
    Node *record = list->data.list.items[i];
    const char *record_name = databind_compiler_string_value(record, "name");
    if (record_name && strcmp(record_name, name) == 0) return record;
  }
  return NULL;
}

static const char *databind_compiler_typed_named_kind(Node *root, const char *type,
                                                 char *c_type, size_t c_type_size,
                                                 char *descriptor, size_t descriptor_size) {
  const char *kind = databind_compiler_typed_kind(type);
  const char *scalar = databind_compiler_typed_c_scalar(type);
  Node *record;
  ...
  record = databind_compiler_find_record(root, "enums", type);
  if (record) {
    snprintf(c_type, c_type_size, "%s_t", type);
    descriptor[0] = '\0';
    return "TBE_TYPED_ENUM";
  }
  record = databind_compiler_find_record(root, "composites", type);
  if (!record) record = databind_compiler_find_record(root, "groups", type);
  if (!record) record = databind_compiler_find_record(root, "messages", type);
  if (record) {
    snprintf(c_type, c_type_size, "%s_t", type);
...
static const char *databind_compiler_typed_wire_kind(Node *root, const char *type,
                                                const char *fallback) {
  Node *record = databind_compiler_find_record(root, "enums", type);
  if (record) {
    const char *underlying = databind_compiler_string_value(record, "underlying_type");
    const char *kind = databind_compiler_typed_kind(underlying ? underlying : "int32");
    return kind ? kind : "TBE_TYPED_I32";
  }
  return fallback;
}

static const char *databind_compiler_cpp_enum_underlying_type(const char *type) {
  return databind_compiler_cpp_scalar_type((type && type[0]) ? type : "int32");
}

static const char *databind_compiler_rust_enum_underlying_type(const char *type) {
  return databind_compiler_rust_scalar_type((type && type[0]) ? type : "int32");
}

typedef const char *(*tbe_scalar_mapper_t)(const char *);

static void databind_compiler_field_type(Node *field,
                                    ...
                                    char *out,
                                    size_t out_size) {
  const char *type = databind_compiler_string_value(field, "type");

  if (!out || out_size == 0) return;

  if (semantic && semantic->kind == CMETA_DATA_SEQUENCE &&
      strcmp(semantic->schema_kind, "group") == 0) {
    const char *group_type = databind_compiler_string_value(field, "group_type");
  ...
  if (semantic && cmeta_data_kind_is_container(semantic->kind)) {
    const char *inner_type = databind_compiler_string_value(field, "inner_type");
    const char *key_type = databind_compiler_string_value(field, "key_type");
    const char *value_type = databind_compiler_string_value(field, "value_type");
  ...
  if (semantic && semantic->kind == CMETA_DATA_BYTES &&
      databind_compiler_has_child(field, "is_fixed_size")) {
...
static int databind_compiler_set_enum_symbol(Node *target, const char *key,
  ...
  status = databind_compiler_set_string(target, key, symbol);
  free(symbol);
  return status;
}

static void databind_compiler_annotate_typed_field(Node *root, Node *field,
                                             const schema_cmeta_field_type *semantic) {
  const char *name = databind_compiler_string_value(field, "name");
  const char *c_name = databind_compiler_attribute_value(field, "c");
  const char *owner = databind_compiler_string_value(field, "owner_name");
  const char *type = databind_compiler_string_value(field, "type");
  char c_type[256] = {0};
  ...
  databind_compiler_set_string(field, "c_name", c_name);
  if (semantic && semantic->kind == CMETA_DATA_SEQUENCE &&
      strcmp(semantic->schema_kind, "group") == 0) {
    const char *group_type = databind_compiler_string_value(field, "group_type");
    ...
    databind_compiler_set_string(field, "typed_kind", "TBE_TYPED_LIST");
    databind_compiler_set_string(field, "typed_element_kind", "TBE_TYPED_OBJECT");
    databind_compiler_set_string(field, "typed_element_wire_kind", "TBE_TYPED_OBJECT");
    databind_compiler_set_string(field, "typed_element_c_type", c_type);
    databind_compiler_set_string(field, "typed_object_descriptor", descriptor);
    databind_compiler_set_string(field, "typed_vector_type", vector_type);
    databind_compiler_set_string(field, "typed_needs_vector", "1");
    databind_compiler_set_string(field, "typed_is_group", "1");
    databind_compiler_set_string(field, "typed_declaration", declaration);
    return;
  }
  if (semantic && semantic->kind == CMETA_DATA_BYTES) {
    if (databind_compiler_has_child(field, "is_fixed_size")) {
      const char *count = databind_compiler_string_value(field, "size_bytes");
      char base[768];
      char symbol[800];
      int written;
      snprintf(declaration, sizeof(declaration), "uint8_t %s[%s];", c_name,
               count ? count : "0");
      databind_compiler_set_string(field, "typed_kind", "TBE_TYPED_FIXED_BYTES");
      databind_compiler_set_string(field, "typed_wire_kind", "TBE_TYPED_FIXED_BYTES");
      databind_compiler_set_string(field, "typed_fixed_count", count ? count : "0");
      ...
      databind_compiler_set_string(field, "native_fixed_bytes_name", base);
      written = snprintf(symbol, sizeof(symbol), "%s_storage", base);
      if (written < 0 || (size_t)written >= sizeof(symbol)) {
        databind_compiler_set_string(field, "typed_declaration", declaration);
        return;
      }
      databind_compiler_set_string(field, "native_c_type", symbol);
      written = snprintf(symbol, sizeof(symbol), "%s_cmeta_data", base);
      if (written < 0 || (size_t)written >= sizeof(symbol)) {
        databind_compiler_set_string(field, "typed_declaration", declaration);
        return;
      }
      databind_compiler_set_string(field, "native_data_symbol", symbol);
      written = snprintf(symbol, sizeof(symbol), "%s_cmeta_type", base);
      if (written < 0 || (size_t)written >= sizeof(symbol)) {
        databind_compiler_set_string(field, "typed_declaration", declaration);
        return;
      }
      databind_compiler_set_string(field, "native_type_symbol", symbol);
    } else {
      snprintf(declaration, sizeof(declaration), "tbe_bytes_t %s;", c_name);
      databind_compiler_set_string(field, "typed_kind", "TBE_TYPED_BYTES");
  ...
  if (semantic && cmeta_data_kind_is_container(semantic->kind)) {
    const char *inner = databind_compiler_string_value(field, "inner_type");
    kind = databind_compiler_typed_named_kind(root, inner, c_type, sizeof(c_type), descriptor,
                                         sizeof(descriptor));
    if (!kind) {
    ...
    databind_compiler_set_string(field, "typed_element_kind", kind);
    databind_compiler_set_string(field, "typed_element_wire_kind",
                            databind_compiler_typed_wire_kind(root, inner, kind));
    databind_compiler_set_string(field, "typed_element_c_type", c_type);
    if (descriptor[0]) databind_compiler_set_string(field, "typed_object_descriptor", descriptor);
    if (databind_compiler_has_child(field, "is_fixed_size")) {
      const char *count = databind_compiler_string_value(field, "length_field");
      snprintf(declaration, sizeof(declaration), "%s %s[%s];", c_type, c_name,
               count ? count : "0");
      databind_compiler_set_string(field, "typed_kind", "TBE_TYPED_FIXED_ARRAY");
      databind_compiler_set_string(field, "typed_fixed_count", count ? count : "0");
      ...
      const char *value_kind = databind_compiler_typed_named_kind(
      ...
      snprintf(vector_type, sizeof(vector_type), "%s_%s_vec_t", owner, name);
      snprintf(declaration, sizeof(declaration), "%s %s;", vector_type, c_name);
      databind_compiler_set_string(field, "typed_kind", "TBE_TYPED_MAP");
      databind_compiler_set_string(field, "typed_map_entry_type", entry_type);
      databind_compiler_set_string(field, "typed_map_value_kind", value_kind);
      databind_compiler_set_string(field, "typed_map_value_wire_kind",
                              databind_compiler_typed_wire_kind(
                                  root, value_type ? value_type : inner, value_kind));
      databind_compiler_set_string(field, "typed_map_value_c_type", value_c_type);
      if (value_descriptor[0])
        databind_compiler_set_string(field, "typed_map_value_descriptor", value_descriptor);
      databind_compiler_set_string(field, "typed_vector_type", vector_type);
      databind_compiler_set_string(field, "typed_element_c_type", entry_type);
      databind_compiler_set_string(field, "typed_needs_map_vector", "1");
    } else {
      snprintf(vector_type, sizeof(vector_type), "%s_%s_vec_t", owner, name);
      snprintf(declaration, sizeof(declaration), "%s %s;", vector_type, c_name);
      databind_compiler_set_string(field, "typed_kind",
  ...
  kind = databind_compiler_typed_named_kind(root, type, c_type, sizeof(c_type), descriptor,
                                       sizeof(descriptor));
  if (!kind) {
  ...
  {
    const databind_compiler_scalar_projection_t *scalar = databind_compiler_scalar_projection(type);
    char symbol[256];
    if (scalar && scalar->native_data_symbol) {
      databind_compiler_set_string(field, "native_data_symbol", scalar->native_data_symbol);
      databind_compiler_set_string(field, "native_type_symbol", scalar->native_type_symbol);
      databind_compiler_set_string(field, "native_external", "1");
      databind_compiler_set_string(field, "native_c_type", c_type);
    } else if (semantic && salts_uuid_cmeta_data_valid(semantic->data)) {
      databind_compiler_set_string(field, "native_data_symbol", "salts_uuid_cmeta_data");
      databind_compiler_set_string(field, "native_type_symbol", "salts_uuid_cmeta_type");
      databind_compiler_set_string(field, "native_external", "1");
      databind_compiler_set_string(field, "native_c_type", c_type);
    } else if (strcmp(kind, "TBE_TYPED_ENUM") == 0) {
      if (databind_compiler_set_enum_symbol(field, "native_data_symbol", type, "Data") == 0 &&
          databind_compiler_set_enum_symbol(field, "native_type_symbol", type, "Type") == 0)
        databind_compiler_set_string(field, "native_c_type", c_type);
  ...
  databind_compiler_set_string(field, "typed_kind", kind);
  databind_compiler_set_string(field, "typed_wire_kind",
                          databind_compiler_typed_wire_kind(root, type, kind));
  if (descriptor[0]) databind_compiler_set_string(field, "typed_object_descriptor", descriptor);
  databind_compiler_set_string(field, "typed_declaration", declaration);
}

static void databind_compiler_annotate_native_requirement(
    Node *root, Node *field, const schema_cmeta_field_type *semantic) {
  const char *type;
  databind_compiler_native_requirement_t requirement;

  if (!root || !field || !semantic) return;
  type = databind_compiler_string_value(field, "type");

  if (cmeta_data_kind_is_container(semantic->kind)) {
    requirement = TBE_COMPILER_NATIVE_DEFERRED_CONTAINER;
  } else if (databind_compiler_has_child(field, "is_optional") &&
             databind_compiler_has_child(field, "is_nullable")) {
    requirement = TBE_COMPILER_NATIVE_OVERLAY_PRESENCE_NULL;
  } else if (databind_compiler_has_child(field, "is_nullable")) {
  ...
  } else if (type && databind_compiler_find_record(root, "enums", type)) {
  ...
  databind_compiler_set_string(
      field, "cmeta_native_requirement",
      databind_compiler_native_requirement_name(requirement));
}

static void databind_compiler_annotate_field_types(Node *root, Node *field) {
  char type_buf[256];
  char field_name[128];
  const char *name = databind_compiler_string_value(field, "name");
  ...
  if (semantic) {
    ...
    databind_compiler_set_string(field, "cmeta_kind", type_buf);
    databind_compiler_set_string(field, "cmeta_schema_kind", semantic->schema_kind);
    if (semantic->data)
      databind_compiler_set_string(field, "cmeta_data_id", semantic->data->stable_id);
  }

  databind_compiler_pascal_identifier(name, field_name, sizeof(field_name));
  databind_compiler_set_string(field, "go_name", field_name);

  databind_compiler_field_type(field, semantic, databind_compiler_cpp_scalar_type,
                          "std::vector<", ">", "std::set<", ">",
                          "std::map<", ", ", ">", type_buf, sizeof(type_buf));
  databind_compiler_set_string(field, "cpp_type", type_buf);

  databind_compiler_field_type(field, semantic, databind_compiler_go_scalar_type,
                          "[]", "", "map[", "]struct{}",
                          "map[", "]", "", type_buf, sizeof(type_buf));
  databind_compiler_set_string(field, "go_type", type_buf);

  databind_compiler_field_type(field, semantic, databind_compiler_ts_scalar_type,
                          "Array<", ">", "Set<", ">",
                          "Map<", ", ", ">", type_buf, sizeof(type_buf));
  databind_compiler_set_string(field, "ts_type", type_buf);

  databind_compiler_field_type(field, semantic, databind_compiler_python_scalar_type,
                          "list[", "]", "set[", "]",
                          "dict[", ", ", "]", type_buf, sizeof(type_buf));
  databind_compiler_set_string(field, "python_type", type_buf);

  databind_compiler_field_type(field, semantic, databind_compiler_rust_scalar_type,
                          "Vec<", ">", "std::collections::HashSet<", ">",
                          "std::collections::HashMap<", ", ", ">", type_buf, sizeof(type_buf));
  databind_compiler_set_string(field, "rust_type", type_buf);

  databind_compiler_field_type(field, semantic, databind_compiler_rfl_scalar_type,
                          "List<", ">", "Set<", ">",
                          "Map<", ", ", ">", type_buf, sizeof(type_buf));
  databind_compiler_set_string(field, "rfl_type", type_buf);
  databind_compiler_annotate_typed_field(root, field, semantic);
  databind_compiler_annotate_native_requirement(root, field, semantic);
}

static void databind_compiler_annotate_record_list_types(Node *root, const char *list_name) {
  Node *list = databind_compiler_find_child(root, list_name);
  if (!list || list->type != NODE_LIST) return;

  for (size_t i = 0; i < list->data.list.count; ++i) {
    Node *record = list->data.list.items[i];
    Node *fields = databind_compiler_find_child(record, "fields");
    if (!fields || fields->type != NODE_LIST) continue;

    for (size_t j = 0; j < fields->data.list.count; ++j) {
      databind_compiler_annotate_field_types(root, fields->data.list.items[j]);
    }
  }
}

static const char *databind_compiler_c_enum_underlying_type(const char *type, int is_flags) {
  const databind_compiler_scalar_projection_t *integer_type;
  const char *fallback = is_flags ? "uint32_t" : "int32_t";

  if (!type || !type[0]) return fallback;
  integer_type = databind_compiler_integer_type(type);
  if (integer_type) return integer_type->c_type;
  return fallback;
}

static void databind_compiler_annotate_enum_types(Node *root) {
  Node *enums = databind_compiler_find_child(root, "enums");
  if (!enums || enums->type != NODE_LIST) return;

  for (size_t i = 0; i < enums->data.list.count; ++i) {
    Node *enum_node = enums->data.list.items[i];
    const char *underlying = databind_compiler_string_value(enum_node, "underlying_type");
    const int is_flags = databind_compiler_has_child(enum_node, "is_flags");
    const databind_compiler_scalar_projection_t *storage = databind_compiler_integer_type(underlying);

    if (databind_compiler_set_enum_symbol(enum_node, "native_enum_symbol",
          databind_compiler_string_value(enum_node, "enum_name"), "") != 0)
      continue;
    if (storage) {
      char bits[4];
      snprintf(bits, sizeof(bits), "%u",
               ((const cmeta_data_integer_shape *)storage->data->shape)->bits);
      databind_compiler_set_string(enum_node, "native_enum_bits", bits);
      databind_compiler_set_string(enum_node, "native_enum_signedness",
          storage->data->kind == CMETA_DATA_SINT ? "CMETA_ENUM_SIGNED"
                                                 : "CMETA_ENUM_UNSIGNED");
      if (storage->data->kind == CMETA_DATA_SINT)
        databind_compiler_set_string(enum_node, "native_enum_signed", "1");
      databind_compiler_set_string(enum_node, "native_enum_supported", "1");
      databind_compiler_set_string(enum_node, "typed_cmeta_runtime_supported", "1");
    }

    databind_compiler_set_string(enum_node, "go_underlying_type",
                            databind_compiler_go_scalar_type(underlying));
    databind_compiler_set_string(enum_node, "cpp_underlying_type",
                            databind_compiler_cpp_enum_underlying_type(underlying));
    databind_compiler_set_string(enum_node, "rust_underlying_type",
                            databind_compiler_rust_enum_underlying_type(underlying));
    databind_compiler_set_string(enum_node, "rfl_underlying_type",
                            underlying && strcmp(underlying, "uint32") == 0 ? "long" :
                                databind_compiler_rfl_scalar_type(underlying));
    databind_compiler_set_string(enum_node, "c_underlying_type",
                            databind_compiler_c_enum_underlying_type(underlying, is_flags));
  }
}
...
typedef struct databind_compiler_cmeta_classify_context_s {
  Node *root;
  Node **records;
  ...
  size_t count;
  int runtime;
} databind_compiler_cmeta_classify_context_t;

static size_t databind_compiler_cmeta_record_index(
    const databind_compiler_cmeta_classify_context_t *context, const Node *record) {
  size_t i;
...
static Node *databind_compiler_find_any_record(Node *root, const char *name) {
  Node *record = databind_compiler_find_record(root, "composites", name);
  if (!record) record = databind_compiler_find_record(root, "groups", name);
  if (!record) record = databind_compiler_find_record(root, "messages", name);
  return record;
}

static int databind_compiler_cmeta_classify_record(
    databind_compiler_cmeta_classify_context_t *context, size_t index) {
  Node *record;
  Node *fields;
  size_t max_depth = 0;
  size_t i;
  ...
  fields = databind_compiler_find_child(record, "fields");
  if (!fields || fields->type != NODE_LIST) goto unsupported;

  for (i = 0; i < fields->data.list.count; ++i) {
    Node *field = fields->data.list.items[i];
    const char *type = databind_compiler_string_value(field, "type");
    const char *kind = databind_compiler_string_value(field, "typed_kind");
    const databind_compiler_scalar_projection_t *scalar;
    Node *target;
    size_t target_index;

    if (!type || !kind ||
        (context->runtime &&
         (databind_compiler_has_child(field, "is_optional") ||
          databind_compiler_has_child(field, "is_nullable"))) ||
        databind_compiler_has_child(field, "is_collection") ||
        databind_compiler_has_child(field, "is_list") ||
        databind_compiler_has_child(field, "is_set") ||
        databind_compiler_has_child(field, "is_map") ||
        databind_compiler_has_child(field, "is_group_field"))
      goto unsupported;

    scalar = databind_compiler_scalar_projection(type);
    if (scalar) {
    ...
    if (context->runtime &&
        databind_compiler_string_value(field, "cmeta_native_requirement") != NULL &&
        strcmp(databind_compiler_string_value(field, "cmeta_native_requirement"),
               "fixed_value") == 0 &&
        databind_compiler_string_value(field, "native_data_symbol") != NULL &&
        databind_compiler_string_value(field, "native_type_symbol") != NULL &&
        (strcmp(kind, "TBE_TYPED_UUID") == 0 ||
         strcmp(kind, "TBE_TYPED_FIXED_BYTES") == 0))
      continue;

    target = databind_compiler_find_record(context->root, "enums", type);
    if (target) {
      const char *marker = context->runtime ? "typed_cmeta_runtime_supported"
                                            : "native_enum_supported";
      if (!databind_compiler_has_child(target, marker))
        goto unsupported;
      continue;
    }

    target = databind_compiler_find_any_record(context->root, type);
    if (target) {
      target_index = databind_compiler_cmeta_record_index(context, target);
      if (target_index == SIZE_MAX ||
          !databind_compiler_cmeta_classify_record(context, target_index) ||
  ...
  if (databind_compiler_set_string(
  ...
  if (context->runtime) {
    for (i = 0; i < fields->data.list.count; ++i) {
      Node *field = fields->data.list.items[i];
      const char *type = databind_compiler_string_value(field, "type");
      Node *target = databind_compiler_find_any_record(context->root, type);
      if (databind_compiler_set_string(field, "typed_cmeta_runtime_supported", "1") != 0)
        goto unsupported;
      if (target) {
        const char *overlay = databind_compiler_string_value(field, "typed_object_descriptor");
        if (!overlay ||
            databind_compiler_set_string(field, "typed_nested_overlay", overlay) != 0)
          goto unsupported;
      }
    }
  }
...
unsupported:
  databind_compiler_remove_children(
      record, context->runtime ? "typed_cmeta_runtime_supported"
                               : "cmeta_graph_supported");
...
static void databind_compiler_collect_cmeta_records(
    databind_compiler_cmeta_classify_context_t *context, const char *list_name,
    size_t *offset) {
  Node *list = databind_compiler_find_child(context->root, list_name);
  size_t i;
  if (!list || list->type != NODE_LIST) return;
  for (i = 0; i < list->data.list.count; ++i) {
    Node *record = list->data.list.items[i];
    ...
    if (context->runtime && fields && fields->type == NODE_LIST)
      for (j = 0; j < fields->data.list.count; ++j) {
        databind_compiler_remove_children(fields->data.list.items[j],
...
static void databind_compiler_annotate_cmeta_support(Node *root, int runtime) {
  static const char *const lists[] = {"composites", "groups", "messages"};
  databind_compiler_cmeta_classify_context_t context = {0};
  size_t i;
  size_t offset = 0;
  ...
  for (i = 0; i < sizeof(lists) / sizeof(lists[0]); ++i) {
    Node *list = databind_compiler_find_child(root, lists[i]);
  ...
  for (i = 0; i < sizeof(lists) / sizeof(lists[0]); ++i)
    databind_compiler_collect_cmeta_records(&context, lists[i], &offset);
  for (i = 0; i < context.count; ++i)
    (void)databind_compiler_cmeta_classify_record(&context, i);
...
static void databind_compiler_annotate_schema_types(Node *root) {
  Node *schema = databind_compiler_find_child(root, "schema");
  ...
  if (!schema) {
    ...
    databind_compiler_set_string(schema, "schema_name", "GeneratedSchema");
    databind_compiler_set_string(schema, "schema_attributes_rendered", "");
    databind_compiler_set_string(schema, "schema_wire_big_endian_value", "0");
  }

  schema_name = databind_compiler_string_value(schema, "schema_name");

  if (!schema_name || !schema_name[0]) {
    databind_compiler_set_string(schema, "go_package_name", "generated");
    return;
  }
  ...
  databind_compiler_set_string(schema, "go_package_name", package_name);
}

void databind_compiler_annotate_language_types(Node *root) {
  databind_compiler_annotate_schema_types(root);
  databind_compiler_annotate_enum_types(root);
  databind_compiler_annotate_record_list_types(root, "composites");
  databind_compiler_annotate_record_list_types(root, "groups");
  databind_compiler_annotate_record_list_types(root, "messages");
  databind_compiler_annotate_record_list_types(root, "unions");
  databind_compiler_annotate_cmeta_support(root, 1);
  databind_compiler_annotate_cmeta_support(root, 0);
}

static const char *databind_compiler_path_basename(const char *path) {
...
static char *databind_compiler_escape_c_string(const char *text) {
...
static int databind_compiler_typed_list_supported(Node *root, const char *list_name) {
  Node *list = databind_compiler_find_child(root, list_name);
  size_t i;
  ...
  for (i = 0; i < list->data.list.count; ++i) {
    ...
    for (j = 0; j < fields->data.list.count; ++j) {
      ...
      const char *c_name = databind_compiler_string_value(field, "c_name");
      size_t k;
      if (!databind_compiler_string_value(field, "typed_kind")) {
        fprintf(stderr, "Typed C serde does not support field %s.%s of type %s\n",
                databind_compiler_string_value(field, "owner_name"),
                databind_compiler_string_value(field, "name"),
                databind_compiler_string_value(field, "type"));
        return 0;
      }
      if (databind_compiler_attribute_count(field, "c") > 1u) {
        fprintf(stderr, "Typed C field %s.%s has multiple c member mappings\n",
                databind_compiler_string_value(field, "owner_name"),
                databind_compiler_string_value(field, "name"));
        return 0;
      }
      if (!databind_compiler_c_identifier_valid(c_name)) {
        fprintf(stderr, "Typed C field %s.%s uses invalid member name %s\n",
                databind_compiler_string_value(field, "owner_name"),
                databind_compiler_string_value(field, "name"), c_name ? c_name : "(null)");
        return 0;
      }
      if (databind_compiler_has_child(field, "is_optional")) {
        const char *bit_text = databind_compiler_string_value(field, "optional_bit_index");
        const char *bitmap_text = databind_compiler_string_value(record, "presence_bitmap_bytes");
        size_t bit;
        size_t bitmap_size;
        if (!databind_compiler_parse_size(bit_text, &bit) ||
            !databind_compiler_parse_size(bitmap_text, &bitmap_size) || bitmap_size == 0u ||
            bit / 8u >= bitmap_size) {
      ...
      if (databind_compiler_has_child(field, "is_nullable")) {
        const char *bit_text = databind_compiler_string_value(field, "nullable_bit_index");
        const char *bitmap_text = databind_compiler_string_value(record, "null_bitmap_bytes");
        size_t bit;
        size_t bitmap_size;
        if (!databind_compiler_parse_size(bit_text, &bit) ||
            !databind_compiler_parse_size(bitmap_text, &bitmap_size) || bitmap_size == 0u ||
      ...
      if (c_name &&
        ...
        fprintf(stderr, "Typed C field %s.%s uses reserved state member name %s\n",
                databind_compiler_string_value(field, "owner_name"),
                databind_compiler_string_value(field, "name"), c_name);
        return 0;
      }
      for (k = j + 1; k < fields->data.list.count; ++k) {
        const char *other_c_name =
            databind_compiler_string_value(fields->data.list.items[k], "c_name");
        if (c_name && other_c_name && strcmp(c_name, other_c_name) == 0) {
          fprintf(stderr, "Typed C fields %s.%s and %s share member name %s\n",
                  databind_compiler_string_value(field, "owner_name"),
                  databind_compiler_string_value(field, "name"),
                  databind_compiler_string_value(fields->data.list.items[k], "name"), c_name);
          return 0;
        }
      }
    }
...
static int databind_compiler_typed_schema_supported(Node *root) {
  Node *unions = databind_compiler_find_child(root, "unions");
  ...
  return databind_compiler_typed_list_supported(root, "composites") &&
         databind_compiler_typed_list_supported(root, "groups") &&
         databind_compiler_typed_list_supported(root, "messages");
}

static Node *databind_compiler_find_typed_record(Node *root, const char *name) {
  Node *record = databind_compiler_find_record(root, "composites", name);
  if (!record) record = databind_compiler_find_record(root, "groups", name);
  if (!record) record = databind_compiler_find_record(root, "messages", name);
  return record;
...
static int databind_compiler_prepare_lua_operations(Node *root) {
  Node *messages = databind_compiler_find_child(root, "messages");
  ...
  for (i = 0; i < messages->data.list.count; ++i) {
    ...
    const size_t operation_name_count =
        databind_compiler_attribute_count(request, "lua_operation");
    const size_t import_name_count =
        databind_compiler_attribute_count(request, "lua_import");
    const size_t response_count = databind_compiler_attribute_count(request, "lua_response");
    const size_t async_count = databind_compiler_attribute_count(request, "lua_async");
    ...
    if (import_name_count != 0u) {
      fprintf(stderr,
              ...
              databind_compiler_string_value(request, "name"));
      return 0;
    }
    if (operation_name_count != 0u && import_name_count != 0u) {
      fprintf(stderr,
    ...
    if ((is_import ? import_name_count : operation_name_count) != 1u ||
      ...
      fprintf(stderr,
              ...
              "attribute\n",
              binding_kind, databind_compiler_string_value(request, "name"),
              name_attribute);
      return 0;
    }
    if (async_count > 1u) {
    ...
    binding_name = databind_compiler_attribute_value(request, name_attribute);
    response_type = databind_compiler_attribute_value(request, "lua_response");
    async_value = databind_compiler_attribute_value(request, "lua_async");
    if (!databind_compiler_c_identifier_valid(binding_name)) {
      fprintf(stderr, "Lua %s %s uses an invalid C/Lua identifier\n", binding_kind,
              binding_name ? binding_name : "(null)");
      return 0;
    }
    if (!databind_compiler_find_typed_record(root, response_type)) {
    ...
    for (previous = 0; previous < i; ++previous) {
      ...
      const char *other_name =
          databind_compiler_attribute_value(other, name_attribute);
    ...
    if (is_import) {
      if (databind_compiler_set_string(request, "lua_import_enabled", "1") != 0 ||
          databind_compiler_set_string(request, "lua_import_name", binding_name) != 0 ||
          databind_compiler_set_string(request, "lua_response_type", response_type) != 0) {
        return 0;
      }
      if (async_count == 1u &&
          (databind_compiler_set_string(request, "lua_async_enabled", "1") != 0 ||
           databind_compiler_set_string(root, "has_lua_async_imports", "1") != 0)) {
        return 0;
      }
      ++import_count;
    } else {
      if (databind_compiler_set_string(request, "lua_operation_enabled", "1") != 0 ||
          databind_compiler_set_string(request, "lua_operation_name", binding_name) != 0 ||
          databind_compiler_set_string(request, "lua_response_type", response_type) != 0) {
        return 0;
      }
      if (async_count == 1u &&
          (databind_compiler_set_string(request, "lua_async_enabled", "1") != 0 ||
           databind_compiler_set_string(root, "has_lua_async_operations", "1") != 0)) {
        return 0;
      }
      ++operation_count;
    }
  ...
  if (operation_count != 0u &&
      databind_compiler_set_string(root, "has_lua_operations", "1") != 0) {
    return 0;
  }
  if (import_count != 0u &&
      databind_compiler_set_string(root, "has_lua_imports", "1") != 0) {
    return 0;
  }
  if ((operation_count != 0u || import_count != 0u) &&
      databind_compiler_set_string(root, "has_lua_bindings", "1") != 0) {
    return 0;
  }
  return 1;
}
...
static const char *databind_compiler_language_name(int64_t lang_enum) {
  switch (lang_enum) {
...
static const char *databind_compiler_resolve_resource(const databind_compiler_options_t *options,
...
static int databind_compiler_is_database_language(int64_t lang_enum) {
  return lang_enum == DATABIND_COMPILER_LANG_SQLITE ||
         lang_enum == DATABIND_COMPILER_LANG_POSTGRESQL;
}

static int databind_compiler_validate_database_output_option(const char *lang_name,
...
static int databind_compiler_validate_options(const databind_compiler_options_t *options,
                                         const char *lang_name) {
  if (options == NULL || lang_name == NULL) return 0;
  if (!databind_compiler_is_database_language(options->lang_enum)) return 1;

  if (options->output_path == NULL || options->output_path[0] == '\0') {
    fprintf(stderr, "--lang %s requires explicit --output\n", lang_name);
    return 0;
  }
  if (!databind_compiler_validate_database_output_option(
          lang_name, "--source-output", options->source_output_path) ||
      !databind_compiler_validate_database_output_option(
...
int databind_compiler_parse_schema_file(const char *schema_path, Node **out_root,
  ...
  Node *root = NULL;
  char *schema_data = databind_compiler_read_file(schema_path);
  if (!schema_data) {
...
int databind_compiler_render_file(Node *root, const char *template_path,
  ...
  char *templ_data = databind_compiler_read_file(template_path);
...
#endif
    if (databind_compiler_create_temporary_output(output_path, &temporary_output_path, &out_file) != 0) {
...
static int databind_compiler_validate_enum_backend(Node *root,
                                             const databind_compiler_options_t *options) {
  Node *enums = databind_compiler_find_child(root, "enums");
  if (!enums || enums->type != NODE_LIST) return 1;
  for (size_t i = 0; i < enums->data.list.count; ++i) {
    Node *node = enums->data.list.items[i];
    const char *storage = databind_compiler_string_value(node, "underlying_type");
    const char *name = databind_compiler_string_value(node, "enum_name");
    if (!storage || !databind_compiler_integer_type(storage)) {
    ...
    if (options->dsl_output_path && !databind_compiler_has_child(node, "is_ordinal")) {
...
static Node *databind_compiler_clone_canonical_node(const Node *node) {
  Node *copy = NULL;
  size_t i;
  ...
  case NODE_LIST:
    ...
    for (i = 0u; i < node->data.list.count; ++i) {
      Node *child =
          databind_compiler_clone_canonical_node(node->data.list.items[i]);
  ...
  case NODE_MAP:
    ...
    for (i = 0u; i < node->data.map.count; ++i) {
      Node *child =
          databind_compiler_clone_canonical_node(node->data.map.items[i]);
...
int databind_compiler_run(const databind_compiler_options_t *options) {
  ...
  const char *lang_name = databind_compiler_language_name(options->lang_enum);
  ...
  database_language = databind_compiler_is_database_language(options->lang_enum);
  if (!databind_compiler_validate_options(options, lang_name)) return 1;
  int status = databind_compiler_parse_schema_file(options->schema_path, &root,
                                              &schema_data);
  ...
  if (options->projection_count != 0u) {
    projection_root = databind_compiler_clone_canonical_node(root);
  ...
  if (!databind_compiler_validate_enum_backend(root, options)) {
    status = 1;
    goto cleanup;
  }
  ...
  if (database_language) {
    ...
    if (databind_compiler_set_string(root, "generated_header",
                                databind_compiler_path_basename(options->output_path)) != 0) {
      status = 1;
      goto cleanup;
    }
    {
      char *schema_literal = databind_compiler_escape_c_string(schema_data);
      if (!schema_literal || databind_compiler_set_string(root, "schema_c_literal", schema_literal) != 0) {
        free(schema_literal);
        status = 1;
        goto cleanup;
      }
  ...
  if (options->source_output_path) {
    ...
    if (!databind_compiler_typed_schema_supported(root)) {
      status = 1;
      goto cleanup;
    }
    if (databind_compiler_set_string(root, "typed_source_enabled", "1") != 0) {
      status = 1;
      goto cleanup;
    }
  }
  ...
  if (options->guest_output_path) {
    ...
    if (databind_compiler_set_string(root, "guest_adapter_enabled", "1") != 0) {
      status = 1;
      goto cleanup;
    }
    if (options->source_output_path == NULL &&
        databind_compiler_set_string(root, "guest_adapter_only", "1") != 0) {
      status = 1;
      goto cleanup;
    }
  }
  ...
  if (options->lua_output_path) {
    ...
    if (databind_compiler_set_string(root, "lua_output_enabled", "1") != 0) {
      status = 1;
      goto cleanup;
    }
    if (!databind_compiler_prepare_lua_operations(root)) {
      status = 1;
      goto cleanup;
    }
  }
  ...
  if (resolved_template == NULL) {
    resolved_template = databind_compiler_resolve_resource(
        options, databind_compiler_resolve_template(NULL, options->lang_enum), template_path,
  ...
  status = databind_compiler_render_file(database_language ? database_ir : root,
  ...
  if (status == 0 && options->source_output_path) {
    ...
    status = resolved_template != NULL
                 ? databind_compiler_render_file(root, resolved_template, options->source_output_path)
                 : 1;
  }
  ...
  if (status == 0 && options->guest_output_path) {
    ...
    status = resolved_template != NULL
                 ? databind_compiler_render_file(root, resolved_template, options->guest_output_path)
                 : 1;
  }
  ...
  if (status == 0 && options->lua_output_path) {
    ...
    status = resolved_template != NULL
                 ? databind_compiler_render_file(root, resolved_template, options->lua_output_path)
                 : 1;
  }
  ...
  if (status == 0 && options->dsl_output_path) {
    ...
    if (resolved_template == NULL ||
        databind_compiler_render_file(root, resolved_template, options->dsl_output_path) != 0) {
      status = 1;
    }
  }
