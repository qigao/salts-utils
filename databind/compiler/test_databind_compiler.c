...
static char *render_compiler_template_from_schema(const char *schema,
  ...
  if (data_bind_compiler_parse_schema_file(schema_path, &root, &schema_data) != 0) goto cleanup;
  if (data_bind_compiler_render_file(root, template_file, output_path) != 0) goto cleanup;
...
static char *run_compiler_capture_stderr(const data_bind_compiler_options_t *options,
                                         int *out_status) {
  int saved_stderr = -1;
  ...
  stderr_path = tt_make_temp_file("data_bind_compiler_stderr", ".log");
  ...
  if (out_status) {
    *out_status = data_bind_compiler_run(options);
  } else {
    (void)data_bind_compiler_run(options);
  }
  fflush(stderr);
...
static void compiler_assign_conflicting_output(data_bind_compiler_options_t *options,
                                               compiler_conflicting_output_kind_t kind,
                                               const char *path) {
  if (!options) return;
  switch (kind) {
...
static void check_database_language_conflicting_output_fails_fast(
  ...
  snprintf(output_path, sizeof(output_path), "test_data_bind_compiler_%s_conflict_output.sql",
           language_name);
  snprintf(conflicting_output_path, sizeof(conflicting_output_path),
           "test_data_bind_compiler_%s_conflict_%s.out", language_name,
...
static void check_database_language_empty_conflicting_output_fails_fast(
  ...
  snprintf(output_path, sizeof(output_path),
           "test_data_bind_compiler_%s_empty_conflict_output.sql", language_name);
  cleanup_test_file(output_path);
...
spec("tbe_compiler") {
  ...
  describe("C template rendering") {
    ...
    it("should parse CLI language names with stable enum values") {
      ...
      check_equal(data_bind_compiler_parse_language_name("cxx", &lang_enum), 0);
      check_equal(lang_enum, DATABIND_COMPILER_LANG_CPP);
      check_equal(data_bind_compiler_parse_language_name("py", &lang_enum), 0);
      check_equal(lang_enum, DATABIND_COMPILER_LANG_PYTHON);
      check_equal(data_bind_compiler_parse_language_name("typescript", &lang_enum), 0);
      check_equal(lang_enum, DATABIND_COMPILER_LANG_TS);
      check_equal(data_bind_compiler_parse_language_name("sqlite", &lang_enum), 0);
      check_equal(lang_enum, DATABIND_COMPILER_LANG_SQLITE);
      check_equal(data_bind_compiler_parse_language_name("postgresql", &lang_enum), 0);
      check_equal(lang_enum, DATABIND_COMPILER_LANG_POSTGRESQL);
      check_equal(data_bind_compiler_parse_language_name("postgres", &lang_enum), 0);
      check_equal(lang_enum, DATABIND_COMPILER_LANG_POSTGRESQL);
    }

    it("should resolve built-in templates through compiler core") {
      check_equal(data_bind_compiler_resolve_template(NULL, 0), "templates/c_structs.mustache");
      check_equal(data_bind_compiler_resolve_template(NULL, 1),
                   "templates/python_dataclass.mustache");
      check_equal(data_bind_compiler_resolve_template(NULL, 2), "templates/rust_structs.mustache");
      check_equal(data_bind_compiler_resolve_template(NULL, DATABIND_COMPILER_LANG_CPP),
                   "templates/cpp_types.mustache");
      check_equal(data_bind_compiler_resolve_template(NULL, DATABIND_COMPILER_LANG_GO),
                   "templates/go_types.mustache");
      check_equal(data_bind_compiler_resolve_template(NULL, DATABIND_COMPILER_LANG_TS),
                   "templates/ts_types.mustache");
      check_equal(data_bind_compiler_resolve_template(NULL, DATABIND_COMPILER_LANG_SQLITE),
                  "templates/sqlite_schema.mustache");
      check_equal(data_bind_compiler_resolve_template(NULL, DATABIND_COMPILER_LANG_POSTGRESQL),
                  "templates/postgresql_schema.mustache");
      check_equal(data_bind_compiler_resolve_template("custom.mustache", 0), "custom.mustache");
    }
    ...
    it("should parse schema files through compiler core") {
      ...
      check_equal(data_bind_compiler_parse_schema_file(SCHEMA_EXAMPLE_FILE, &root, &schema_data), 0);
      check_not_null(root);
    ...
    it("should render template output through compiler core") {
      const char *output_path = "test_data_bind_compiler_render.out";
      size_t output_size = 0;
      Node *root = NULL;
      char *schema_data = NULL;
      char *output = NULL;
      ...
      cleanup_test_file(output_path);
      check_equal(data_bind_compiler_parse_schema_file(SCHEMA_EXAMPLE_FILE, &root, &schema_data), 0);
      check_equal(data_bind_compiler_render_file(root, C_STRUCT_TEMPLATE_FILE, output_path), 0);
    ...
    it("should run compiler core end-to-end with custom template") {
      const char *output_path = "test_data_bind_compiler_run.out";
      ...
      cleanup_test_file(output_path);
      check_equal(data_bind_compiler_run(&options), 0);
    ...
    it("should render deterministic SQLite bootstrap DDL") {
      const char *schema_path = "test_data_bind_compiler_sqlite.schema";
      const char *output_path = "test_data_bind_compiler_sqlite.sql";
      const char *schema =
    ...
    it("should render deterministic PostgreSQL bootstrap DDL") {
      const char *schema_path = "test_data_bind_compiler_postgresql.schema";
      const char *output_path = "test_data_bind_compiler_postgresql.sql";
      const char *schema =
          "[db_table(\"User Records\")] message User {"
          " [db_column(\"User Id\"), db_primary_key(1), db_generated(identity)] int64 id;"
          " optional uint32 login_count default 0;"
    ...
    it("should render deterministic database relationships indexes checks and seed inserts") {
      const char *schema_path = "test_data_bind_compiler_database_v2.schema";
      const char *output_paths[] = {
          "test_data_bind_compiler_database_v2.sqlite.sql",
          "test_data_bind_compiler_database_v2.postgresql.sql"};
      ...
      for (size_t dialect_index = 0; dialect_index < 2u; ++dialect_index) {
        ...
        check_not_null(output);
        if (output) {
          check(output_size >= 15u);
          check_equal(memcmp(output, "BEGIN;\n", 7u), 0);
          check_equal(memcmp(output + output_size - 8u, "COMMIT;\n", 8u), 0);
    ...
    it("should render parsed numeric and injection-shaped PostgreSQL defaults") {
      ...
      const char *output_path = "test_data_bind_compiler_postgresql_defaults.sql";
      const char *expected =
          "BEGIN;\n"
    ...
    it("should render a non-identity single primary key for SQLite") {
      const char *schema_path = "test_data_bind_compiler_sqlite_single_pk.schema";
      const char *output_path = "test_data_bind_compiler_sqlite_single_pk.sql";
      const char *schema =
    ...
    it("should render a non-identity single primary key for PostgreSQL") {
      const char *schema_path = "test_data_bind_compiler_postgresql_single_pk.schema";
      const char *output_path = "test_data_bind_compiler_postgresql_single_pk.sql";
      const char *schema =
          "[db_table(single_pk)] message SinglePk {"
          " [db_primary_key(1)] int64 id;"
          " string value;"
    ...
    it("should pass normalized database IR to custom database templates") {
      const char *schema_path = "test_data_bind_compiler_database_custom.schema";
      const char *template_path = "test_data_bind_compiler_database_custom.mustache";
      const char *output_path = "test_data_bind_compiler_database_custom.out";
      const char *schema =
    ...
    it("should expose relationship index check and initializer IR to custom templates") {
      const char *schema_path = "test_data_bind_compiler_database_v2_custom.schema";
      const char *template_path = "test_data_bind_compiler_database_v2_custom.mustache";
      const char *output_path = "test_data_bind_compiler_database_v2_custom.out";
      const char *schema =
    ...
    it("should expose scoped database template markers without is_last") {
      const char *schema_path = "test_data_bind_compiler_database_markers.schema";
      const char *template_path = "test_data_bind_compiler_database_markers.mustache";
      const char *output_path = "test_data_bind_compiler_database_markers.out";
      const char *schema =
...
#ifndef _WIN32
      const char *schema_path = "test_data_bind_compiler_posix_mode.schema";
      const char *output_path = "test_data_bind_compiler_posix_mode.sql";
      const char *schema =
    ...
    it("should preserve existing POSIX output permissions when overwriting") {
      ...
      const char *schema_path = "test_data_bind_compiler_posix_overwrite_mode.schema";
      const char *output_path = "test_data_bind_compiler_posix_overwrite_mode.sql";
      const char *schema =
          "[db_table(records)] message Record { [db_primary_key(1)] int64 id; }";
      data_bind_compiler_options_t options = {
          .schema_path = schema_path,
    ...
    it("should preserve a similarly named pre-existing temporary file") {
      const char *schema_path = "test_data_bind_compiler_temp_owner.schema";
      const char *output_path = "test_data_bind_compiler_temp_owner.sql";
      const char *old_temp_path = "test_data_bind_compiler_temp_owner.sql.tbe.tmp";
      const char *schema =
          "[db_table(records)] message Record { [db_primary_key(1)] int64 id; }";
      const char *expected =
          "BEGIN;\n"
    ...
    it("should preserve an existing output when template compilation fails") {
      const char *template_path = "test_data_bind_compiler_invalid_template.mustache";
      const char *output_path = "test_data_bind_compiler_preserved_output.out";
      Node *root = create_node_map(NULL);
      char *output = NULL;
      size_t output_size = 0;
      ...
      check_equal(write_test_file(output_path, "known-good-output"), 0);
      if (root) check_not_equal(data_bind_compiler_render_file(root, template_path, output_path), 0);
      output = tt_read_file(output_path, &output_size);
      check_not_null(output);
      if (output) {
    ...
    it("should preserve an existing output when replacement fails") {
#ifdef _WIN32
      const char *schema_path = "test_data_bind_compiler_rename_failure.schema";
      const char *output_path = "test_data_bind_compiler_rename_failure.sql";
      const char *schema =
          "[db_table(records)] message Record { [db_primary_key(1)] int64 id; }";
      data_bind_compiler_options_t options = {
          .schema_path = schema_path,
      ...
      if (output_lock != INVALID_HANDLE_VALUE) {
        check_not_equal(data_bind_compiler_run(&options), 0);
    ...
    it("should generate RulesForge type declarations with the built-in DSL template") {
      const char *schema_path = "test_data_bind_compiler_rfl.schema";
      const char *header_path = "test_data_bind_compiler_rfl.h";
      const char *dsl_path = "test_data_bind_compiler_rfl.rfl";
      const char *schema =
          "schema Market;"
    ...
    it("should generate a Wasm guest bridge adapter with the built-in C generator") {
      const char *header_path = "test_data_bind_compiler_guest.h";
      const char *guest_path = "test_data_bind_compiler_guest.c";
    ...
    it("should honor c field annotations in typed C output") {
      const char *schema_path = "test_data_bind_compiler_c_name.tbe";
      const char *header_path = "test_data_bind_compiler_c_name.h";
      const char *source_path = "test_data_bind_compiler_c_name.c";
      const char *schema =
          "schema Annotated;"
    ...
    it("should expose public descriptors for canonical runtime records") {
      const char *schema_path = "test_data_bind_compiler_cmeta_descriptor.tbe";
      const char *header_path = "test_data_bind_compiler_cmeta_descriptor.h";
      const char *source_path = "test_data_bind_compiler_cmeta_descriptor.c";
      const char *schema =
    ...
    it("should generate typed C to Lua adapters") {
      const char *header_path = "test_data_bind_compiler_lua.h";
      const char *source_path = "test_data_bind_compiler_lua_typed.c";
      const char *lua_path = "test_data_bind_compiler_lua.c";
      size_t header_size = 0;
      size_t lua_size = 0;
      char *header = NULL;
    ...
    it("should route supported Lua records through their public descriptor") {
      const char *schema_path = "test_data_bind_compiler_lua_descriptor.tbe";
      const char *header_path = "test_data_bind_compiler_lua_descriptor.h";
      const char *source_path = "test_data_bind_compiler_lua_descriptor_typed.c";
      const char *lua_path = "test_data_bind_compiler_lua_descriptor.c";
      const char *schema =
    ...
    it("should require typed metadata for Lua adapter output") {
      const char *header_path = "test_data_bind_compiler_lua_invalid.h";
      const char *lua_path = "test_data_bind_compiler_lua_invalid.c";
      data_bind_compiler_options_t options = {
          .schema_path = SCHEMA_EXAMPLE_FILE,
          .template_path = NULL,
          .output_path = header_path,
      ...
      cleanup_test_file(header_path);
      cleanup_test_file(lua_path);
      check(data_bind_compiler_run(&options) != 0);
      cleanup_test_file(header_path);
      cleanup_test_file(lua_path);
    }

    it("should generate schema-declared Lua operation callbacks") {
      const char *schema_path = "test_data_bind_compiler_lua_operation.tbe";
      const char *header_path = "test_data_bind_compiler_lua_operation.h";
      const char *source_path = "test_data_bind_compiler_lua_operation_typed.c";
      const char *lua_path = "test_data_bind_compiler_lua_operation.c";
      const char *schema =
          "schema Orders;"
    ...
    it("should reject removed synchronous Lua imports") {
      const char *schema_path = "test_data_bind_compiler_lua_import.tbe";
      const char *header_path = "test_data_bind_compiler_lua_import.h";
      const char *source_path = "test_data_bind_compiler_lua_import_typed.c";
      const char *lua_path = "test_data_bind_compiler_lua_import.c";
      const char *schema =
          "schema Rules;"
    ...
    it("should generate Future operations without C coroutine dependencies") {
      const char *schema_path = "test_data_bind_compiler_lua_future.tbe";
      const char *header_path = "test_data_bind_compiler_lua_future.h";
      const char *source_path = "test_data_bind_compiler_lua_future_typed.c";
      const char *lua_path = "test_data_bind_compiler_lua_future.c";
      const char *schema =
          "schema Stateful;"
    ...
    it("should reject the removed salts_coro async interface") {
      const char *schema_path = "test_data_bind_compiler_lua_async_invalid.tbe";
      const char *header_path = "test_data_bind_compiler_lua_async_invalid.h";
      const char *source_path = "test_data_bind_compiler_lua_async_invalid_typed.c";
      const char *lua_path = "test_data_bind_compiler_lua_async_invalid.c";
      const char *schema =
          "schema Orders;"
    ...
    it("should reject removed asynchronous Lua imports") {
      const char *schema_path = "test_data_bind_compiler_lua_import_async.tbe";
      const char *header_path = "test_data_bind_compiler_lua_import_async.h";
      const char *source_path = "test_data_bind_compiler_lua_import_async_typed.c";
      const char *lua_path = "test_data_bind_compiler_lua_import_async.c";
      const char *schema =
          "schema Orders;"
    ...
    it("should reject an unknown Lua operation response type") {
      const char *schema_path = "test_data_bind_compiler_lua_operation_invalid.tbe";
      const char *header_path = "test_data_bind_compiler_lua_operation_invalid.h";
      const char *source_path = "test_data_bind_compiler_lua_operation_invalid_typed.c";
      const char *lua_path = "test_data_bind_compiler_lua_operation_invalid.c";
      const char *schema =
          "schema Orders;"
    ...
    it("should reject duplicate c field annotations in typed C output") {
      const char *schema_path = "test_data_bind_compiler_c_collision.tbe";
      const char *header_path = "test_data_bind_compiler_c_collision.h";
      const char *source_path = "test_data_bind_compiler_c_collision.c";
      const char *schema =
          "schema Annotated;"
    ...
    it("should reject invalid c identifiers in typed C output") {
      const char *schema_path = "test_data_bind_compiler_c_invalid.tbe";
      const char *header_path = "test_data_bind_compiler_c_invalid.h";
      const char *source_path = "test_data_bind_compiler_c_invalid.c";
      const char *schema =
          "schema Annotated;"
    ...
    it("should reject guest adapter output outside the built-in C generator") {
      const char *output_path = "test_data_bind_compiler_guest_invalid.out";
      data_bind_compiler_options_t options = {
          .schema_path = SCHEMA_EXAMPLE_FILE,
          .template_path = C_STRUCT_TEMPLATE_FILE,
          .output_path = output_path,
          .guest_output_path = "test_data_bind_compiler_guest_invalid.c",
          .lang_enum = DATABIND_COMPILER_LANG_C,
      };
    ...
    it("should render C++ Go Python Rust and TypeScript type outputs") {
      ...
      char *cpp_output = render_compiler_template_from_schema(
          schema, "test_data_bind_compiler_lang.schema", CPP_TYPES_TEMPLATE_FILE,
          "test_data_bind_compiler_lang.cpp.out");
      char *go_output = render_compiler_template_from_schema(
          schema, "test_data_bind_compiler_lang.schema", GO_TYPES_TEMPLATE_FILE,
          "test_data_bind_compiler_lang.go.out");
      char *py_output = render_compiler_template_from_schema(
          schema, "test_data_bind_compiler_lang.schema", PYTHON_DATACLASS_TEMPLATE_FILE,
          "test_data_bind_compiler_lang.py.out");
      char *rust_output = render_compiler_template_from_schema(
          schema, "test_data_bind_compiler_lang.schema", RUST_STRUCTS_TEMPLATE_FILE,
          "test_data_bind_compiler_lang.rs.out");
      char *ts_output = render_compiler_template_from_schema(
          schema, "test_data_bind_compiler_lang.schema", TS_TYPES_TEMPLATE_FILE,
          "test_data_bind_compiler_lang.ts.out");
