...
static qvm_limits_t cxml_xp_qvm_limits;
static qvm_diagnostic_t cxml_xp_qvm_diagnostic;
static qvm_status_t cxml_xp_qvm_status;

static void cxml_xp_visit_QVM(cxml_xp_astnode* root)
{
    cxml_xp_qvm_program_t program = { 0 };
    uint16_t next_register = 1;
    qvm_diagnostic_t error;
    qvm_value_t result = { 0 };
    if (cxml_xp_qvm_status != QVM_STATUS_OK) {
        _cxml_xp_data* data = _cxml_xp_new_data();
        data->type = CXML_XP_DATA_NODESET;
        cxml_set_init(&data->nodeset);
        _cxml_xp__e_push(data);
    ...
    if (!cxml_xp_qvm_compile(&program, root, 0, &next_register)) {
        cxml_xp_qvm_status = QVM_STATUS_UNSUPPORTED;
        cxml_xp_qvm_diagnostic.status = QVM_STATUS_UNSUPPORTED;
        cxml_xp_qvm_diagnostic.instruction = QVM_NO_INSTRUCTION;
        cxml_xp_qvm_diagnostic.opcode = QVM_NO_OPCODE;
        cxml_xp_qvm_diagnostic.operand = QVM_NO_OPERAND;
        cxml_xp_qvm_diagnostic.message = "XPath expression cannot be lowered to query VM";
    } else {
        cxml_xp_qvm_status = (qvm_status_t)qvm_verify_slice_ex(
            program.instructions, program.instruction_count, 0,
            program.instruction_count, next_register, program.operand_count, 0,
            &cxml_xp_qvm_limits, &error);
        if (cxml_xp_qvm_status == QVM_STATUS_OK)
            cxml_xp_qvm_status = (qvm_status_t)qvm_execute_ex(
                program.instructions, program.instruction_count, 0,
                program.instruction_count, &cxml_xp_qvm_ops, &program, NULL,
                &result, &cxml_xp_qvm_limits, &error);
        if (cxml_xp_qvm_status != QVM_STATUS_OK)
            cxml_xp_qvm_diagnostic = error;
    }
    if (cxml_xp_qvm_status != QVM_STATUS_OK) {
        _cxml_xp_data* data = _cxml_xp_new_data();
        data->type = CXML_XP_DATA_NODESET;
...
cxml_set * cxml_xpath(void * root, const char *expr) {
    ...
    if (cxml_xpath_ex(root, expr, &nodeset, NULL, NULL) != QVM_STATUS_OK)
        return NULL;
    return nodeset;
}
...
int cxml_xpath_ex(void *root, const char *expr, cxml_set **out,
    ...
    cxml_set *nodeset;
    if (out) *out = NULL;
    if (!root || !expr || !out) return QVM_STATUS_INVALID_ARGUMENT;
    cxml_xp_qvm_limits = limits ? *limits : qvm_default_limits();
    memset(&cxml_xp_qvm_diagnostic, 0, sizeof(cxml_xp_qvm_diagnostic));
    cxml_xp_qvm_diagnostic.status = QVM_STATUS_OK;
    cxml_xp_qvm_diagnostic.instruction = QVM_NO_INSTRUCTION;
    cxml_xp_qvm_diagnostic.opcode = QVM_NO_OPCODE;
    cxml_xp_qvm_diagnostic.operand = QVM_NO_OPERAND;
