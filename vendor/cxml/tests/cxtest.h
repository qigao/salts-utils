...
typedef struct{
    ...
    void (*test_setup)();
    void (*test_teardown)();
    cxml_test_fn tests[CXML_T_SUITE_TEST_THRESHOLD];
}cxml_t_suite;
...
static void _cxml_t_add_test(cxml_t_suite *suite, cxml_test_fn test_fn) {
...
static void _cxml_t_add_mtest(cxml_t_suite *suite, int n, ...) {
    ...
    va_start(ap, n);
    for (int i=0; i<n; i++){
        suite->tests[suite->count] = va_arg(ap, cxml_test_fn);
        suite->count++;
    }
    va_end(ap);
}
...
static void _cxml_t_shuffle(cxml_test_fn arr[], int count){
