 ...
 */
typedef struct salts_cron_expr_s {
...
typedef struct salts_cron_entry_s {
  salts_cron_expr_t expr;
  char *payload;
} salts_cron_entry_t;

typedef struct salts_cron_table_s {
  salts_cron_entry_t *entries;
  size_t count;
  size_t capacity;
} salts_cron_table_t;

typedef struct salts_cron_runner_s salts_cron_runner_t;
 ...
 * result is written, the function returns `SALTS_CRON_ENEXT`. If at least one
