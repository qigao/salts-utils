...
Package version `2.1.0` is built against and depends on `Salts.Native 1.3.0`.
...
Consumers restore `SaltsUtils.Native 2.1.0` and its exact `Salts.Native 1.3.0` dependency,
then set `SALTS_ROOT` to the matching Salts SDK and point CMake at the matching
SaltsUtils SDK:

    export SALTS_ROOT=<nuget>/salts.native/1.3.0/sdk/linux-x64
    export SALTS_UTILS_ROOT=<nuget>/saltsutils.native/2.1.0/sdk/linux-x64
