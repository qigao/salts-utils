...
struct turbo_selector_program_s {
...
typedef struct selector_eval_ctx_s {
  ...
  const turbo_selector_eval_ops_v1_t *ops;
  void *context;
...
static void selector_diagnostic_set(
    turbo_selector_diagnostic_v1_t *diagnostic,
    turbo_selector_status_t status, const char *source, size_t source_size,
    size_t offset, const char *message) {
  size_t index;
  uint32_t line = 1u;
  uint32_t column = 1u;
  if (!diagnostic || diagnostic->size < TURBO_SELECTOR_DIAGNOSTIC_V1_SIZE)
    return;
...
static void selector_diagnostic_clear(
  ...
  if (size < TURBO_SELECTOR_DIAGNOSTIC_V1_SIZE) return;
  memset(diagnostic, 0, sizeof(*diagnostic));
  diagnostic->size = size;
  diagnostic->status = TURBO_SELECTOR_OK;
  diagnostic->line = 1u;
...
void selector_ctx_fail(selector_parse_ctx_t *ctx,
                       ...
                       const char *message) {
  if (!ctx || ctx->status != TURBO_SELECTOR_OK) return;
  ctx->status = status;
...
static selector_semantic_t selector_add_node(selector_parse_ctx_t *ctx,
  ...
  if (!ctx || !node || ctx->status != TURBO_SELECTOR_OK) return result;
  if (ctx->node_count >= TURBO_SELECTOR_MAX_NODES_V1) {
    selector_ctx_fail(ctx, TURBO_SELECTOR_RESOURCE_LIMIT,
...
static int selector_pool_append(selector_parse_ctx_t *ctx, const char *value,
  ...
  if (value_size > TURBO_SELECTOR_MAX_STRING_BYTES_V1) {
    selector_ctx_fail(ctx, TURBO_SELECTOR_RESOURCE_LIMIT, source_offset,
...
static int selector_store_field(selector_parse_ctx_t *ctx,
  ...
  if (!allowed) {
    selector_ctx_fail(ctx, TURBO_SELECTOR_SEMANTIC_ERROR, token.offset,
...
static int selector_store_string(selector_parse_ctx_t *ctx,
                                 selector_semantic_t token,
                                 selector_string_ref_t *out_ref) {
  char decoded[TURBO_SELECTOR_MAX_STRING_BYTES_V1 + 1u];
  size_t input = 1u;
  size_t output = 0u;
  ...
  while (input + 1u < token.length) {
    ...
    if (ch == '\\') {
      ...
      case 'u': {
        ...
        if (codepoint >= 0xd800u && codepoint <= 0xdbffu) {
          ...
          if (input + 6u > token.length - 1u ||
            ...
            selector_ctx_fail(ctx, TURBO_SELECTOR_SEMANTIC_ERROR,
                              token.offset + input,
      ...
      default:
        selector_ctx_fail(ctx, TURBO_SELECTOR_SYNTAX_ERROR,
                          token.offset + input - 1u,
                          "invalid selector string escape");
        return 0;
      }
    }
    if (output >= TURBO_SELECTOR_MAX_STRING_BYTES_V1) goto string_limit;
    decoded[output++] = (char)ch;
  }
  return selector_pool_append(ctx, decoded, output, out_ref, token.offset);

string_limit:
  selector_ctx_fail(ctx, TURBO_SELECTOR_RESOURCE_LIMIT, token.offset,
                    "selector string limit exceeded");
  return 0;
}
...
selector_semantic_t selector_make_binary(selector_parse_ctx_t *ctx,
  ...
  if (node.depth > TURBO_SELECTOR_MAX_DEPTH_V1) {
    selector_ctx_fail(ctx, TURBO_SELECTOR_RESOURCE_LIMIT, left.offset,
...
selector_semantic_t selector_make_not(selector_parse_ctx_t *ctx,
  ...
  if (node.depth > TURBO_SELECTOR_MAX_DEPTH_V1) {
    selector_ctx_fail(ctx, TURBO_SELECTOR_RESOURCE_LIMIT, child.offset,
                      "selector AST depth limit exceeded");
    return selector_invalid_semantic();
  }
  return selector_add_node(ctx, &node);
...
selector_semantic_t selector_make_capability(selector_parse_ctx_t *ctx,
  ...
  if (node.value.length == 0u) {
    selector_ctx_fail(ctx, TURBO_SELECTOR_SEMANTIC_ERROR, value.offset,
...
static int selector_list_add_value(selector_parse_ctx_t *ctx,
                                   selector_semantic_t value,
                                   selector_string_ref_t *out_ref) {
  if (ctx->list_value_count >= TURBO_SELECTOR_MAX_LIST_ITEMS_V1) {
    selector_ctx_fail(ctx, TURBO_SELECTOR_RESOURCE_LIMIT, value.offset,
...
static int selector_schema_valid(const turbo_selector_schema_v1_t *schema) {
  size_t index;
  if (!schema || schema->size < TURBO_SELECTOR_SCHEMA_V1_SIZE ||
      !schema->allowed_fields || schema->allowed_field_count == 0u ||
      schema->allowed_field_count > TURBO_SELECTOR_MAX_SCHEMA_FIELDS_V1)
    return 0;
  for (index = 0u; index < schema->allowed_field_count; ++index) {
    ...
    if (length == 0u || length > TURBO_SELECTOR_MAX_STRING_BYTES_V1)
      return 0;
...
int turbo_selector_compile_v1(
  ...
  int token_id;
  int result = TURBO_SELECTOR_OK;
  if (out_program) *out_program = NULL;
  selector_diagnostic_clear(diagnostic);
  if (!source || source_size == 0u || !selector_schema_valid(schema) ||
      !out_program ||
      (diagnostic && diagnostic->size < TURBO_SELECTOR_DIAGNOSTIC_V1_SIZE)) {
    selector_diagnostic_set(diagnostic, TURBO_SELECTOR_INVALID_ARGUMENT,
                            source, source_size, 0u,
                            "invalid selector compile arguments");
    return TURBO_SELECTOR_INVALID_ARGUMENT;
  }
  if (source_size > TURBO_SELECTOR_MAX_SOURCE_BYTES_V1) {
    selector_diagnostic_set(diagnostic, TURBO_SELECTOR_RESOURCE_LIMIT, source,
                            source_size, TURBO_SELECTOR_MAX_SOURCE_BYTES_V1,
                            "selector source limit exceeded");
    return TURBO_SELECTOR_RESOURCE_LIMIT;
  }
  if (!selector_utf8_validate(source, source_size, &utf8_error)) {
    selector_diagnostic_set(diagnostic, TURBO_SELECTOR_INVALID_UTF8, source,
                            source_size, utf8_error,
                            "selector source is not valid UTF-8");
    return TURBO_SELECTOR_INVALID_UTF8;
  }
  ctx = (selector_parse_ctx_t *)calloc(1u, sizeof(*ctx));
  if (!ctx) {
    selector_diagnostic_set(diagnostic, TURBO_SELECTOR_NO_MEMORY, source,
                            source_size, 0u,
                            "cannot allocate selector compiler state");
    return TURBO_SELECTOR_NO_MEMORY;
  }
  ctx->source = source;
  ctx->source_size = source_size;
  ctx->schema = schema;
  ctx->root = SELECTOR_NO_NODE;
  ctx->status = TURBO_SELECTOR_OK;
  parser = SelectorV1ParseAlloc(malloc);
  if (!parser) {
    result = TURBO_SELECTOR_NO_MEMORY;
  ...
  while ((token_id = selector_lexer_next(&lexer, &token)) > 0) {
    if (++token_count > TURBO_SELECTOR_MAX_TOKENS_V1) {
      selector_ctx_fail(ctx, TURBO_SELECTOR_RESOURCE_LIMIT, token.offset,
    ...
    if (ctx->status != TURBO_SELECTOR_OK) break;
  }
  if (token_id < 0 && ctx->status == TURBO_SELECTOR_OK)
    selector_ctx_fail(ctx, lexer.status, lexer.error_offset, lexer.message);
  if (ctx->status == TURBO_SELECTOR_OK) {
  ...
  if (ctx->status != TURBO_SELECTOR_OK || ctx->root == SELECTOR_NO_NODE ||
      ctx->root >= ctx->node_count) {
    result = ctx->status == TURBO_SELECTOR_OK ? TURBO_SELECTOR_SYNTAX_ERROR
                                              : ctx->status;
    if (ctx->status == TURBO_SELECTOR_OK)
  ...
  program = (turbo_selector_program_t *)calloc(1u, sizeof(*program));
  if (!program) {
    result = TURBO_SELECTOR_NO_MEMORY;
    selector_ctx_fail(ctx, result, 0u,
...
uint32_t turbo_selector_program_language_version(
    const turbo_selector_program_t *program) {
  return program ? TURBO_SELECTOR_LANGUAGE_VERSION_V1 : 0u;
}
...
static int selector_render_node(const turbo_selector_program_t *program,
                                uint16_t node_index, size_t depth,
                                selector_writer_t *writer) {
  const selector_node_t *node;
  size_t index;
  if (depth > TURBO_SELECTOR_MAX_DEPTH_V1 ||
...
int turbo_selector_program_canonical_v1(
  ...
  if (!program || !program->nodes || program->root >= program->node_count ||
      ...
      (diagnostic && diagnostic->size < TURBO_SELECTOR_DIAGNOSTIC_V1_SIZE)) {
    selector_diagnostic_set(diagnostic, TURBO_SELECTOR_INVALID_ARGUMENT, NULL,
  ...
  if (!selector_render_node(program, program->root, 0u, &writer) ||
      writer.overflow ||
      writer.required > TURBO_SELECTOR_MAX_CANONICAL_BYTES_V1) {
    selector_diagnostic_set(diagnostic, TURBO_SELECTOR_RESOURCE_LIMIT, NULL,
  ...
  *required = writer.required;
  ...
  if (!output && output_capacity == 0u) return TURBO_SELECTOR_OK;
  if (output_capacity <= writer.required) {
    selector_diagnostic_set(diagnostic, TURBO_SELECTOR_BUFFER_TOO_SMALL, NULL,
                            0u, 0u, "canonical output buffer is too small");
    return TURBO_SELECTOR_BUFFER_TOO_SMALL;
  }
  return TURBO_SELECTOR_OK;
}
...
static void selector_eval_fail(selector_eval_ctx_t *ctx,
  ...
  ctx->failed = 1;
  selector_diagnostic_set(ctx->diagnostic, TURBO_SELECTOR_EVALUATION_ERROR,
...
int turbo_selector_program_evaluate_v1(
  ...
  if (!program || !ops || ops->size < TURBO_SELECTOR_EVAL_OPS_V1_SIZE ||
  ...
  if (eval.failed) return TURBO_SELECTOR_EVALUATION_ERROR;
