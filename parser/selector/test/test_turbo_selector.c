...
static turbo_selector_schema_v1_t test_schema(void) {
...
static turbo_selector_program_t *compile_selector(
    const char *source, turbo_selector_diagnostic_v1_t *diagnostic) {
  ...
  describe("re2c and Lemon compilation") {
    it("compiles precedence and renders a deterministic canonical form") {
      ...
      static const char expected[] =
          "((region == \"eu-west\" && role in [\"cache\", \"edge\"]) || "
          "capability(\"m3-cache\"))";
      turbo_selector_diagnostic_v1_t diagnostic = {0};
      turbo_selector_program_t *program = compile_selector(source, &diagnostic);
      char canonical[256];
      size_t required = 0u;
    ...
    it("keeps decoded control bytes in a reparsable canonical form") {
      ...
      turbo_selector_diagnostic_v1_t diagnostic = {0};
      turbo_selector_program_t *program = compile_selector(source, &diagnostic);
      turbo_selector_program_t *round_trip = NULL;
      char canonical[64];
      size_t required = 0u;
      ...
      turbo_selector_program_destroy(round_trip);
      turbo_selector_program_destroy(program);
    }
    ...
    it("enforces source and decoded-string byte limits exactly") {
      ...
      turbo_selector_diagnostic_v1_t diagnostic = {
          TURBO_SELECTOR_DIAGNOSTIC_V1_SIZE};
      turbo_selector_program_t *program = NULL;
      char *source = (char *)malloc(TURBO_SELECTOR_MAX_SOURCE_BYTES_V1 + 2u);
      char string_source[TURBO_SELECTOR_MAX_STRING_BYTES_V1 + 16u];
      ...
      memset(string_source + prefix_size, 'a',
             TURBO_SELECTOR_MAX_STRING_BYTES_V1);
      string_source[prefix_size + TURBO_SELECTOR_MAX_STRING_BYTES_V1] = '"';
      ...
      turbo_selector_program_destroy(program);
      program = NULL;
      string_source[prefix_size + TURBO_SELECTOR_MAX_STRING_BYTES_V1] = 'a';
      string_source[prefix_size + TURBO_SELECTOR_MAX_STRING_BYTES_V1 + 1u] =
          '"';
    ...
    it("supports canonical size queries and reports a short buffer") {
      turbo_selector_diagnostic_v1_t diagnostic = {0};
      turbo_selector_program_t *program =
    ...
    it("rejects schema field names that the lexer cannot produce") {
      ...
      turbo_selector_schema_v1_t schema = {
          TURBO_SELECTOR_SCHEMA_V1_SIZE, bad_fields, 1u, 0};
      turbo_selector_diagnostic_v1_t diagnostic = {
          TURBO_SELECTOR_DIAGNOSTIC_V1_SIZE};
      turbo_selector_program_t *program = NULL;
  ...
  describe("fail-closed immutable evaluation") {
    it("evaluates fields tags capabilities and boolean precedence") {
      ...
      selector_record_t record = {"eu-west", "edge", "gold", capabilities,
                                  2u};
      turbo_selector_eval_ops_v1_t ops = {
          TURBO_SELECTOR_EVAL_OPS_V1_SIZE, resolve_field, has_capability};
      turbo_selector_diagnostic_v1_t diagnostic = {0};
      turbo_selector_program_t *program = compile_selector(
          "region == \"eu-west\" && role in [\"cache\", \"edge\"] && "
    ...
    it("keeps missing comparisons unknown through negation") {
      ...
      turbo_selector_eval_ops_v1_t ops = {
          TURBO_SELECTOR_EVAL_OPS_V1_SIZE, resolve_field, has_capability};
      turbo_selector_diagnostic_v1_t diagnostic = {0};
      turbo_selector_program_t *comparison =
          compile_selector("!(region == \"eu\")", &diagnostic);
      turbo_selector_program_t *presence =
      ...
      turbo_selector_program_destroy(presence);
      turbo_selector_program_destroy(comparison);
    }
    ...
    it("reports resolver failure without a partial match") {
      turbo_selector_eval_ops_v1_t ops = {
          TURBO_SELECTOR_EVAL_OPS_V1_SIZE, NULL, has_capability};
      turbo_selector_diagnostic_v1_t diagnostic = {0};
      turbo_selector_program_t *program =
          compile_selector("has(region)", &diagnostic);
      int matched = 1;
