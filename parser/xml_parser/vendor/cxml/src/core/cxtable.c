...
static void _cxml_table_rehash(cxml_table *table, _cxml_table_hash_t hash_type) {
    ...
    replacement.count = 0;
    replacement.capacity = new_capacity;
    replacement.entries = CALLOC(_cxml_ht_entry, new_capacity);
    replacement.keys = table->keys;
    ...
    if (table->count) {  // does the table contain any entry yet?
        ...
        for (int i = 0; i < old_capacity; i++)
        {
            entry = &old_entries[i];
            if (entry->key != NULL) {
                _cxml_table__put(&replacement, entry->key, entry->value,
                                 false, hash_type);
            }
        }
    }
    table->count = replacement.count;
    table->capacity = replacement.capacity;
        ...
        _cxml_table_hash_t hash_type)
{
    ...
    if (table->entries[index].key == NULL){  // new insert operation
        ...
        // Ordering the key allocates.  Complete that operation before the
        // entry publishes ownership of value so allocation failure leaves the
        // table logically unchanged and the caller remains its sole owner.
