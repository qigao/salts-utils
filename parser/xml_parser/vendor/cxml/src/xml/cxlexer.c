        ...
        bool stream)
{
    ...
    cxlexer->line = 1;
    cxlexer->column = 1;
    cxlexer->byte_offset = 0u;
    // escape utf-8 byte order mark if present.
    if (cxlexer->current && has_utf8_bom(cxlexer->current)){
        cxlexer->current += 3;
        cxlexer->start = cxlexer->current;
        cxlexer->column += 3;
        cxlexer->byte_offset += 3u;
    }
    cxlexer->start_line = cxlexer->line;
    cxlexer->start_column = cxlexer->column;
    cxlexer->start_byte_offset = cxlexer->byte_offset;
    cxlexer->document_start_byte_offset = cxlexer->byte_offset;
    cxlexer->error = 0;
    cxlexer->vflag = 0;
...
static _cxml_token new_cxml_token(_cxml_lexer *cxlexer, _cxml_token_t type) {
    _cxml_token token;
    char *untrimmed_start = cxlexer->start;
    token.type = type;
    if (type == CXML_TOKEN_TEXT) {
        token.length = get_length(cxlexer);
        while (untrimmed_start < cxlexer->start) {
            if (*untrimmed_start++ == '\n') {
                cxlexer->start_line++;
                cxlexer->start_column = 1;
            } else {
                cxlexer->start_column++;
            }
            cxlexer->start_byte_offset++;
        }
    } else {
...
static void _atttype(_cxml_lexer *lexer){
    ...
    else if (*lexer->current == 'I')
    {
        ...
        if (*lexer->current == 'R'){
            // REF
            _move(lexer, 3);
            if (*lexer->current == 'S') cxml__move(lexer);
        }
    }
    else if (*lexer->current == 'E'){
        // ENTIT
        _move(lexer, 5);
        if (*lexer->current == 'Y') cxml__move(lexer); // Y
...
static _cxml_token lex_dtd(_cxml_lexer *lexer){
    ...
    if (_is_id(*lexer->current)){ // NAME
        ...
        lexer->start_byte_offset = lexer->byte_offset;
        _cxml_token token;
        ...
        token.byte_offset = lexer->start_byte_offset;
        token.literal_type = CXML_NON_LITERAL;
        ...
        return new_cxml_token(lexer, CXML_TOKEN_ERROR);
    }
}
