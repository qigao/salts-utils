...
TURBO_PARSER_API int turbo_parse_yaml(const uint8_t *data, size_t len, turbo_yaml_doc_t **out);
/**
 ...
 */
TURBO_PARSER_API int turbo_parse_yaml_ex(const uint8_t *data, size_t len, turbo_yaml_doc_t **out,
                                  turbo_yaml_error_t *error);
TURBO_PARSER_API int turbo_parse_yaml_sax(const uint8_t *data, size_t len,
 ...
 */
TURBO_PARSER_API turbo_yaml_sax_parser_t *
turbo_yaml_sax_parser_create(const turbo_yaml_sax_handler_t *handler, void *ctx);
TURBO_PARSER_API int turbo_yaml_sax_parser_feed(turbo_yaml_sax_parser_t *parser, const char *data,
                                         size_t len);
TURBO_PARSER_API int turbo_yaml_sax_parser_finish(turbo_yaml_sax_parser_t *parser);
TURBO_PARSER_API const char *turbo_yaml_sax_parser_error(const turbo_yaml_sax_parser_t *parser);
TURBO_PARSER_API void turbo_yaml_sax_parser_destroy(turbo_yaml_sax_parser_t *parser);

/** Free a YAML document and set its pointer to NULL. */
TURBO_PARSER_API void turbo_free_yaml(turbo_yaml_doc_t **doc);

TURBO_PARSER_API turbo_yaml_node_t *turbo_yaml_root(const turbo_yaml_doc_t *doc);
TURBO_PARSER_API turbo_yaml_node_type_t turbo_yaml_node_type(const turbo_yaml_node_t *node);
TURBO_PARSER_API turbo_yaml_scalar_kind_t turbo_yaml_scalar_kind(const turbo_yaml_doc_t *doc,
                                                          const turbo_yaml_node_t *node);

/** Return a processed scalar string. Free it with turbo_yaml_string_free(). */
TURBO_PARSER_API char *turbo_yaml_scalar_dup(const turbo_yaml_doc_t *doc, const turbo_yaml_node_t *node);
TURBO_PARSER_API size_t turbo_yaml_sequence_size(const turbo_yaml_node_t *node);
TURBO_PARSER_API turbo_yaml_node_t *turbo_yaml_sequence_get(const turbo_yaml_node_t *node, size_t index);
TURBO_PARSER_API size_t turbo_yaml_mapping_size(const turbo_yaml_node_t *node);
TURBO_PARSER_API turbo_yaml_node_t *turbo_yaml_mapping_key(const turbo_yaml_node_t *node, size_t index);
TURBO_PARSER_API turbo_yaml_node_t *turbo_yaml_mapping_value(const turbo_yaml_node_t *node, size_t index);
/** Return a borrowed mapping value for an exact, null-terminated key. */
TURBO_PARSER_API turbo_yaml_node_t *turbo_yaml_mapping_get(const turbo_yaml_doc_t *doc,
                                                    const turbo_yaml_node_t *node, const char *key);
TURBO_PARSER_API bool turbo_yaml_mapping_contains(const turbo_yaml_doc_t *doc,
                                           const turbo_yaml_node_t *node, const char *key);

/** Copy source coordinates for a borrowed node into caller-owned storage. */
TURBO_PARSER_API bool turbo_yaml_node_location(const turbo_yaml_node_t *node,
                                        turbo_yaml_location_t *location);
/** Return the borrowed resolved target of an alias node, or NULL. */
TURBO_PARSER_API turbo_yaml_node_t *turbo_yaml_alias_target(const turbo_yaml_node_t *node);

/** Execute a YPATH expression relative to context, or the root when context is NULL. */
TURBO_PARSER_API turbo_yaml_path_result_t *turbo_yaml_path_query(const turbo_yaml_doc_t *doc,
...
TURBO_PARSER_API turbo_yaml_path_result_t *turbo_yaml_path_query_ex(
...
TURBO_PARSER_API size_t turbo_yaml_path_result_size(const turbo_yaml_path_result_t *result);
TURBO_PARSER_API turbo_yaml_node_t *turbo_yaml_path_result_get(const turbo_yaml_path_result_t *result,
                                                        size_t index);
TURBO_PARSER_API const char *turbo_yaml_path_result_error(const turbo_yaml_path_result_t *result);
TURBO_PARSER_API size_t turbo_yaml_path_result_error_pos(const turbo_yaml_path_result_t *result);
TURBO_PARSER_API void turbo_yaml_path_result_free(turbo_yaml_path_result_t *result);

/** Emit a document or node as YAML. Free serialized output with turbo_yaml_serialize_free(). */
TURBO_PARSER_API char *turbo_yaml_emit(const turbo_yaml_doc_t *doc, size_t *out_len);
TURBO_PARSER_API char *turbo_yaml_serialize(const turbo_yaml_doc_t *doc, size_t *out_len);
TURBO_PARSER_API char *turbo_yaml_emit_node(const turbo_yaml_doc_t *doc, const turbo_yaml_node_t *node,
                                     size_t *out_len);
TURBO_PARSER_API void turbo_yaml_string_free(char *str);
TURBO_PARSER_API void turbo_yaml_serialize_free(char *str);
TURBO_PARSER_API int turbo_yaml_write(const turbo_yaml_doc_t *doc, turbo_write_fn write, void *user);
TURBO_PARSER_API turbo_yaml_doc_t *turbo_yaml_from_json(const json_value_t *value);

/** Convert representable YAML semantics into an independently owned JSON DOM. */
TURBO_PARSER_API json_value_t *turbo_yaml_to_json(const turbo_yaml_doc_t *doc);
TURBO_PARSER_API json_value_t *turbo_yaml_node_to_json(const turbo_yaml_doc_t *doc,
                                                const turbo_yaml_node_t *node);
