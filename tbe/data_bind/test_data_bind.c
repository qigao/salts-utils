...
suite("Data Bind") {
  ...
  group("Variable-length String Parsing") {
    ...
    given("a schema with var-data followed by fixed field") {
      ...
      then("codec creation should reject the invalid schema order") {
        check_null(codec);
      }
      data_bind_free(codec);
    ...
    given("a schema with fixed field followed by var-data") {
      ...
      then("JSON binding should follow field names rather than schema order") {
        check_not_null(codec);
        if (codec) {
