...
int turbo_parse_xml(const uint8_t *data, size_t len, turbo_xml_doc_t **out) {
  ...
  *out = NULL;
  ...
  if (salts_xml_parse(&document->document, (const char *)data, len, NULL, NULL) !=
      SALTS_XML_OK) {
    free(document);
    return -1;
  }
...
void turbo_free_xml(turbo_xml_doc_t **out) {
  if (!out || !*out) return;
  salts_xml_document_destroy(&(*out)->document);
  free(*out);
  *out = NULL;
}

char *turbo_xml_serialize(const turbo_xml_doc_t *doc, size_t *out_len) {
  return doc ? salts_xml_document_serialize(&doc->document, out_len) : NULL;
}
...
int turbo_xml_write(const turbo_xml_doc_t *doc, turbo_write_fn write, void *user) {
  ...
  if (!text) return -1;
  rc = write(text, len, user);
  salts_xml_owned_string_free(text);
  return rc == 0 ? 0 : -1;
}

void turbo_xml_serialize_free(char *str) { salts_xml_owned_string_free(str); }
...
turbo_xml_doc_t *turbo_xml_create_document(const char *root_name) {
  ...
  if (!doc) return NULL;
  if (salts_xml_document_create(&doc->document, root_name) != SALTS_XML_OK) {
    free(doc);
    return NULL;
  }
  return doc;
...
turbo_xml_node_t *turbo_xml_add_element(void *parent, const char *name) {
  salts_xml_node element = {NULL};
  const salts_xml_node parent_node = {parent};
  if (!parent || !name || !name[0]) return NULL;
  if (salts_xml_node_add_element(parent_node, name, &element) != SALTS_XML_OK)
    return NULL;
...
int turbo_xml_set_text(turbo_xml_node_t *node, const char *text) {
  ...
  return salts_xml_node_set_text((salts_xml_node){node}, text) == SALTS_XML_OK ? 0 : -1;
}

turbo_xml_node_t *turbo_xml_root_element(const turbo_xml_doc_t *doc) {
  salts_xml_node root;
  if (!doc) return NULL;
  root = salts_xml_document_root(&doc->document);
  return (turbo_xml_node_t *)root.impl;
}

const char *turbo_xml_node_name(const turbo_xml_node_t *node) {
  const salts_xml_string_view name = salts_xml_node_display_name((salts_xml_node){node});
  return name.data;
}
...
void turbo_xml_find_all(turbo_xml_node_t *root, const char *query, turbo_xml_list_t *out) {
  ...
  if (salts_xml_node_find_all((salts_xml_node){root}, query, &matches) != SALTS_XML_OK)
    return;
  for (index = 0u; index < salts_xml_node_list_size(&matches); ++index) {
    const salts_xml_node node = salts_xml_node_list_at(&matches, index);
    if (!turbo_xml_list_append(out, (void *)node.impl)) break;
  }
  salts_xml_node_list_destroy(&matches);
}

char *turbo_xml_text_dup(turbo_xml_node_t *node) {
  return node ? salts_xml_node_text_dup((salts_xml_node){node}) : NULL;
}
...
char *turbo_xml_child_text_dup(turbo_xml_node_t *parent, const char *name) {
  ...
  for (index = 0u; index < salts_xml_node_child_count(parent_node); ++index) {
    const salts_xml_node child = salts_xml_node_child_at(parent_node, index);
    const salts_xml_string_view local_name = salts_xml_node_local_name(child);
    if (salts_xml_node_type(child) == SALTS_XML_ELEMENT && local_name.data != NULL &&
        local_name.size == strlen(name) && memcmp(local_name.data, name, local_name.size) == 0) {
      char *text = salts_xml_node_text_dup(child);
...
const char *turbo_xml_get_text(const turbo_xml_doc_t *doc, const char *xpath) {
  salts_xml_node_list matches = {0};
  salts_xml_node node;
  salts_xml_string_view text;
  salts_xml_node_kind type;
  if (!doc || !xpath ||
      salts_xml_document_xpath_query(&doc->document, xpath, &matches, NULL, NULL) !=
          QVM_STATUS_OK ||
      salts_xml_node_list_size(&matches) == 0u) {
    salts_xml_node_list_destroy(&matches);
    return NULL;
  }
  node = salts_xml_node_list_at(&matches, 0u);
  type = salts_xml_node_type(node);
  text = salts_xml_node_text_view(node);
  salts_xml_node_list_destroy(&matches);
  if (type == SALTS_XML_ELEMENT) return text.data ? text.data : "";
  return type == SALTS_XML_TEXT ? text.data : NULL;
}
...
size_t turbo_xml_count(const turbo_xml_doc_t *doc, const char *xpath) {
  ...
  if (!doc || !xpath ||
      salts_xml_document_xpath_query(&doc->document, xpath, &matches, NULL, NULL) !=
          QVM_STATUS_OK)
    return 0u;
  count = salts_xml_node_list_size(&matches);
  salts_xml_node_list_destroy(&matches);
  return count;
}
...
void turbo_xml_xpath_query(const turbo_xml_doc_t *doc, const char *xpath, turbo_xml_list_t *out) {
  ...
  turbo_xml_list_init(out);
  if (!doc || !xpath) return;
  if (salts_xml_document_xpath_query(&doc->document, xpath, &matches, NULL, NULL) !=
      QVM_STATUS_OK)
    return;
  for (index = 0u; index < salts_xml_node_list_size(&matches); ++index) {
    const salts_xml_node node = salts_xml_node_list_at(&matches, index);
...
turbo_query_status_t turbo_xml_xpath_query_ex(
  ...
  turbo_xml_list_init(out);
  turbo_qvm_diagnostic_init(&native_diagnostic);
  status = salts_xml_document_xpath_query(&doc->document, xpath, &matches,
...
static salts_xml_node_kind turbo_parser_xml_node_kind(
    const turbo_xml_xpath_node_t *node) {
  return salts_xml_node_type((salts_xml_node){node});
}
...
turbo_xml_node_type_t turbo_xml_xpath_node_type(const turbo_xml_xpath_node_t *node) {
  ...
  case SALTS_XML_ELEMENT:
    return TURBO_XML_NODE_ELEMENT;
  case SALTS_XML_COMMENT:
    return TURBO_XML_NODE_COMMENT;
  case SALTS_XML_ATTRIBUTE:
    return TURBO_XML_NODE_ATTRIBUTE;
  case SALTS_XML_DOCUMENT:
    return TURBO_XML_NODE_ROOT;
  case SALTS_XML_PROCESSING_INSTRUCTION:
    return TURBO_XML_NODE_PI;
  case SALTS_XML_NAMESPACE:
    return TURBO_XML_NODE_NAMESPACE;
  case SALTS_XML_XML_HEADER:
...
const char *turbo_xml_xpath_node_name(const turbo_xml_xpath_node_t *node) {
  const salts_xml_string_view name =
      salts_xml_node_display_name((salts_xml_node){node});
  return name.data;
}

const char *turbo_xml_xpath_node_text(const turbo_xml_xpath_node_t *node) {
  const salts_xml_string_view text =
      salts_xml_node_text_view((salts_xml_node){node});
  return text.data;
}

char *turbo_xml_xpath_node_xml_dup(const turbo_xml_xpath_node_t *node) {
  return node ? salts_xml_node_serialize((salts_xml_node){node}, NULL) : NULL;
}

void turbo_xml_string_free(char *str) { salts_xml_owned_string_free(str); }
